...
Orders: customer,email,invoice #,date,status,product,qty,unit price,total</pre><p>Use YYYY-MM-DD dates. Review names, order numbers, totals, and payment states in a small sample first. Customer duplicates match by email; records without email need manual review. PDF invoices, artwork files, mailbox archives, and custom fields are not imported by this CSV tool.</p><div class="setup-actions"><button class="btn ghost" id="setup-customers">Import customers</button><button class="btn ghost" id="setup-orders">Import order history</button><a class="btn ghost" href="/api/export/all.json" download>Export shop backup</a></div></details>
    ...
    <section class="setup-section"><h2>3. Choose how much help you want</h2><p>AI is an optional assistant. You keep all of the normal screens and controls.</p>
      <div class="setup-row"><span class="setup-number">05</span><div><h3>Work manually</h3><p>Turn off the model and set assisted workflows to ask you first. Independently configured automation rules can be reviewed under More tools → Automations.</p><span class="setup-status">${aiOn ? 'An AI provider is selected' : 'AI is off'}</span></div><button class="btn ghost" id="setup-manual">Use manual controls</button></div>
      ${row('06','Add an AI model','Optional parsing and reply drafts using your own provider account. Test the connection in settings.','#/settings?section=ai','AI settings',aiOn ? 'Provider selected — test to verify' : 'Optional — off')}
      ${row('07','Connect Slack','Create your Slack app from a ready-made setup file. The built-in /quote command creates draft estimates for review.','#/settings?section=slack','Connect Slack',s.slack_bot_token_set && s.slack_signing_secret_set ? 'Credentials saved — test in Slack settings' : 'Optional — not connected')}
