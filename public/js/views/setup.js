...
Orders: customer,email,invoice #,date,status,product,qty,unit price,total</pre><p>Use YYYY-MM-DD dates. Review names, order numbers, totals, and payment states in a small sample first. Customer duplicates match by email; records without email need manual review. PDF invoices, artwork files, mailbox archives, and custom fields are not imported by this CSV tool.</p><div class="setup-actions"><button class="btn ghost" id="setup-customers">Import customers</button><button class="btn ghost" id="setup-orders">Import order history</button><a class="btn ghost" href="/api/export/all.json" download>Export shop backup</a></div></details>
    ...
    <section class="setup-section"><h2>Replace your old CRM</h2><p>Preview a GHL customer, deal and follow-up task migration. Keep messages held while your team reviews the cutover.</p><div class="setup-actions"><a class="btn ghost" href="#/crm-migration">Move your CRM</a><a class="btn ghost" href="#/communication-holds">Communication holds</a></div></section>
    <section class="setup-section"><h2>Suppliers & production</h2><div class="setup-actions"><a class="btn ghost" href="#/suppliers">Connect suppliers</a><a class="btn ghost" href="#/production/workflows">Workflow templates</a><a class="btn ghost" href="#/matrices">Price matrices</a><a class="btn ghost" href="#/costing/settings">Shop costs & machines</a></div></section>
    ...
    <section class="setup-section"><h2>4. Choose how much help you want</h2><p>AI is an optional assistant. You keep all of the normal screens and controls.</p>
      ...
      <p><a class="btn ghost" href="#/agent-keys">Connect your own agent</a> <span class="dim">Separate keys, selected permissions and individual revocation.</span></p><details><summary>Legacy shared API key</summary><p>Already use an agent in Slack? Give that agent your shop API endpoint and a PrintShopCRM key. Your agent keeps its existing Slack and model connection. This is different from the optional model key above.</p>
