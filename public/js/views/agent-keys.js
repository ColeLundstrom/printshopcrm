...
export async function agentKeysView(){
  ...
  $('#view').innerHTML=`<div class="stack production-page">
    ...
    <section class="card card-b"><h3>Your connections</h3><p>Keys are shown only once when created. To replace a key, create a new connection, update the agent, test it, then revoke the old key.</p><div id="agent-key-list">${d.keys.length?d.keys.map(k=>{
      ...
      return `<article class="card card-b"><h4>${esc(k.name)}</h4><p>${esc(k.prefix)}… · ${k.revoked_at?'Revoked':expired?'Expired':'Active'} · Expires ${esc(k.expires_at.slice(0,10))}</p><p>${k.scopes.map(s=>esc(d.scopes[s] || s)).join(' · ')}</p><p class="dim">Last request: ${esc(k.last_used_at || 'Not used yet')}</p><button class="btn ghost" data-agent-audit="${k.id}">Recent requests</button> ${inactive?'':`<button class="btn ghost" data-agent-revoke="${k.id}" data-name="${esc(k.name)}">Revoke access</button>`}<div id="agent-audit-${k.id}"></div></article>`
