...
function renderList() {
  ...
  $('#gsb-list').innerHTML = designs.map((d) => `<div class="gsb-item">
    ...
    <div class="gsb-item-a">
      <button class="btn ghost sm" data-dup="${d.id}" title="Add a separate copy of this design as its own row" aria-label="Add another copy of ${esc(d.name || 'this design')} as its own row">＋</button>
