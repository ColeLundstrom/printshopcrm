...
async function run() {
  ...
  try {
    ...
    try { await api.post(`/api/jobs/${r.job.id}/separation`, separateArt(artImg, r.order.dark_garment, garment.name)) } catch { /* bonus */ }
...
const garmentFor = (order) => {
  ...
  return S.GARMENT_COLORS.find((g) => g.name === name) || S.GARMENT_COLORS[0]
}
...
function renderMockup(artImg, garmentHex) {
  ...
  const [r, gg, b] = S.hexToRgb(garmentHex)
...
/** Run the separation engine on the art so the job carries its screen count. */
function separateArt(artImg, dark, garmentName) {
  const size = 260
  const cc = document.createElement('canvas'); cc.width = size; cc.height = size
  const cx = cc.getContext('2d', { willReadFrequently: true })
  const ar = artImg.width / artImg.height
  let dw = size, dh = size / ar; if (dh > size) { dh = size; dw = size * ar }
  cx.drawImage(artImg, (size - dw) / 2, (size - dh) / 2, dw, dh)
  const data = cx.getImageData(0, 0, size, size)
  const palette = S.quantize(data, 4)
  const inks = palette.map((p) => ({ name: S.inkName(p.rgb), rgb: p.rgb, hex: p.hex }))
  return { colors: inks.length, screens: S.screenCount(inks, dark), dark: !!dark, garment: garmentName, mode: 'spot',
    inks: inks.map((i) => ({ name: i.name, hex: i.hex })) }
}
