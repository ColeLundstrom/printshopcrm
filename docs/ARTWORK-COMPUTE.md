...
Autopilot's optional concept uses a fixed-size browser canvas. It leaves missing art missing, labels the generic garment illustration, and never uploads that illustration as a customer proof. The illustration is not an exact catalog mockup. There is no native catalog/photo compositor, hosted rendering service, artwork-provider adapter, local worker connector, paid compute plan, PSD editor integration or built-in separation engine in this release. No new vendor account has been connected or billed for this review.
...
These compute and billing controls are requirements for future adapters, not installed services or an existing paid compute offering. The manual CRM core remains usable without AI or a paid artwork connector. Default future rendering should use the shop's browser/hardware or its own provider account; hosted processing must be a separately priced, explicitly enabled option with the caps and metering above. No silent fallback may move a local/BYO job onto the CRM operator's compute bill.
...
Delivered foundation: separate source/prepared-file handoff, appearance approvals and explicit staff technical release. Next delivery order: native catalog/photo compositor and richer source preflight; optional PSD editor; shop-owned worker/provider adapters; separately metered hosted compute last. No paid connector should gate quoting, proofs, production, invoicing or other manual workflows.
