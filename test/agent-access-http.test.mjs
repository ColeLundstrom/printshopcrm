...
test('scoped agent API keeps cookie/shop boundaries, gates production and logs individual requests',{timeout:120000},async()=>{
  ...
  try{
    ...
    const request=(path,method='GET',body,token,cookieValue=cookie)=>fetch(base+path,{method,headers:{'Content-Type':'application/json',Cookie:cookieValue,...(token?{Authorization:'Bearer '+token}:{})},body:body===undefined?undefined:JSON.stringify(body)})
