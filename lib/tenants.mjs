...
import { importCheckpointActive } from './import-checkpoints.mjs'
import { createAgentAccess } from './agent-access.mjs'
/**
...
import { DB_PATH, initDb, tenantStore, seedSettings, setSetting, isSqliteFile } from './db.mjs'
...
import { createHostingCheckouts } from './hosting-checkouts.mjs'
import { createHostingDeletion } from './hosting-deletion.mjs'

const iso = (d) => d.toISOString().replace('T', ' ').slice(0, 19)

const DATA_DIR = dirname(DB_PATH)
export const usage = createUsage(join(DATA_DIR, 'usage.db'))
...
/**
 * Is there actually a database at this path?
 *
 * `existsSync` was the test everywhere, and a zero-byte file exists. SQLite opens one happily and
 * treats it as a brand-new empty database, so every "the file is gone" guard in this module —
 * each one written because losing a shop silently is the worst thing this product can do — was
 * blind to the commonest way a database file actually goes wrong. A disk that filled mid-write, a
 * `cp` that was interrupted, a restore that copied a zero-length artifact, a volume that came back
 * up thin: all of them leave a file that is there and holds nothing.
 *
 * Sixteen bytes off the front. Every SQLite file on earth begins with the same header string.
 */
// isSqliteFile now lives in lib/db.mjs, beside the default handle it also has to guard.

/**
 * Refuse to invent a shop registry.
 *
 * control.db is the list of every shop on the box. `new DatabaseSync(path)` creates on open, so a
 * control.db that was missing or zero bytes was silently rebuilt EMPTY — and the result is the
 * quietest catastrophe in the product. Driven on a two-shop install: every owner is told "Wrong
 * email or password", both shops' databases are still sitting untouched under data/tenants/, the
 * boot log says nothing at all, and `/health` AND `/health?strict=1` both answer 200 — so
 * deploy/ship.sh, which polls exactly that to decide whether to roll a release back, calls it a
 * success. The one tool INSTALL.md names for a lockout then misdiagnoses it: `admin.mjs
 * list-shops` reports "this is the wrong database" while pointed at the right one.
 *
 * Creating a registry is only ever correct on a genuinely new install. If shop databases are on
 * disk, the registry that lists them is not missing-because-new, it is missing-because-lost, and
 * the honest answer is to stop with a reason rather than serve 200 OK over an empty box. A
 * supervisor restart loop with a loud cause beats a silent one that looks healthy.
 */
const shopsOnDisk = () => {
  // Only meaningful when the registry and the shop directories are the same install. PSC_CONTROL_DB
  // can point the registry somewhere else entirely (the gate does exactly that), and then "there
  // are shops under data/tenants" says nothing about the registry being asked for.
  if (dirname(CONTROL_PATH) !== DATA_DIR) return 0
  try {
    return readdirSync(TENANTS_DIR, { withFileTypes: true })
      .filter((e) => e.isDirectory() && existsSync(join(TENANTS_DIR, e.name, 'printshop.db'))).length
  } catch { return 0 }
}
const refuseControl = (why, extra) => {
  // Not a throw: this runs at module scope, before server.mjs installs its startup handlers, so a
  // throw here is a raw stack trace at the worst possible moment for this file to stop speaking
  // English. Say what is wrong, say that nothing was overwritten, name the tool that fixes it.
  console.error(`\n  PrintShopCRM will not start — ${why}`)
  console.error(`\n  ${extra}`)
  console.error(`  Nothing has been overwritten. Restore it from a snapshot:\n`)
  console.error(`    npm run restore -- /path/to/backup\n`)
  process.exit(1)
}
if (existsSync(CONTROL_PATH) && !isSqliteFile(CONTROL_PATH)) {
  let size = '?'
  try { size = String(statSync(CONTROL_PATH).size) } catch { /* report what we can */ }
  refuseControl(`the shop registry at ${CONTROL_PATH} is not a database (${size} bytes).`,
    `Starting anyway would rebuild it empty, and every shop on this box would be told "Wrong email or password"\n  while its books sat untouched under ${TENANTS_DIR}.`)
}
if (!existsSync(CONTROL_PATH)) {
  const n = shopsOnDisk()
  if (n > 0) {
    refuseControl(`the shop registry at ${CONTROL_PATH} is gone, but ${n} shop database(s) are still on disk.`,
      `Creating a fresh registry is only correct on a new install. Here it would hide ${n} shop(s) under\n  ${TENANTS_DIR} and lock out every owner, with /health still answering 200.`)
  }
}
...
// handle in the product that had no busy_timeout — lib/db.mjs gives every tenant handle one.
...
//
// BEFORE journal_mode, not after. Setting journal_mode is itself a locking operation — it takes
// an exclusive lock to convert the file — and `VACUUM INTO` writes its copies in
// `journal_mode=delete`, so every RESTORED control.db needs that conversion on first open. This
// line runs at MODULE SCOPE, so with any second reader attached it threw `database is locked` at
// 0 ms and killed the whole process at import, with a raw stack, bypassing the app's own friendly
// startup handler — every shop on the box at once, in a 3-second Restart=always loop, immediately
// after a restore. Measured: throws at 0 ms in the shipped order, survives at 1366 ms in this one.
...
for (const [col, decl] of [
  ...
  ['hosting_revision', 'INTEGER NOT NULL DEFAULT 0'],
...
export const agentAccess = createAgentAccess(control)
export const hostingCheckouts = createHostingCheckouts(control)
const hostingDeletion = createHostingDeletion(control, { checkouts: hostingCheckouts,
  finalize: (id, token, evidence) => deleteTenantInternal(id, token, evidence) })

// SQLite may reuse the highest deleted INTEGER PRIMARY KEY. Billing receipts and delayed
// provider events outlive a shop, so a new shop must never inherit that numeric identity.
// Keep a durable high-water mark, including IDs retained by pre-existing control history.
control.exec(`CREATE TABLE IF NOT EXISTS tenant_identity_sequence (
  singleton INTEGER PRIMARY KEY CHECK(singleton=1), last_id INTEGER NOT NULL CHECK(last_id>=0)
);
INSERT OR IGNORE INTO tenant_identity_sequence(singleton,last_id) VALUES(1,0);`)
for (const {name} of control.prepare("SELECT name FROM sqlite_schema WHERE type='table'").all()) {
  const table='"'+name.replaceAll('"','""')+'"'
  const column=name==='tenants'?'id':'tenant_id'
  if(!control.prepare(`PRAGMA table_info(${table})`).all().some(c=>c.name===column)) continue
  control.exec(`UPDATE tenant_identity_sequence SET last_id=MAX(last_id,COALESCE((SELECT MAX("${column}") FROM ${table}),0)) WHERE singleton=1`)
}
const nextTenantId = () => {
  // Called only inside createTenant's BEGIN IMMEDIATE. The increment commits with registration;
  // later provisioning cleanup intentionally leaves the consumed identity behind.
  const previous=control.prepare('SELECT last_id FROM tenant_identity_sequence WHERE singleton=1').get()?.last_id
  if(!Number.isSafeInteger(previous) || previous < 0 || previous >= Number.MAX_SAFE_INTEGER) throw Object.assign(new Error('Shop registration needs server-operator attention.'),{code:'tenant_identity_exhausted'})
  const id=previous+1
  control.prepare('UPDATE tenant_identity_sequence SET last_id=? WHERE singleton=1').run(id)
  return id
}
 ...
 * AsyncLocalStorage, which turns a memory optimisation into a use-after-close crash. So we keep
 * handles and bound the memory instead. The counter is a tripwire, not a limit.
 *
 * File descriptors ARE a constraint, and this comment used to say they were not. It cited "the
 * service's limit is 524288" — that is systemd's HARD default, not the soft one, and
 * deploy/printshopcrm.service set no LimitNOFILE at all, so the process actually ran with the
 * usual soft limit of 1024. At four descriptors per shop, and with boot opening every shop, that
 * runs out somewhere around 245 shops — BELOW the 250 tripwire, so the warning this counter exists
 * to print could never fire before the failure it was warning about. Driven at `ulimit -n 200`:
 * shop 60's owner logged in fine and every screen answered "Something went wrong on our end.",
 * while shop 1 was perfectly healthy. The unit now sets the limit the comment always assumed, and
 * the tripwire fires below the failure point rather than above it.
 */
const MAX_OPEN_DBS = Number(process.env.PSC_MAX_OPEN_DBS || 200)
/** Descriptors per open shop: the database, its -wal, its -shm, and the journal. */
const FDS_PER_DB = 4
...
/**
 * Open a tenant's database. Cached per process. Only `create: true` may bring one into existence.
 *
 * This used to let SQLite create the file, because `new DatabaseSync(path)` creates on open. So a
 * tenant database that had been deleted, renamed, restored to the wrong path or lost with a mount
 * came back as an EMPTY database, bootstrapDb ran the schema against it perfectly happily, and
 * nothing anywhere threw. Measured end to end: a shop with a customer and a paid invoice, .db
 * removed, server restarted — the owner logs in to a blank shop, brokenTenants stays empty,
 * listTenantsAdmin reports 0 customers and $0 revenue (indistinguishable from a new signup), and
 * /health answers 200 {"ok":true}.
 *
 * That last one is the reason this is a crit rather than a papercut: deploy/ship.sh polls /health
 * to decide whether to roll a release back. The whole brokenTenants → 503 → automatic-rollback
 * chain was built for exactly this class of failure, and it could not see the worst member of it —
 * a deploy that lost a shop's books was reported as a success.
 *
 * So: createTenant creates. Everything else opening a file that is not there is an error that says
 * so and names the tool that fixes it.
 */
export function openTenantDb(slug, { create = false } = {}) {
  ...
  // Not existsSync: a zero-byte file exists, and SQLite would treat it as a brand-new empty
  // database. bootstrapDb then writes the whole schema into it, the owner lands in a blank shop
  // with /health still green, and the shop starts minting EST-1001 over the top of its own
  // history. Driven: a shop with two customers, EST-1001/1002, INV-1001 and a $300 payment,
  // truncated to 0 bytes — /api/dashboard answered 200 with every KPI zero and the boot log said
  // nothing. The same file REMOVED was already handled correctly, which is the whole point: this
  // is the identical outcome through a guard that only knew how to see absence.
  if (!create && !isSqliteFile(path)) {
    let size = null
    try { size = statSync(path).size } catch { /* not there at all */ }
    const detail = size === null
      ? `its database file is missing (${path})`
      : `its database file is empty or is not a database (${path}, ${size} bytes)`
    brokenTenants.set(slug, detail)
    const e = new Error(`Shop "${slug}" cannot be opened — ${detail}. Restore it from a snapshot with: npm run restore`)
    e.code = 'tenant_db_missing'
    throw e
  }
  // The mirror case, and the one that is a tenant-isolation failure rather than an outage: a
  // database that is ALREADY THERE when a brand-new shop is being created. `create: true` has
  // exactly one call site (createTenant), so reaching this means a slug is about to be handed to
  // an owner who is not the one whose work is in the file.
  //
  // It is reachable because uniqueSlug consulted only the `tenants` table while baseSlug strips
  // punctuation — "Rebel Ink Press" and "Rebel Ink & Press!!" are one slug — and because
  // deleteTenantFully's rmSync was wrapped in a bare `catch {}`, so a delete that removed the
  // rows and failed to remove the directory still answered `{ ok: true }` to the Control Room.
  // Driven: the next signup, a different shop with a different owner, opened the old one's books —
  // an unpaid $4,850 invoice, a $1,200 payment, the customer list — and its own onboarding then
  // overwrote that shop's letterhead and reset its tax rate to 0.
  //
  // Refuse, loudly, and name the file. Adopting it silently is the one outcome that must never
  // happen; a signup that fails is recoverable, and createTenant now unwinds cleanly around this.
  if (create && existsSync(path)) {
    const detail = `a database already exists at ${path}`
    const e = new Error(`Cannot create shop "${slug}" — ${detail}. It belongs to a shop that was deleted without its data being removed; move that directory aside before the name is reused.`)
    e.code = 'tenant_db_exists'
    throw e
  }
  ...
  if (openDbs.size > MAX_OPEN_DBS && !warnedOpenDbs) {
    warnedOpenDbs = true
    // Print the number that actually matters — a descriptor budget, not a database count.
    console.warn(`[tenants] ${openDbs.size} tenant databases open (tripwire ${MAX_OPEN_DBS}), about ${openDbs.size * FDS_PER_DB} file descriptors. Check LimitNOFILE on the service.`)
  }
  return dbh
}

/** Run a function inside a tenant's database context (query helpers resolve to it). */
export function withTenant(slug, fn, { create = false } = {}) {
  return tenantStore.run({ db: openTenantDb(slug, { create }), slug }, fn)
}
...
function uniqueSlug(name) {
  ...
  let slug = base
  let n = 1
  // The registry is not the only place a slug can be taken. A shop's data lives on disk under the
  // slug, and the two can disagree — a delete whose rmSync failed leaves the directory behind, and
  // baseSlug maps several shop names onto one slug anyway. Skipping a name whose directory exists
  // costs nothing and keeps the openTenantDb guard as a backstop rather than the only defence.
  const taken = (s) => control.prepare('SELECT 1 FROM tenants WHERE slug = ?').get(s) || existsSync(join(TENANTS_DIR, s))
...
/**
 * Is this shop open for business at all?
 *
 * Suspension is the operator's only lever — for non-payment, and for a shop using the platform to
 * phish or defraud. getSession() and getTenantByApiKey() both enforce it, so the owner's login and
 * the REST API close. Nothing else did: the embed routes and the /p/ customer pages resolved a
 * tenant by embed key or slug and never looked at status, so a suspended shop kept answering
 * customers as itself through the AI receptionist (on the shop's own AI key), kept opening Stripe
 * checkouts on the shop's account, and kept writing numbered estimates into a database no human
 * could now open to see them. Measured on a suspended shop: owner login 401, while
 * POST /api/embed/gangsheet/order returned EST-1002 to an anonymous stranger.
 *
 * `status` is only ever 'active' or 'suspended' (setTenantStatus normalises anything else). Trial
 * and subscription state live in `subscription_status` and are a different question.
 */
export const tenantOpen = (t) => !!t && t.status === 'active'
...
export async function createTenant({ shop_name, owner_name, owner_email, password }) {
  ...
  const attempt = () => {
    ...
    try {
      ...
      control.prepare(
        'INSERT INTO tenants (id, slug, shop_name, owner_name, owner_email, password_hash, embed_key, plan, status, trial_ends_at, subscription_status) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
      ).run(tenantId, slug, shop_name.trim(), (owner_name || '').trim(), email, hash, embed_key, 'trial', 'active', trialEnds, 'trialing')
  ...
  // Spin up the isolated database and stamp the shop's own name into its settings. This is the
  // ONE call site allowed to bring a tenant database into existence — see openTenantDb.
  //
  // Inside the same failure boundary as the control-plane rows above. It used to sit outside any
  // rollback, after the COMMIT, and the consequences of that were the worst kind: an account that
  // exists and cannot work, that its owner cannot escape, and that nobody can recreate.
  //
  // Reproduced from an unwritable data directory (the state `sudo node bin/restore.mjs` leaves
  // behind) and again from a full disk — both are the same shape, and so is EROFS:
  //   · the public signup form answered with a raw `EACCES: permission denied, mkdir /var/…`
  //   · the owner's email address was taken FOREVER — retrying gave `dupe_email`, and no route
  //     in the product can free it
  //   · they could log in, and every screen 503'd telling them to restore a snapshot of a shop
  //     one minute old that had never had one
  //   · brokenTenants got an entry, so /health?strict=1 — the URL ship.sh and release.sh poll —
  //     answered 503 for ever, and every subsequent deploy FOR THE WHOLE FLEET auto-rolled back.
  //
  // So: undo the registration if the database cannot be built. The address is free again, the
  // owner can retry, and nothing is left for /health to trip over.
  try {
    withTenant(slug, () => {
    ...
    // Order matters: drop the handle and the half-written directory first, so a retry with the
    // same shop name gets a clean slug rather than tripping the "a database already exists" guard.
    try { openDbs.get(slug)?.close?.() } catch { /* never opened */ }
    openDbs.delete(slug)
    brokenTenants.delete(slug)
    try { rmSync(join(TENANTS_DIR, slug), { recursive: true, force: true }) } catch { /* nothing landed */ }
    try {
      control.exec('BEGIN IMMEDIATE')
      control.prepare('DELETE FROM sessions WHERE tenant_id = ?').run(tenantId)
      control.prepare('DELETE FROM members WHERE tenant_id = ?').run(tenantId)
      control.prepare('DELETE FROM tenants WHERE id = ?').run(tenantId)
      control.exec('COMMIT')
    } catch (undoErr) {
      try { control.exec('ROLLBACK') } catch { /* already aborted */ }
      // If even the undo fails the operator needs the real reason, not this one.
      console.error('createTenant: could not undo a failed signup for', slug, '-', String(undoErr && undoErr.message))
    }
    console.error('createTenant: could not create the shop database for', slug, '-', String(err && err.message))
    // Never the raw errno string: an ENOSPC/EACCES message on a public signup page prints the
    // server's filesystem layout to anyone who can reach the form.
    const e = new Error('We could not set up your shop just now, and nothing was created. Please try again — if it keeps happening, the server is out of disk space or cannot write to its data directory.')
    e.code = 'provisioning_failed'
    e.cause = err
    throw e
  }
...
export async function addMember(tenantId, { name, email, password, role }) {
  ...
  // …and it has to be ONE LINE of printable characters, for the same reason oneRecipient() in
  // lib/notify.mjs refuses list separators. This value is interpolated verbatim into the body of
  // an invitation email that the PLATFORM sends to an address the caller chose, so a password
  // carrying "\n\nACTION REQUIRED: your card was declined. Update it at http://…" put the
  // attacker's own paragraph into a message signed by the platform. Nothing downstream escapes it,
  // because a password legitimately contains punctuation.
  // Charset only — the length rule is the MIN_PASSWORD check above, so the two cannot drift apart.
  if (!/^[\x21-\x7e]+$/.test(String(password))) {
    const e = new Error('Temporary password must have no spaces or line breaks.'); e.code = 'weak_password'; throw e
  }
  // The operator's address is reserved on the SIGNUP form (server.mjs) because requireAdmin grants
  // the Control Room on isAdminEmail(tenant.owner_email). It was reserved nowhere else — and
  // members.email is globally UNIQUE, so one POST /api/members from any trial shop took that
  // address permanently, as a staff row inside a stranger's shop. `npm run admin -- create-shop`,
  // the only documented way the operator's own shop comes into existence, then answers "already
  // has an account", `reset-password` signs the operator into the SQUATTER's shop as staff, and no
  // CLI command in the product can delete a member or free an address. The Control Room is off the
  // install for good. Checked here rather than at the route because addMember is the door every
  // invite path goes through. Deliberately NOT in createTenant: create-shop must keep working,
  // since that is the door the signup guard exists to preserve.
  if (isAdminEmail(email)) {
    const e = new Error('That address is reserved for this install\u2019s operator. Use another one.'); e.code = 'reserved_email'; throw e
  }
...
export async function setMemberPassword(id, password) {
  ...
  /**
   * …including the sessions that carry no member_id at all.
   *
   * getSession resolves a NULL member_id to the shop's first owner and hands back `role owner`.
   * The purge above keys on member_id, so those rows survived it — and the admin "sign in as this
   * shop" session was exactly that shape. Driven: an operator impersonates a shop, the shop's
   * owner changes their password (the ONE self-service lever the product offers for ending
   * sessions), the owner's own cookie goes 401, and the impersonation cookie still 200s on
   * PUT /api/settings, GET /api/export/all.json and POST /api/developers/key/rotate. Suspending
   * the shop only GATES it — reactivating brings the same row back. The only durable end was the
   * 30-day expiry or sqlite3.
   *
   * The impersonation route now binds a real member, so new sessions are caught by the purge
   * above. This closes the legacy rows: every pre-members and pre-fix session that would resolve
   * to the member whose password just changed.
   */
  const m = getMemberById(id)
  if (m && firstOwnerId(m.tenant_id) === Number(id)) {
    control.prepare('DELETE FROM sessions WHERE member_id IS NULL AND tenant_id = ?').run(Number(m.tenant_id))
  }
}
 ...
 *
 * A CORRECT password against a shop that is suspended, or a member whose access was removed,
 * comes back as { blocked } instead of null. It used to come back as null like any other failure,
 * so the login route took its `if (!r)` branch and answered "Wrong email or password" — and the
 * line immediately after it, `if (r.tenant.status === 'suspended') …`, was unreachable dead code.
 * The owner of a suspended shop typed the right password, was told it was wrong, tried again, and
 * walked into the account backoff. There was no way out of that from any screen: the one message
 * that explains what happened, and who to call, could not be reached.
 *
 * The status checks moved BELOW verifyPassword deliberately. Answering "suspended" to a wrong
 * password would turn this into an account-existence oracle for anyone who can reach /login.
 */
export async function authMember(email, password) {
  ...
  if (m.status !== 'active') return { member: memberPublic(m), tenant, blocked: 'member_disabled' }
  if (tenant.status !== 'active') return { member: memberPublic(m), tenant, blocked: `tenant_${tenant.status}` }
...
export async function authTenant(email, password) {
  ...
  return r && !r.blocked ? r.tenant : null
}
...
/**
 * The member a session with no member_id resolves to. Pre-members sessions carry none, and until
 * now so did every admin "sign in as this shop" session — which is why this is exported: the
 * password-change purge has to be able to find them.
 */
export const firstOwner = (tenantId) =>
  control.prepare("SELECT * FROM members WHERE tenant_id = ? AND role = 'owner' ORDER BY id LIMIT 1").get(Number(tenantId))
export const firstOwnerId = (tenantId) => firstOwner(tenantId)?.id ?? null

export function createSession(tenantId, memberId = null, source = 'unknown') {
...
export function getSession(token) {
  ...
  if (!member) member = firstOwner(tenant.id)
...
export function billingState(tenant) {
  ...
  // A self-hosted install is not on a trial, because there is nothing to buy.
  //
  // With no platform Stripe key — which is exactly what .env.example instructs a self-hoster to
  // leave blank — createCheckout answers 503, /api/billing/free is 410 plan_retired, and
  // PLAN_ORDER holds the single plan the shop already has. `locked` was already guarded on
  // billingLive() for that reason: locking a shop that cannot pay would be a trap. But `status`
  // was not, and `status` is what the banners actually read — app.js and views/billing.js each
  // fire on `status === 'trial_expired' || locked`. So on day 31 every self-hosted install grew a
  // permanent full-width red bar across every screen, "Your free trial has ended — choose a plan
  // to keep working", pointing at a page with nothing to choose. No UI action cleared it; the
  // only exit was an UPDATE on control.db. README and HOSTING.md promise "free forever, no
  // feature flags, no license key", and a countdown to a paywall is the feature gate this product
  // does not have.
  //
  // Answered before the trial arithmetic rather than after it, so no downstream field can carry
  // the countdown either. Where a platform key IS configured — the hosted service — every value
  // below is unchanged.
  if (!billingLive()) {
    return {
      status: 'active', subscribed: true, plan: t.plan_tier || '',
      trial_ends_at: t.trial_ends_at || null, trial_days_left: 0,
      stripe_customer_id: t.stripe_customer_id || '', locked: false,
    }
  }
...
export function setSubscription(tenantId, { plan, status, customerId, subscriptionId }) {
  ...
  if (!cur) return
  control.prepare(
    'UPDATE tenants SET plan_tier = ?, subscription_status = ?, stripe_customer_id = COALESCE(?, stripe_customer_id), stripe_subscription_id = COALESCE(?, stripe_subscription_id), plan = ?, hosting_revision = hosting_revision + 1 WHERE id = ?',
...
export function deleteTenantFully(id) {
  return deleteTenantInternal(id)
}

/** Read-only provider verification, followed by the same guarded synchronous removal. */
export async function deleteTenantWithHostingCheck(id) {
  return hostingDeletion.remove(Number(id))
}

function deleteTenantInternal(id, deletionToken = null, deletionEvidence = null) {
  const t = control.prepare('SELECT slug FROM tenants WHERE id = ?').get(Number(id))
  if (!t) return hostingDeletion.tombstone(Number(id)) || false
  // The import and its checkpoint worker both hold this database. Refuse before removing
  // registry rows or closing the main handle; retry once their shared lease has drained.
  const importingDb = openDbs.get(t.slug)
  if (importingDb && importCheckpointActive(importingDb)) {
    throw Object.assign(new Error('This shop has an import in progress. Try deleting it after the import finishes.'), { status: 409, expose: true, code: 'import_in_progress' })
  }
  control.exec('BEGIN IMMEDIATE')
  try {
    hostingCheckouts.assertTenantDeletionAllowed(Number(id))
    hostingDeletion.authorizeCommit(Number(id), deletionToken, deletionEvidence)
    ...
    control.exec('COMMIT')
  } catch (error) { try { control.exec('ROLLBACK') } catch {} throw error }
  const dbh = openDbs.get(t.slug); try { dbh?.close?.() } catch { /* noop */ }
  openDbs.delete(t.slug)
  // A deleted shop cannot also be a broken one. Left behind, its slug keeps /health?strict=1 at
  // 503 for the rest of the process's life — and that is the URL ship.sh polls, so every later
  // deploy auto-rolls back over a shop that no longer exists.
  brokenTenants.delete(t.slug)
  // This rmSync was `catch { /* already gone */ }`, and "already gone" is one of the things a
  // failure here can mean. The others are a read-only mount, a permissions change, and an open
  // handle — and in every one of those the rows were deleted while the DATA stayed on disk, and
  // the route above answered `{ ok: true }`. The next shop to take that slug then opened it.
  // Report what actually happened and let the Control Room say so.
  const dir = join(TENANTS_DIR, t.slug)
  let dataRemoved = true, error = ''
  try {
    rmSync(dir, { recursive: true, force: true })
    // force:true swallows ENOENT rather than telling us the tree is gone, so ask.
    if (existsSync(dir)) { dataRemoved = false; error = 'the directory is still there after the delete' }
  } catch (e) {
    dataRemoved = false
    error = String((e && e.message) || e)
  }
  if (!dataRemoved) console.error(`deleteTenantFully: removed the shop "${t.slug}" but not its data at ${dir} — ${error}`)
  const result = { ok: true, slug: t.slug, dataRemoved, path: dir, error }
  hostingDeletion.recordResult(Number(id), result)
  return result
}
