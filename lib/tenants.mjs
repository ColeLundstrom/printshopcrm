...
export function openTenantDb(slug) {
  ...
  try { bootstrapDb(dbh) } catch (e) { try { dbh.close() } catch { /* already gone */ } throw e }
...
export async function createTenant({ shop_name, owner_name, owner_email, password }) {
  ...
  const slug = uniqueSlug(shop_name)
  const embed_key = crypto.randomBytes(9).toString('base64url')
  const trialEnds = iso(new Date(Date.now() + TRIAL_DAYS * 864e5))
  const hash = await hashPassword(password)
  // The tenant row and its owner member are two writes in control.db. A crash between them left a
  // tenant with no owner — un-loginable, and the email permanently claimed (retry hit dupe_email).
  // Wrap both in one control-db transaction so the pair commits or neither does.
  let tenantId
...
export function isAdminEmail(email) {
  const admin = (process.env.PSC_ADMIN_EMAIL || '').toLowerCase()
...
export function dueNurture(nowMs, dayOffsets) {
  ...
  return rows.filter((r) => {
    ...
    return Number(nowMs) >= r.enrolled_at + off * 86400000
  })
}
