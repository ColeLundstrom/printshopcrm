 ...
 */
import { all, get, run, now, round2, getSettings, getUpcharges, logActivity, computeTotals,
  syncInvoiceStatus, nextEstimateNumber, sizeTotal, sizeSummary, rollupSizes, lineQty } from './db.mjs'
import { quoteScreenPrint } from '../public/js/shared/pricing.js'
...
const findInvoice = (token) => {
  ...
  const q = `%${t.toLowerCase()}%`
  return get(`SELECT i.* FROM invoices i LEFT JOIN contacts c ON c.id=i.contact_id WHERE lower(COALESCE(c.name,'')) LIKE ? AND i.status!='paid' ORDER BY i.id DESC LIMIT 1`, q)
}
...
async function doQuote(msg) {
  ...
  const s = getSettings()
  const q = quoteScreenPrint({ garmentCost: order.garment_cost || 3.2, markup: Number(s.default_markup) || 2, qty,
    locations: order.locations || [{ name: 'Front', colors: 1 }], screenFee: Number(s.screen_fee) || 25, darkGarment: order.dark_garment })
  const sizes = Object.keys(order.sizes || {}).length ? order.sizes : { S: 0, M: qty, L: 0, XL: 0 }
  const items = [{ description: `${order.garment} — ${(order.locations || []).map((l) => `${l.colors}/0 ${l.name}`).join(' + ') || 'print'}`,
    detail: `${order.dark_garment ? 'Dark garment, underbase incl. ' : ''}${order.rush ? 'RUSH. ' : ''}`.trim(),
    decoration: order.decoration, sizes, unit_price: q.perPiece, taxable: true }]
  if (q.screens > 0) items.push({ description: `Screen setup — ${q.totalColors} screen${q.totalColors === 1 ? '' : 's'}`, detail: 'One-time', qty: q.totalColors, unit_price: Number(s.screen_fee) || 25, taxable: false })
  const t = computeTotals(items, s.tax_rate, getUpcharges())

  if (!contact) {
    // No known customer — hand back a ready-to-fill draft rather than inventing one.
    return reply(`Here's a draft: **${qty} ${order.garment}**, ${order.decoration.toLowerCase()}, at **${money(q.perPiece)}/pc** → **${money(t.total)}** (${q.totalColors} screen${q.totalColors === 1 ? '' : 's'}).\nWho's it for? Say “…for <customer>” and I'll save it, or open the editor to finish it.`,
  ...
  const estId = Number(run('INSERT INTO estimates (contact_id, estimate_number, status, items, subtotal, tax, total, notes, created_at) VALUES (?,?,?,?,?,?,?,?,?)',
    contact.id, num, 'draft', JSON.stringify(items), t.subtotal, t.tax, t.total, '', now()).lastInsertRowid)
...
function doRevenue() {
  ...
  const out = round2(get(`SELECT COALESCE(SUM(amount_due-amount_paid),0) v FROM invoices WHERE status!='paid'`).v)
...
function doMarkPaid(msg) {
  const inv = findInvoice(msg)
  if (!inv) return reply("I couldn't find that invoice. Try the invoice number, e.g. “mark INV-1002 paid”.")
  const bal = round2(inv.amount_due - inv.amount_paid)
  if (bal <= 0) return reply(`${inv.invoice_number} is already paid in full.`)
  run('INSERT INTO payments (invoice_id, amount, method, note, created_at) VALUES (?,?,?,?,?)', inv.id, bal, 'other', 'Marked paid via assistant', now())
  const updated = syncInvoiceStatus(inv.id)
  logActivity('payment', `Payment ${money(bal)} on ${inv.invoice_number} (marked paid via assistant)`, { contact_id: inv.contact_id })
  return reply(`Marked **${inv.invoice_number}** paid — recorded ${money(bal)}. Balance is now ${money(updated.amount_due - updated.amount_paid)}.`,
    { cards: [{ icon: '▣', title: inv.invoice_number, sub: 'paid', href: `/invoices/${inv.id}` }] })
}
