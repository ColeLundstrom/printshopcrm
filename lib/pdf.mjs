...
export function renderDocument(kind, { doc, contact, settings, items, payments = [], upcharges }) {
  ...
  /* Every other text site in this file goes through clip() or wrap(); this block and its SHIP TO
   * twin did not, and they carry the value most likely to be long: the customer's own email.
   * `katherine.vandermeulen@riverside-consolidated-schools.example.org` — 65 characters, an
   * ordinary school-district address — reaches x=603.2 against a right text margin of 558, and at
   * 67 characters it runs off the 612pt sheet and the tail is simply not drawn. No ellipsis, and
   * nothing on the page saying a character was lost. A truncated email on an estimate is the
   * address the customer is told to reply to. */
  p.text(340, y, 'BILL TO', { size: 8, bold: true, color: MUTED })
  p.text(340, y + 15, clip(contact?.name || '—', 11, true, BLOCK_W), { size: 11, bold: true })
  const billLines = [contact?.company, contact?.email, contact?.phone].filter(Boolean)
  billLines.forEach((ln, i) => p.text(340, y + 29 + i * 11.5, clip(ln, 8.5, false, BLOCK_W), { size: 8.5, color: MUTED }))
...
export function packingSlip({ job, contact, settings, items = [] }) {
  ...
  shipLines.forEach((ln, i) => p.text(340, y + 29 + i * 11.5, clip(ln, 8.5, false, BLOCK_W), { size: 8.5, color: MUTED }))
