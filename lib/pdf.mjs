...
export function renderDocument(kind, { doc, contact, settings, items, payments = [], upcharges }) {
  ...
  for (const [i, ln] of [settings.shop_address, settings.shop_phone, settings.shop_email].entries()) {
  ...
  pages.forEach((pg, i) => {
    ...
    pg.text(MARGIN, PAGE_H - 32, `${settings.shop_name} — ${settings.shop_phone}`, { size: 7.5, color: MUTED })
...
export function packingSlip({ job, contact, settings, items = [] }) {
  ...
  p.text(MARGIN, PAGE_H - 32, `${settings.shop_name} — ${settings.shop_phone}`, { size: 7.5, color: MUTED })
 ...
 * @param {{ job:object, settings:object, sizes:object }} args
 */
export function pickTicket({ job, settings, sizes = {} }) {
  const p = new Page()
  ...
  if (!present.length) {
    ...
    for (const s of present) {
      ...
      p.text(c.qty, y + 20, String(sizes[s]), { size: 13 })
    ...
    p.text(c.qty, y + 12, String(sizeTotal(sizes)), { size: 13, bold: true })
    y += 26
  }
