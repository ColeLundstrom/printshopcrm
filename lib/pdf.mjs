...
const PAGE_H = 792
const MARGIN = 54
// The right-hand address block (BILL TO / SHIP TO) starts at x=340 and must end at the same right
// margin as everything else, or a long email runs off the paper.
const ADDR_X = 340
const BLOCK_W = PAGE_W - MARGIN - ADDR_X
...
//
// Two more cases before the '?', in the order they are tried:
//   · The euro sign is not latin-1 (U+20AC) but WinAnsi does carry it, at 0x80. A shop invoicing
//     in euros printed "? 1.234,50" on every document until this mapped it.
//   · A letter carrying a mark the font lacks is folded to its base letter: "Nguyễn" prints
//     "Nguyen". A few letters have no decomposition to fold to — Đ, ł, ı — and are mapped by hand.
//     Only characters OUTSIDE latin-1 are touched; "José" keeps its real é.
const DRAWABLE = /^[\x20-\x7E\xA0-\xFF]$/
const BY_HAND = { 'Đ': 'D', 'đ': 'd', 'Ł': 'L', 'ł': 'l', 'ı': 'i', 'Ħ': 'H', 'ħ': 'h', 'Ŧ': 'T', 'ŧ': 't', 'Œ': 'OE', 'œ': 'oe', 'Ǆ': 'DZ', 'ǆ': 'dz' }
const substitute = (c) => {
  if (c === '€') return '\x80'
  if (BY_HAND[c]) return BY_HAND[c]
  const base = c.normalize('NFD').replace(/\p{M}/gu, '')
  if (base && [...base].every((ch) => DRAWABLE.test(ch) || BY_HAND[ch])) return [...base].map((ch) => BY_HAND[ch] || ch).join('')
  return c.trim() ? '?' : ' '
}
const esc = (s) =>
  fold(s)
    .replace(/[^\x20-\x7E\xA0-\xFF]/g, substitute)
    .replace(/\\/g, '\\\\')
    .replace(/\(/g, '\\(')
    .replace(/\)/g, '\\)')
...
// Money in the shop's own currency and locale, from the settings every renderer here is handed.
// `charset: 'latin1'` because the font cannot draw ₹ or ₩: those currencies print by ISO code.
// Negatives read as -$40.00, not $-40.00 — discount credits appear on the printed document.
const moneyFor = (settings) => moneyFormatter({ currency: settings?.currency, locale: settings?.locale, charset: 'latin1' })
const INK = [0.13, 0.14, 0.16]
const MUTED = [0.42, 0.45, 0.5]
const ACCENT = [0.02, 0.71, 0.51]

// Postal addresses must remain complete. Unlike the clipped contact identifiers, they wrap
// even an unbroken building/reference name. Measure the same substitutions the PDF draws;
// reserve a full em for non-ASCII glyphs whose widths the small metrics table does not contain.
function postalLines(address) {
  if (!String(address ?? '').trim()) return []
  const width = (value) => {
    const drawn = fold(value).replace(/[^\x20-\x7E\xA0-\xFF]/g, substitute)
    let units = 0
    for (const ch of drawn) {
      const code = ch.charCodeAt(0)
      units += code >= 32 && code <= 126 ? W_REG[code - 32] : 1000
    }
    return units * 8.5 / 1000
  }
  const lines = []
  for (const paragraph of String(address).replace(/\r\n?/g, '\n').split('\n')) {
    let line = ''
    for (const word of paragraph.trim().split(/\s+/).filter(Boolean)) {
      if (line && width(`${line} ${word}`) <= BLOCK_W) { line += ` ${word}`; continue }
      if (line) { lines.push(line); line = '' }
      for (const ch of word) {
        if (line && width(line + ch) > BLOCK_W) { lines.push(line); line = '' }
        line += ch
      }
    }
    lines.push(line)
...
function recipientBlock(p, y, label, contact, address) {
  p.text(ADDR_X, y, label, { size: 8, bold: true, color: MUTED })
  p.text(ADDR_X, y + 15, clip(contact?.name || '—', 11, true, BLOCK_W), { size: 11, bold: true })
  let baseline = y + 29
  let last = y + 15
  const draw = (line) => {
    p.text(ADDR_X, baseline, line, { size: 8.5, color: MUTED })
    last = baseline
    baseline += 11.5
  }
  if (contact?.company) draw(clip(contact.company, 8.5, false, BLOCK_W))
  if (contact?.attention) draw(clip(`Attn: ${contact.attention}`,8.5,false,BLOCK_W))
  for (const line of postalLines(address)) draw(line)
  for (const value of [contact?.email, contact?.phone].filter(Boolean)) draw(clip(value, 8.5, false, BLOCK_W))
  return Math.max(y + 62, last + 10)
}
...
export function renderDocument(kind, { doc, contact, settings, items, payments = [], upcharges }) {
  const fmt = moneyFor(settings)
  const money = fmt.money
  const pages = []
  const COLS = { desc: MARGIN, qty: 372, price: 452, total: PAGE_W - MARGIN }
  // 12pt of gutter, and room for the widest quantity a line can carry ("1,000,000" at 9.5pt).
  const DESC_W = COLS.qty - COLS.desc - textWidth('1,000,000', 9.5, false) - 12
  ...
  // A saved blank is intentional; only old callers without a string snapshot use the contact.
  const billingAddress = typeof doc.billing_address === 'string' ? doc.billing_address : contact?.billing_address
  y = recipientBlock(p, y, 'BILL TO', contact, billingAddress)
  ...
  /* The customer's own purchase-order number. invoices.po_number is a real column, captured in
   * the invoice form, stored by PUT /api/invoices/:id, and rendered on three screens and on the
   * public pay page — and on no printed document at all. A B2B accounts-payable department will
   * not pay an invoice that does not carry their PO number: it comes back as an exception and the
   * invoice ages into the 31-60 bucket over a field the shop had already typed. */
  if (kind === 'INVOICE' && doc.po_number) meta.push(['PO', String(doc.po_number)])
  // The strip is 504pt wide from MARGIN+14, so four cells need a tighter step than three.
  const step = meta.length > 3 ? 118 : 160
  meta.forEach(([k, v], i) => {
    const x = MARGIN + 14 + i * step
    p.text(x, y + 12, k.toUpperCase(), { size: 7, bold: true, color: MUTED })
    p.text(x, y + 23, clip(String(v ?? ''), 9, false, step - 10), { size: 9 })
  })
  y += 52
  ...
  for (const it of items) {
    ...
    /* The clip width was a bare literal 300 and was never reconciled with COLS.qty. DESCRIPTION
     * starts at x=54, so 300 lets it reach 350.7; QTY is drawn RIGHT-aligned at 372, so "12000"
     * starts at 345.6 — 5.1pt of overlap. At 1,200 pieces the gutter is 0.2pt, i.e. touching. It
     * gets worse with the piece count, so the collision lands on exactly the orders the customer
     * checks hardest. Reserve the widest quantity a line can actually carry. */
    p.text(COLS.desc, y, clip(it.description || 'Item', 9.5, true, DESC_W), { size: 9.5, bold: true })
    let dy = y + 11
    ...
    // …and a bare "12000" reads like a part number on a document about garments.
    p.text(COLS.qty, y, lineQty(it) ? fmt.number(lineQty(it)) : '', { size: 9.5, align: 'right' })
    ...
    y += rowH
    // Leave clear space below the size run and above the next row's capital letters.
    p.line(MARGIN, y - 10, PAGE_W - MARGIN, y - 10, [0.92, 0.93, 0.94], 0.5)
  }
  ...
  if(kind==='INVOICE') for(const c of doc.credits || []) if(!c.cancelled_at) rows.push([clip(`Credit — ${c.reason}${c.tax_cents ? ` (tax ${money(c.tax_cents/100)})` : ''}`,9,false,180),`-${money((c.subtotal_cents+c.tax_cents)/100)}`])
  rows.push(['TOTAL', money(kind === 'ESTIMATE' ? doc.total : doc.amount_due)])
  if (kind === 'INVOICE') {
    /* The key half of a totals row is drawn at tx-230 with no clip, so a long method ran straight
     * through the amount beside it and off the sheet: POST /api/invoices/:id/payments takes
     * `req.body?.method` with no length cap and no whitelist (the in-app dialog uses a 5-value
     * select; the API path does not). And payments.note — the field the dialog labels "Check
     * #1042, deposit, etc." — never appeared on the document at all, so the cheque number the
     * shop typed was on three screens and no piece of paper. */
    for (const pay of payments) {
      const ref = [pay.method, pay.note].filter(Boolean).join(' — ')
      const amt = pay.amount < 0 ? money(-pay.amount) : `-${money(pay.amount)}`
      rows.push([clip(`${pay.amount < 0 ? 'Returned' : 'Payment'} — ${ref} ${(pay.created_at || '').slice(0, 10)}`, 9, false, 230 - textWidth(amt, 9, false) - 12), amt])
    }
  ...
  /* The notes/terms band, fitted to the space it actually has.
   *
   * `ty += 48` was a FIXED reservation for a block that renders one to three lines, and TERMS then
   * needs 11 + 3 x 9.5 = 39.5 more. With a note present and a real three-line terms block, the
   * third terms clause landed at page-y 752 — BELOW the footer rule at 748 and inside the footer
   * text band, printing through "Ink & Iron Print Co. — (555) 010-2233". The comment on the totals
   * block twenty lines up states the invariant this block was itself breaking.
   *
   * The clause that lands there is the LAST one, which on a real terms block is the spoilage
   * allowance and the rush surcharge — the money clauses, on the document the customer approves.
   *
   * Both halves were also .slice(0, 3) with no ellipsis, so a longer note or a longer terms block
   * lost its tail with nothing on the page saying so. Whatever is dropped now says it was. */
  const terms = kind === 'ESTIMATE' ? (doc.terms_snapshot ?? settings.estimate_terms) : settings.invoice_terms
  const BAND_TOP = PAGE_H - 118
  const BAND_BOTTOM = PAGE_H - 52 // 8pt clear of the footer rule at PAGE_H - 44
  const allNotes = doc.notes ? wrap(doc.notes, 8.5, false, bodyW) : []
  const allTerms = terms ? wrap(terms, 8, false, bodyW) : []
  const noteLines = allNotes.slice(0, 3)
  const termLines = allTerms.slice(0, 3)
  const notesH = (n) => (n ? 12 + n * 10 + 6 : 0)
  const lastTermsAt = () => BAND_TOP + notesH(noteLines.length) + 11 + Math.max(0, termLines.length - 1) * 9.5
  // Shrink to fit: a terms line first, then a note line, then terms again — so the terms block
  // keeps two lines wherever it can and neither half is ever emptied.
  while (lastTermsAt() > BAND_BOTTOM) {
    if (termLines.length > 2) termLines.pop()
    else if (noteLines.length > 1) noteLines.pop()
    else if (termLines.length > 1) termLines.pop()
    else break
  }
  /** Mark a block that lost its tail, so the page never implies it is complete. */
  const marked = (drawn, all, size) => {
    if (!drawn.length || drawn.length >= all.length) return drawn
    const out = drawn.slice()
    out[out.length - 1] = clip(`${out[out.length - 1]} …`, size, false, bodyW)
    return out
  }
  ...
  if (noteLines.length) {
    ...
    marked(noteLines, allNotes, 8.5).forEach((ln, i) => p.text(MARGIN, ty + 12 + i * 10, ln, { size: 8.5 }))
    ty += notesH(noteLines.length)
  }
  p.text(MARGIN, ty, 'TERMS', { size: 7.5, bold: true, color: MUTED })
  marked(termLines, allTerms, 8).forEach((ln, i) => p.text(MARGIN, ty + 11 + i * 9.5, ln, { size: 8, color: MUTED }))
...
/**
 * Packing slip — what's in the box, no money. Printavo lists these on Standard and Premium.
...
export function packingSlip({ job, contact, settings, items = [] }) {
  ...
  const shippingAddress = typeof job?.shipping_address === 'string'
    ? job.shipping_address
    : contact?.shipping_address || contact?.billing_address || contact?.address
  y = recipientBlock(p, y, 'SHIP TO', contact, shippingAddress)

  // Meta strip: job / order ref / date
  const bodyW = PAGE_W - MARGIN * 2
  p.rect(MARGIN, y, bodyW, 30, [0.96, 0.97, 0.98])
  /* Two of these three read columns that have never existed. `jobs` has no po_number, no
   * order_ref and no reference, so PO / REF has printed "—" on every packing slip this product
   * has ever produced — while the customer's PO number, the thing their accounts-payable matches
   * the box against, sits on invoices.po_number where the shop typed it. And there is no
   * shipped_at either; the column is ship_date, so DATE was the day the job was BOOKED. Seventy
   * days stale on an ordinary order, stamped on the document receiving signs. */
  const meta = [
    ['Job', job?.job_number || '—'],
    ['PO / Ref', job?.po_number || '—'],
    ['Date', String(job?.ship_date || job?.created_at || new Date().toISOString()).slice(0, 10)],
  ]
  meta.forEach(([k, v], i) => {
    const x = MARGIN + 14 + i * 160
    p.text(x, y + 12, k.toUpperCase(), { size: 7, bold: true, color: MUTED })
    // clip(), the way the pick ticket's identical strip three hundred lines down already does.
    // This was the one meta cell in the product drawn raw, and PO / Ref is the value most likely
    // to be long: a 40-character school-district purchase order ended at x=431 where the DATE
    // cell starts at 388, so the ship date on the document receiving SIGNS was printed over.
    // Invisible to the right-margin rule, because 431 is well inside the 558 margin.
    // 140 keeps the same 20pt gutter the pick ticket leaves on its 170pt pitch.
    p.text(x, y + 23, clip(v, 9, false, 140), { size: 9 })
  })
  y += 52
...
export function customerStatement({ contact, settings, invoices = [], asOf = new Date().toISOString().slice(0, 10) }) {
  const money = moneyFor(settings).money
  const pages = []
  ...
  const newPage = (continued) => {
    ...
    y = 96
    if (!continued) {
      // The company name and TOTAL DUE share a baseline, and neither was clipped: a 77-character
      // booster-club name reaches x=490 while a five-figure balance starts at 452 — the customer's
      // name and the amount they owe printed through each other, on the one document whose whole
      // job is to name that number.
      const forW = PAGE_W - MARGIN * 2 - textWidth(money(totalDue), 19, true) - 18
      p.text(MARGIN, y, 'FOR', { size: 8, bold: true, color: MUTED })
      p.text(MARGIN, y + 15, clip(contact?.company || contact?.name || 'Customer', 11.5, true, forW), { size: 11.5, bold: true })
      if (contact?.company && contact?.name) p.text(MARGIN, y + 29, clip(contact.name, 9, false, forW), { size: 9, color: MUTED })
      if (contact?.email) p.text(MARGIN, y + 42, clip(contact.email, 9, false, forW), { size: 9, color: MUTED })
  ...
  for (const i of invoices) {
    ...
    const b = bal(i)
    const late = !i.payment_review && b > 0 && daysPast(i) > 0
    ...
    p.text(COLS.status, y, i.payment_review ? 'REVIEW' : String(i.status || '').toUpperCase(), { size: 8, color: b > 0 ? (late ? [0.8, 0.25, 0.2] : MUTED) : ACCENT })
