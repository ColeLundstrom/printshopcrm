...
export function parseSheet(text) {
  ...
  if (Buffer.byteLength(text, 'utf8') > 2_000_000) throw new Error('Price sheets must be under 2 MB.')
  // Pick one delimiter from the header, outside quoted fields. Never switch delimiters
  // per row: a tab inside a quoted CSV heading is data, not a new file format.
  let quoted = false, delimiter = ','
  for (let i = 0; i < text.length; i++) {
    if (text[i] === '"') { if (quoted && text[i + 1] === '"') i++; else quoted = !quoted }
    else if (!quoted && text[i] === '\t') delimiter = '\t'
    else if (!quoted && text[i] === '\n') break
  }
  const records = [], fields = []
  let value = '', inQuote = false, closed = false
  const field = () => {
    fields.push(value.trim()); value = ''; closed = false
    if (fields.length > LIMITS.cols + 1) throw new Error(`Price sheets support at most ${LIMITS.cols} price columns.`)
  }
  const row = () => {
    field()
    if (fields.some(v => v !== '')) records.push(fields.splice(0))
    else fields.length = 0
    if (records.length > LIMITS.rows + 1) throw new Error(`Price sheets support at most ${LIMITS.rows} price rows. Split the sheet before importing.`)
  }
  for (let i = 0; i < text.length; i++) {
    const ch = text[i]
    if (inQuote) {
      if (ch === '"') { if (text[i + 1] === '"') { value += '"'; i++ } else { inQuote = false; closed = true } }
      else value += ch
    } else if (ch === delimiter) field()
    else if (ch === '\n') row()
    else if (closed) { if (!/\s/.test(ch)) throw new Error('Unexpected text after a closing quote. Check the CSV quoting.') }
    else if (ch === '"') {
      if (value.trim()) throw new Error('A quoted field must start with a quote. Check the CSV quoting.')
      value = ''; inQuote = true
    } else value += ch
  }
  if (inQuote) throw new Error('An opening quote has no closing quote. Check the CSV quoting.')
  if (value || fields.length || closed) row()
  if (records.length < 2) throw new Error('Need a header row of column names and at least one price row.')
  const head = records[0]
  if (head.length < 2) throw new Error('The first row needs at least one column name after the corner cell.')
  const label = (v, where, max = LIMITS.header) => {
    const normalized = v.replace(/\s+/g, ' ').trim()
    if (!normalized) throw new Error(`${where} needs a heading; empty headings can misalign prices.`)
    if (normalized.length > max) throw new Error(`${where} exceeds ${max} characters. Shorten the heading before importing.`)
    return normalized
  }
  const cols = head.slice(1).map((v, i) => label(v, `Column ${i + 2}`))
  const unique = (items, kind) => {
    if (new Set(items.map(v => v.toLowerCase())).size !== items.length) throw new Error(`Duplicate ${kind} headings. Give each heading a distinct name.`)
  }
  unique(cols, 'column')
  const rows = [], cells = []
  for (const [index, parts] of records.slice(1).entries()) {
    if (parts.length !== head.length) throw new Error(`Row ${index + 2} has ${parts.length} fields; expected ${head.length}. Keep blank prices as empty fields.`)
    rows.push(label(parts[0], `Row ${index + 2}`))
    cells.push(parts.slice(1).map((v, ci) => {
      if (!v) return null
      const n = v.replace(/^\$\s*/, '')
      if (!/^(?:(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?|\.\d+)$/.test(n)) throw new Error(`Invalid price at row ${index + 2}, column ${ci + 2}. Use a non-negative number or leave it blank.`)
      const price = cellValue(v)
      if (price === null || price > Number.MAX_SAFE_INTEGER / 100) throw new Error(`Price at row ${index + 2}, column ${ci + 2} is too large.`)
      return price
    }))
  }
  unique(rows, 'row')
  ...
  return { cols, rows, cells, filled, cornerLabel: head[0] ? label(head[0], 'Corner', LIMITS.label) : '' }
}
...
// Contract starters are independent copies. Prices are examples to review, not a recommended discount.
for (const key of ['screen-print','dtf','embroidery']) {
  const base=TEMPLATES.find(t=>t.key===key)
  base.decoration=key==='screen-print'?'Screen Print':key==='dtf'?'DTF':'Embroidery'
  TEMPLATES.splice(3,0,{...base,customerSupplied:true,key:`contract-${key}`,name:`Contract — ${base.name}`,
    description:'Customer-supplied garments. Decoration only; review every sample price and your spoilage policy before quoting.',
    rows:[...base.rows],cols:[...base.cols],cells:base.cells.map(r=>[...r])})
}
...
export function hydrate(row) {
  ...
  return {
    id: row.id,
    customerSupplied:!!row.customer_supplied, decoration:row.decoration||'',
    name: row.name,
...
export function sanitize(input = {}, base = null) {
  ...
  // A caller that sends one of these as the wrong type must be told so, not told "saved".
  //
  // `cells` was the dangerous one: `input.cells ?? base?.cells` accepted anything non-null, and
  // resizeGrid() answers a grid of nulls for any value that is not an array of arrays. So a client
  // that stringified the grid — a CSV paste, a spreadsheet cell, a Zapier field mapping — sent
  // {"cells":"18,20,26"} and got a 200 with every price in the sheet erased. `rows` and `cols` were
  // the quieter half of the same bug: silently ignored, so a rename that never happened reported
  // success. Only null and omitted mean "leave this alone"; the docs say exactly that.
  gridArray('cells', input.cells)
  gridArray('rows', input.rows)
  gridArray('cols', input.cols)
  ...
  if(input.customerSupplied!==undefined&&typeof input.customerSupplied!=='boolean')throw new Error('customerSupplied must be true or false')
  const cells = resizeGrid(Array.isArray(input.cells) ? input.cells : base?.cells, rows.length, cols.length)
  return {
    name, rows, cols, cells,
    customerSupplied:input.customerSupplied??base?.customerSupplied??false,
    decoration:clampStr(input.decoration??base?.decoration??'',60),
    ...
    unit: readUnit(input.unit, base?.unit),
  }
}

/** Refuse a present-but-wrong-typed grid field. null/undefined still mean "keep what is there". */
function gridArray(field, v) {
  if (v === undefined || v === null || Array.isArray(v)) return
  throw new Error(`${field} must be an array — send null or leave it out to keep the current ${field}.`)
}

/**
 * The price sheet's unit, which decides whether a cell is multiplied by the quantity or charged
 * once. It used to be `(input.unit ?? base.unit) === 'flat' ? 'flat' : 'piece'`, so ANY value that
 * was not exactly the lowercase string 'flat' became 'piece' with a 200 — including 'Flat' from a
 * select-option label and 'FLAT' from a spreadsheet column. A shop's setup-fee sheet quietly became
 * per-piece and started multiplying a one-off charge by the order quantity.
 */
function readUnit(v, current) {
  if (v === undefined || v === null) return current === 'flat' ? 'flat' : 'piece'
  const u = String(v).trim().toLowerCase()
  if (u !== 'piece' && u !== 'flat') throw new Error(`unit must be "piece" or "flat" (got ${JSON.stringify(v)}).`)
  return u
}
...
export function createMatrix(input) {
  ...
  const ts = now()
  const r = run(
    `INSERT INTO price_matrices (name, description, row_label, col_label, unit, grid_rows, grid_cols, cells, is_default, created_at, updated_at, customer_supplied, decoration)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    ...
    first || input.isDefault ? 1 : 0, ts, ts, m.customerSupplied?1:0,m.decoration)
...
export function updateMatrix(id, input) {
  ...
  run(`UPDATE price_matrices SET name=?, description=?, row_label=?, col_label=?, unit=?, grid_rows=?, grid_cols=?, cells=?, customer_supplied=?,decoration=?,updated_at=? WHERE id=?`,
    m.name, m.description, m.rowLabel, m.colLabel, m.unit,
    JSON.stringify(m.rows), JSON.stringify(m.cols), JSON.stringify(m.cells), m.customerSupplied?1:0,m.decoration,now(), existing.id)
