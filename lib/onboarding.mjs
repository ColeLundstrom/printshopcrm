...
export function parseShopProfile(text) {
  ...
  if (rate && rate >= 20 && rate <= 500) set('shop_hourly_rate', rate, `Shop rate set to $${rate}/hr`)

  // DTF price per inch: "$1/inch", "a dollar an inch", "0.95 per inch"
  const perInch = num(t.match(/\$?\s*([\d.]+)\s*(?:\/|per\s*|an?\s*)?\s*(?:inch|in)\b/i))
  if (perInch && perInch > 0 && perInch < 20) set('dtf_price_per_inch', perInch, `DTF price set to $${perInch}/inch`)
  else if (/\bdollar\s+(?:an?|per)\s+inch\b/i.test(t)) set('dtf_price_per_inch', 1, 'DTF price set to $1/inch')
...
export function onboardingSteps(settings, ctx = {}) {
  ...
  const defs = [
    ...
    { key: 'payments', icon: 'billing', title: 'Collect payments (Stripe)', hint: 'Your own Stripe key — take deposits and online payments into your account.',
