...
export function createHostingCheckouts(db, { createClient = createPlatformBillingClient, buildRequest = buildHostingCheckoutRequest, now = Date.now, limits = {} } = {}) {
  ...
  const status = (tenantId, ignoreLease = null) => {
    ...
    return {
      intent: row ? { id: row.id, state: row.resolved_at ? 'closed' : row.state, plan: row.plan, interval: row.interval, amount_cents: row.amount_cents, currency: row.currency,
        url: row.state === 'open' && !row.resolved_at && row.expires_at > stamp && !row.diagnostic && !hasAnomaly(tenantId) ? row.session_url : null,
  ...
  const start = options => bounded(async () => {
  ...
  const action = (options, kind) => bounded(async () => {
  ...
  return Object.freeze({
    start, reconcile: options => action(options, 'reconcile'), expire: options => action(options, 'expire'), status, processCheckoutEvent, resolveAnomaly,
    ...
    assertTenantDeletionAllowed(tenantId) {
      if (unresolved(tenantId) || hasAnomaly(tenantId) || db.prepare('SELECT 1 FROM hosting_checkout_intents WHERE tenant_id=? AND lease_until>? AND lease_token IS NOT NULL LIMIT 1').get(tenantId, now())) throw safe('hosting_checkout_unresolved', 'Reconcile this shop’s hosting checkout before deleting the shop.')
    },
    health() {
