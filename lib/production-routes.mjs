import { shippingView, mutateShipment } from './shipping.mjs'
import { productionCalendar } from './production-calendar.mjs'
...
export function registerProductionRoutes(
  ...
  const details = (j, req) => {
    const w = p.workflow(j.id), who = actor(req), shipping = shippingView({job_id:j.id},who)
    return {
      job: {
        id: j.id,
      ...
      shipping,
      tasks: w.tasks.map((t) => {
        const blocked = p.taskBlock(j, t, w.tasks)
        // Match transitionTask's assignment rule, including unassigned work and manager access.
        // This is a view hint; the mutation still checks permission, revision and live gates.
        const otherEmployee = !who.manager && t.assigned_id !== null && t.assigned_id !== who.id
        const completion_blocked = otherEmployee
          ? 'Assigned to another employee. That employee or a manager can complete this task.'
          : t.status !== 'pending' ? 'This task has already been resolved.' : blocked
        return { ...t, blocked, can_complete: !completion_blocked, completion_blocked }
      }),
  ...
  app.get('/api/production/calendar', safe((req, res) => res.json(productionCalendar({
    start: req.query.start, end: req.query.end, department: String(req.query.department || ''),
    mine: req.query.mine === '1', memberId: req.member?.id ?? null
  }))))
  app.put('/api/production/jobs/:id/timing', requireRole('manager'), safe((req, res) => {
    const j = job(req.params.id)
    p.saveJobTiming(j.id, req.body, actor(req).name)
  ...
  app.post(
    ...
    safe((req, res) => {
      const j = job(req.params.id),
        b = req.body || {},
        who = actor(req)
      const mutation = mutateShipment({job_id:j.id},b,who,{legacy:b.request_id == null})
      if (mutation.changed && !mutation.replayed) broadcast('production', { job_id:j.id })
      res.json({...details(job(j.id),req),shipment_mutation:mutation})
    })
  )
  app.post(
