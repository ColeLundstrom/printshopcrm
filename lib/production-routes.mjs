...
export function registerProductionRoutes(
  ...
  app.get(
    ...
    safe((req, res) => {
      ...
      const jobs = all(
        "SELECT j.* FROM jobs j JOIN production_jobs p ON p.job_id=j.id WHERE j.status='active' ORDER BY j.rush DESC,j.due_date IS NULL,j.due_date,j.id"
      )
      const rows = []
      for (const j of jobs) {
        const w = p.workflow(j.id)
        for (const t of w.tasks) {
          if (
            ...
            (mine && t.assigned_id !== req.member?.id)
          )
            continue
          rows.push({
            job_id: j.id,
            ...
            rush: j.rush,
            task: t,
            blocked: p.taskBlock(j, t, w.tasks),
            revision: w.revision
          })
        }
      }
