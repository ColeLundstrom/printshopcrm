...
// Re-exported: lib/agent.mjs and the gate both reach these through here.
import { phoneCandidate, countableText } from './text.mjs'
export { phoneCandidate, countableText }
...
export const AI_PROVIDERS = [
  ...
  // The locally installed, OAuth'd claude CLI. Single-tenant only, and it has to be CHOSEN — a
  // blank provider means Off. It is listed here so that choosing it is possible from the UI at all:
  // before, it was only ever reachable by NOT choosing anything, which is how the off switch came
  // to turn it on.
  ...(MULTI_TENANT ? [] : [{ id: 'cli', label: 'Local claude CLI (this machine, OAuth)', keyHint: 'no key — uses your CLI login', models: ['claude-haiku-4-5', 'claude-sonnet-5', 'claude-opus-5'] }]),
]
...
export function aiConfig() {
  ...
  // A BLANK provider is the Settings dropdown's "Off — no AI model", and it used to fall through
  // to this branch. So on a single-tenant install with the claude binary present — the ordinary
  // self-hoster, since PSC_AUTH is unset by default and the server warns about exactly that at
  // boot — the off switch did nothing: aiConfig() returned {provider:'cli'} and every AI feature
  // kept calling a model, on the platform's OAuth login rather than the key the shop is supposed
  // to bring, while the card beside it read "Off — deterministic parser only". Off means off; the
  // CLI is an explicit choice like every other provider, and is offered in AI_PROVIDERS above.
  if (provider === 'cli' && !MULTI_TENANT) {
    // The !MULTI_TENANT guard covers the EXPLICIT choice too — otherwise a shop on a shared
    // install could store ai_provider='cli' and run every AI feature on the platform's own login.
...
export function parseIntakeHeuristic(text) {
  const t = String(text || '')
  // Nothing below may read a phone number or a date as a piece count. See countableText above.
  const tq = countableText(t)
  ...
  {
    /**
     * Take the style number OUT of the sentence before looking for a count.
     *
     * The gap between a count and its unit noun was 22 characters, and this branch did not scrub
     * the style — only the fallback below did. One colour adjective is enough to push the real
     * count past 22, at which point the scan restarts and matches the STYLE NUMBER against the
     * noun sitting right next to it. Driven through POST /api/autopilot on a seeded shop, each of
     * these wrote a real estimate and a real job:
     *
     *   "Looking for 144 white Next Level 6210 tees, one color left chest" → EST at $90,357.21
     *     for 6,210 pieces. The right answer is 144 pieces at $2,119.66. 42.6x.
     *   "300 black Comfort Colors 1717 tees"  → 1,717 pieces, $40,304.96
     *   "200 heather grey Bella Canvas 3001"  → 3,001 pieces, $36,241.07
     *   "96 forest green Richardson 112 hats" →   112 pieces
     *   "250 navy Port & Company PC61"        → refused outright, "doesn't say how many pieces"
     *
     * In Full Auto the deterministic parser and the model AGREE, so needs_review is empty,
     * nothing holds it, the customer is mailed the quote and the job goes on the board.
     *
     * agent.mjs's extract() has carried this scrub all along, and its comment says "lib/ai.mjs
     * already solves this and bin/gate.mjs already asserts it there". It did not: every gate case
     * put the colour AFTER the style ("200 Gildan 5000 tees in Black"), which keeps the gap at 13
     * and never exercises the class.
     *
     * With the style gone the digits that used to be matched cannot be, so the gap widens to 32 —
     * which is what recovers "250 navy Port & Company PC61". It is still bounded by [^.\n] so it
     * cannot cross a sentence or a line, and countableText() has already stripped the phone
     * numbers and dates the 22 was chosen to keep out.
     */
    const styleOf = parseGarmentText(t).style
    const reEsc = (v) => String(v).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    const scrubbed = styleOf ? tq.replace(new RegExp(`\\b${reEsc(styleOf)}\\b`, 'gi'), ' ') : tq
    const m = scrubbed.match(/\b(\d{2,4})\b[^.\n]{0,32}?\b(?:pcs?|pieces?|units?|shirts?|tees?|t-?shirts?|hoodies?|hoodys?|polos?|tanks?|jerseys?|crews?|aprons?|hats?|caps?|beanies?)\b/i)
  ...
  // "No rush at all", "there's no hurry", "not urgent", "we don't need it this week" are the
  // customer DECLINING a rush, and an unanchored substring match read every one of them as asking
  // for one: +50% on the per-piece, "RUSH +50%." printed on the line they sign, and a 3-working-day
  // promise on a job they said could wait until autumn. On 300 tees that is $4,255.00 against
  // $2,845.00. "No rush" is one of the most ordinary courtesies in an inbound enquiry.
  //
  // It also could not be undone downstream: mergeIntake's rush is one-way (`ev.rush ? true : …`)
  // and ev.rush comes from this same regex, so a model that read the sentence correctly and
  // returned rush:false was refused. It has to be right at the source.
  //
  // The negated span is removed before the test rather than the test being anchored, so a message
  // that declines a rush in one sentence and asks for one in another still reads as a rush. The
  // repetition count is greedy on purpose: "no need to rush" must be swallowed whole, not left as
  // a bare "rush" after "no need" is taken out.
  //
  // The remaining half of the same problem: the test ran over the WHOLE pasted message, so any
  // occurrence of the letters anywhere in it billed +50%. Customers are called things. Measured on
  // 300 tees, all three of these signed a line reading "RUSH +50%." for $4,582.83 against
  // $3,063.55 — $1,519.28 taken from a customer who never asked for anything:
  //
  //   "Please quote 300 tees for the Hurricanes booster club."      hurr\w* → Hurricanes
  //   "Riverside Urgent Care staff shirts, 300 needed."             urgent\w* → Urgent Care
  //   "Athletics, Rush County High School"                          rush\w* → a signature block
  //
  // A rush word written as a NAME is Title Case and sits away from the start of its sentence.
  // People actually ASKING for a rush write it in lower case ("we need these rushed"), or shout it
  // ("RUSH ORDER"), or open the sentence with it ("Rush order please") — and all three of those
  // still read as a rush here. That one distinction covers every case above without a vocabulary
  // list of team names, and it keeps 'brushed' and 'Hurley', which the word boundaries already
  // handled, out of it.
  const RUSH_WORD = /\b(rush\w*|asap|urgent\w*|hurr\w*)\b/gi
  const RUSH_DECLINED = /\b(no|not|isn['’]?t|aren['’]?t|without|never|zero|don['’]?t|doesn['’]?t)\s+(?:\w+\s+){0,2}(rush\w*|asap|urgent\w*|hurr\w*|need\w*)\b/gi
  const rushSignal = (text) => {
    // The negated span is removed before the test rather than the test being anchored, so a
    // message that declines a rush in one sentence and asks for one in another still reads as one.
    const cleaned = String(text || '').replace(RUSH_DECLINED, ' ')
    for (const sentence of cleaned.split(/[.!?\n]+/)) {
      if (/need.*(tomorrow|by friday|this week)/i.test(sentence)) return true
      RUSH_WORD.lastIndex = 0
      for (let m; (m = RUSH_WORD.exec(sentence));) {
        const word = m[1]
        const atSentenceStart = !/[A-Za-z0-9]/.test(sentence.slice(0, m.index))
        if (/^[A-Z][a-z]/.test(word) && !atSentenceStart) continue   // a name, not a request
        return true
      }
    }
    return false
  }
  const rush = rushSignal(t)
  ...
  // Which slots the TEXT actually supplied, as opposed to which slots have a default. Every
  // field below falls back to something when the message is silent — garment lands on
  // GARMENTS[last], decoration on 'Screen Print', locations on [{Front,1}] — so "is it set?"
  // cannot tell a value the customer stated from a value we invented. mergeIntake needs that
  // distinction to know what a model is allowed to overwrite; see the note on mergeIntake.
  const evidence = {
    garment: !!gmHit || !!(generic && parsedG.style && parsedG.brand),
    decoration: DECOS.some((d) => d.re.test(t)),
    locations: /colou?r|front|back|sleeve|hood|chest/i.test(t),
    dark_garment: dark || !!colorHit,
    rush,                       // the regex only ever sets rush true; false means "not stated"
...
/**
 * Fold a model's extraction into the deterministic parse — additively, never destructively.
 *
 * The message being parsed is written by the customer, and on the Autopilot path the estimate
 * this produces is priced, saved and emailed without anyone reading it. So the model is treated
 * as a source of detail, not as an authority on the count: it may fill a slot the parser left
 * empty, and it may not overwrite one the parser filled. lib/agent.mjs applyValidated() has
 * followed that rule since round 1; this path never got it, and the gap was worth $17,960 on a
 * 500-piece hoodie order — a reply of {"sizes":{"S":1}} re-based total_pieces from 500 to 1.
 *
 * Exported so the guardrail is unit-testable without a model.
 */
export function mergeIntake(base, j) {
  // Sizes the model proposes are re-keyed against SIZES and must be positive — same as before.
  const sizes = {}
  ...
  const baseGrid = Object.keys(base.sizes || {}).length > 0
  // A grid the parser already read is the customer's own breakdown. The model may only supply
  // one where there was none.
  const useSizes = baseGrid ? base.sizes : (Object.keys(sizes).length ? sizes : base.sizes)

  // Every priced field gets the rule `sizes` already had: the model may FILL a slot the text
  // left empty, and may never REPLACE one the text supplied. base.evidence says which is which.
  // Without it "is it set?" is not the question we need answered — garment, decoration and
  // locations always come back populated from a fallback, so they read as stated when they were
  // guessed, and every one of them was a free overwrite for anyone who could write the email.
  const ev = base.evidence || {}
  const keep = (field, proposed) => (ev[field] ? base[field] : (proposed ?? base[field]))

  const propGarment = acceptGarment(j.garment)
  const propDeco = DECOS.some((d) => d.name === j.decoration) ? j.decoration : null
  const propLocs = Array.isArray(j.locations) && j.locations.length
    ? j.locations.slice(0, 5).map((l) => ({ name: String(l.name || 'Front').slice(0, 20), colors: Math.min(12, Math.max(1, Number(l.colors) || 1)) }))
    : null
  const propDark = typeof j.dark_garment === 'boolean' ? j.dark_garment : null
  const propRush = typeof j.rush === 'boolean' ? j.rush : null
  const propDue = acceptDue(j.due_hint)

  const merged = {
    ...base,
    source: 'model',
    garment: keep('garment', propGarment),
    decoration: keep('decoration', propDeco),
    sizes: useSizes,
    locations: keep('locations', propLocs),
    dark_garment: keep('dark_garment', propDark),
    // rush is one-way. The model may raise a rush the text implied without saying so; it may
    // never clear one the customer stated, because rush is a price multiplier AND a promise.
    rush: ev.rush ? true : (propRush ?? base.rush),
    due_hint: keep('due_hint', propDue),
    notes: String(j.notes || base.notes || '').slice(0, 2000),
  ...
  // The count may only go up. A stated total ("we need 500") outranks any breakdown, which is
  // the same Math.max priceIntake already applies between a grid and a stated total.
  const gridTotal = Object.values(merged.sizes).reduce((s, n) => s + n, 0)
  merged.total_pieces = Math.max(gridTotal, Number(base.total_pieces) || 0)

  // Refusing the overwrite is not the same as noticing it. Record every field where the model
  // read the message differently from the parser, so the disagreement reaches a person instead
  // of being dropped on the floor — and so Full Auto can decline to send a contested quote.
  const same = (a, b) => (typeof a === 'string' && typeof b === 'string'
    ? a.trim().toLowerCase() === b.trim().toLowerCase()
    : JSON.stringify(a) === JSON.stringify(b))
  const proposals = { garment: propGarment, decoration: propDeco, locations: propLocs, dark_garment: propDark, rush: propRush, due_hint: propDue }
  const disagreed = Object.keys(proposals).filter((f) => proposals[f] != null && ev[f] && !same(proposals[f], base[f]))

  // …and a field the PARSER never read, which the model asserted on its own, is not a
  // disagreement — `ev[f]` is false by definition, so the filter above cannot see it. That is
  // exactly the case where the model is the SOLE author of the fact, and for these two that
  // authorship is worth money: rush multiplies the per-piece by the shop's rush tier AND shortens
  // the promise. Measured on an email that named neither ("300 tees, one colour front, autumn is
  // fine"): a model reply of {rush:true, due_hint:"2026-09-01"} moved the quote from $2,845.00 to
  // $4,255.00 (+49.6%) and the job's due date from ten working days out to two — and Full Auto
  // mailed it, because needs_review came back empty. Neither may leave the building unread.
  const invented = [
    propRush === true && !ev.rush ? 'rush' : null,
    propDue && !ev.due_hint ? 'due_hint' : null,
  ].filter(Boolean)

  merged.needs_review = [...disagreed, ...invented]
  merged.ai_note = [
    disagreed.length ? `The assistant read ${disagreed.join(', ')} differently from the message — the quote uses what the message says.` : '',
    invented.length ? `The assistant added ${invented.join(', ')} on its own; the message does not say it.` : '',
    merged.needs_review.length ? 'Check it before sending.' : '',
  ].filter(Boolean).join(' ')
  return merged
}

/**
 * A garment name is only accepted from the model if it reads as apparel we can price. Taken
 * verbatim it was a free-text field on a customer-authored document, and garment_cost is looked
 * up from it.
 */
/**
 * A CANONICAL garment name, never the model's prose.
 *
 * The test was `GARMENTS.some(g => g.re.test(s))` — a substring match on an 80-character
 * free-text field — and on a hit the whole string was returned VERBATIM. So
 * "Gildan 5000 Tee - FREE RUSH UPGRADE, no deposit required, 40% off" was accepted intact, with
 * needs_review empty and ai_note blank, and that value is the estimate LINE DESCRIPTION printed
 * under the customer's Approve button, plus jobs.garment on the work ticket and the purchase
 * order. On Full Auto nothing holds it and the estimate is mailed. And on the receptionist path
 * the model's whole input is the visitor's own 4,000 characters, so the string is not merely
 * model-authored — it is attacker-influenced, on a customer-facing priced document, with the
 * price unchanged.
 *
 * Brand + style first, because it is the more specific answer and it is the same shape
 * parseIntakeHeuristic already builds; the shortlist is the fallback for a bare "hoodie".
 */
const GARMENT_NOUN = /^[\s,-]*(long\s*sleeves?|t-?shirts?|tees?|shirts?|hoodies?|hoodys?|sweatshirts?|crews?|polos?|tanks?|hats?|caps?|beanies?|totes?|bags?|aprons?|jerseys?|jackets?)\b/i

function acceptGarment(v) {
  const s = String(v || '').trim().slice(0, 80)
  if (!s) return null
  const p = parseGarmentText(s)
  if (p.brand && p.style) {
    // Brand, style, and the garment noun IF it sits immediately after the style — so
    // "Bella+Canvas 3001 Tee" survives intact while everything after the noun is dropped.
    const at = s.toLowerCase().indexOf(String(p.style).toLowerCase())
    const noun = at < 0 ? '' : (s.slice(at + String(p.style).length).match(GARMENT_NOUN)?.[1] || '')
    return `${p.brand} ${p.style}${noun ? ` ${noun}` : ''}`.replace(/\b[a-z]/g, (c) => c.toUpperCase())
  }
  const hit = GARMENTS.find((g) => g.re.test(s))
