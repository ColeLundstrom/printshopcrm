...
export function mergeIntake(base, j) {
  ...
  merged.needs_review = Object.keys(proposals)
    .filter((f) => proposals[f] != null && ev[f] && !same(proposals[f], base[f]))
  merged.ai_note = merged.needs_review.length
    ? `The assistant read ${merged.needs_review.join(', ')} differently from the message — the quote uses what the message says. Check it before sending.`
    : ''
  return merged
}
