...
export function initProductionSchema(db) {
  ...
  db.exec(`CREATE TRIGGER IF NOT EXISTS production_auto_new_job AFTER INSERT ON jobs
    ...
    BEGIN
      INSERT INTO production_jobs(job_id) SELECT NEW.id WHERE EXISTS(SELECT 1 FROM production_templates WHERE archived=0 AND match_text<>'' AND instr(lower(NEW.decoration),lower(match_text))>0);
      INSERT INTO production_tasks(job_id,template_name,title,department,stage,gate,position,assigned_id)
        SELECT NEW.id,t.name,json_extract(s.value,'$.title'),json_extract(s.value,'$.department'),json_extract(s.value,'$.stage'),coalesce(json_extract(s.value,'$.gate'),''),s.key,json_extract(s.value,'$.assigned_id')
