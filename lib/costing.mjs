...
import { jobRoi, loadRoiContext } from './roi.mjs'
...
export function jobCosting(id, context = null) {
  const job = context ? context.jobs.get(id) : jobRecord(id),
    legacy = context
      ? jobRoi(job, context.roi.settings, context.roi.garmentRows, context.roi.pre)
      : jobRoi(job),
    r = (context ? context.costs.get(id) : get('SELECT * FROM costing_jobs WHERE job_id=?', id)) || {
      revision: 0,
      customer_supplied: 0,
      material_cost: null,
  ...
  const ops = (
    context
      ? context.operations.get(id) || []
      : all('SELECT * FROM costing_operations WHERE job_id=? AND voided_at IS NULL ORDER BY id', id)
  ).map((o) => {
  ...
  const runs = job.estimate_id
    ? context
      ? context.roi.pre.runs.get(job.estimate_id) || []
  ...
  return {
    ...
    events: context
      ? []
      : all('SELECT actor,detail,created_at FROM costing_events WHERE job_id=? ORDER BY id DESC LIMIT 30', id)
  }
}
export function costComparison(options = {}) {
  pageOptions(options)
  const roi = loadRoiContext()
  const jobs = all('SELECT j.* FROM jobs j JOIN costing_jobs c ON c.job_id=j.id ORDER BY j.id DESC')
  const operations = new Map()
  for (const op of all('SELECT * FROM costing_operations WHERE voided_at IS NULL ORDER BY id')) {
    if (!operations.has(op.job_id)) operations.set(op.job_id, [])
    operations.get(op.job_id).push(op)
  }
  const context = {
    roi,
    jobs: new Map(jobs.map((j) => [j.id, j])),
    operations,
    costs: new Map(all('SELECT * FROM costing_jobs').map((c) => [c.job_id, c])),
    credits: new Map(
      all(
        'SELECT invoice_id,coalesce(sum(subtotal_cents),0)/100.0 amount FROM invoice_credits WHERE cancelled_at IS NULL GROUP BY invoice_id'
      ).map((c) => [c.invoice_id, c.amount])
    )
  }
  const rows = jobs.map((j) => jobCosting(j.id, context))
  ...
  return {
    ...result,
    jobs: pageRows(rows, options).rows.map(({ job, revenue, cost, profit, margin, basis }) => ({
      job,
      revenue,
      cost,
      profit,
    ...
    pagination: (({ rows, ...meta }) => meta)(pageRows(rows, options)),
    allocation:
