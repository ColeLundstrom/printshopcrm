...
export function applyWorkflow(jobId, b, actor) {
  return tx(() => {
    ...
    if (chosen.length > 1) steps.sort((a, b) => JOB_STAGE_KEYS.indexOf(a.stage) - JOB_STAGE_KEYS.indexOf(b.stage))
...
export function taskBlock(job, t, rows) {
  ...
  if (job.invoice_id && get('SELECT payment_review FROM invoices WHERE id=?', job.invoice_id)?.payment_review)
  ...
  if (t.gate === 'receiving') {
    ...
    if (!get('SELECT 1 FROM purchase_orders WHERE job_id=?', job.id)) {
      const expected = JSON.parse(job.sizes || '{}'),
        counts = JSON.parse(
          get('SELECT counts FROM production_counts WHERE job_id=?', job.id)?.counts || '{}'
        )
      if (
