import { timingConfig, offset, validDate, planDates } from './production-timing.mjs'
import { pageRows, pageOptions } from './paging.mjs'
import { getArtProduction, readinessForJobs } from './art-production.mjs'
...
export const templates = () =>
  all('SELECT * FROM production_templates ORDER BY archived,id').map((t) => ({
    ...t,
    steps: JSON.parse(t.steps), timing: timingConfig(JSON.parse(t.timing))
  }))
export function validateSteps(steps, members = []) {
  ...
  return steps.map((s) => {
    ...
    if (!['', 'receiving', 'approval', 'preflight'].includes(s.gate || '')) fail('Invalid task requirement')
    const assigned =
    ...
    return {
      ...
      stage: s.stage,
      gate: s.gate || '',
      assigned_id: assigned, due_offset: offset(s.due_offset)
    }
  })
}
export function saveTemplate(id, b, members) {
  return tx(() => {
    const timing = timingConfig(b.timing ?? (id ? JSON.parse(get('SELECT timing FROM production_templates WHERE id=?', id)?.timing || '{}') : {})), steps = validateSteps(b.steps, members),
      name = text(b.name, 60),
    ...
    if (id) {
      ...
      run(
        'UPDATE production_templates SET name=?,match_text=?,steps=?,timing=?,archived=?,revision=revision+1 WHERE id=?',
        name,
        match,
      ...
      id = Number(
        run(
          'INSERT INTO production_templates(name,match_text,steps,timing) VALUES(?,?,?,?)',
          name,
          match,
          JSON.stringify(steps), JSON.stringify(timing)
        ).lastInsertRowid
      )
...
export function workflow(jobId) {
  const row = get('SELECT * FROM production_jobs WHERE job_id=?', jobId)
  const planned = planDates(JSON.parse(row?.timing || '{}'), all('SELECT * FROM production_tasks WHERE job_id=? ORDER BY position,id', jobId))
  const { tasks, ...timing } = planned
  return { revision: row?.revision || 0, timing, tasks,
    events: all('SELECT * FROM production_events WHERE job_id=? ORDER BY id DESC LIMIT 50', jobId) }
}
export function saveJobTiming(jobId, b, actor) {
  return tx(() => {
    const j = check(jobId, b.revision)
    const timing = timingConfig(b.timing)
    if (timing.enabled && !timing.start_date && !timing.production_date) fail('Set a start date or a production date.')
    const before = workflow(jobId).timing
    run('INSERT OR IGNORE INTO production_jobs(job_id) VALUES(?)', jobId)
    run('UPDATE production_jobs SET timing=? WHERE job_id=?', JSON.stringify(timing), jobId)
    event(jobId, null, actor, 'schedule.changed', JSON.stringify({ before, after: timing, reason: String(b.reason || '').slice(0,500) }))
    ...
    for (const t of chosen)
      for (const s of t.steps) {
        ...
        if (!seen.has(key)) {
          ...
          const shared = steps.find(x => JSON.stringify([x.title,x.department,x.stage,x.gate,x.assigned_id]) === key)
          if (s.due_offset != null) shared.due_offset = shared.due_offset == null ? s.due_offset : Math.min(shared.due_offset,s.due_offset)
        }
      }
    if (chosen.length > 1)
      steps.sort((a, b) => JOB_STAGE_KEYS.indexOf(a.stage) - JOB_STAGE_KEYS.indexOf(b.stage))
    run('INSERT OR IGNORE INTO production_jobs(job_id) VALUES(?)', jobId)
    if (!workflow(jobId).timing.enabled) {
      const timed = chosen.filter(t => t.timing.enabled).sort((a,b) => b.timing.turnaround_days - a.timing.turnaround_days)[0]
      if (timed) run('UPDATE production_jobs SET timing=? WHERE job_id=?', JSON.stringify({ ...timed.timing, start_date: String(j.created_at || now()).slice(0,10) }), jobId)
    }
    steps.forEach((s, i) =>
      run(
        'INSERT INTO production_tasks(job_id,template_name,title,department,stage,gate,position,assigned_id,timing) VALUES(?,?,?,?,?,?,?,?,?)',
        jobId,
        s.template_name,
        s.title,
        s.department,
        ...
        s.gate,
        i,
        s.assigned_id, JSON.stringify({due_offset:s.due_offset})
      )
    )
...
export function editTask(jobId, taskId, b, actor, members) {
  return tx(() => {
    ...
    const dueDate = b.due_date === undefined ? JSON.parse(before?.timing || '{}').due_date || null : b.due_date || null
    if (dueDate && !validDate(dueDate)) fail('Enter a real task date in YYYY-MM-DD format.')
    run('UPDATE production_tasks SET timing=? WHERE id=?', JSON.stringify({due_offset:s.due_offset, due_date:dueDate}), taskId)
    event(
      jobId,
      taskId,
      actor,
      before ? 'task.edited' : 'task.added',
      JSON.stringify({ before, after: { ...s, due_date: dueDate }, position: b.position })
    )
    return workflow(jobId)
  })
}
export function taskBlock(job, t, rows, state = null) {
  ...
  if (
    ...
    job.approval_gated &&
    (!job.art_approved_at || ((state ? state.latest_art_status : get('SELECT status FROM art_versions WHERE job_id=? ORDER BY version DESC,id DESC LIMIT 1',job.id)?.status) || 'approved') !== 'approved')
  )
    return 'Artwork approval required'
  if (t.gate === 'preflight' || ['production','qc','shipping','complete'].includes(t.stage)) {
    const art = state ? state.art_production : getArtProduction(job.id)
    if ((t.gate === 'preflight' || art?.required) && !art?.technical_ready) return 'Technical production release required'
  }
  if (t.gate === 'receiving') {
    const open = state
...
export function transitionTask(jobId, taskId, b, { name = 'Staff', id = null, manager = false } = {}) {
  return tx(() => {
    ...
    // Skipping a task or finishing prepress must not bypass the job's separate release hold.
    guardTechnicalRelease(jobId,stage)
    run(
...
export function guardTechnicalRelease(jobId, stage) {
  if (!['production','qc','shipping','complete'].includes(stage)) return
  const art = getArtProduction(jobId)
  if (art.required && !art.technical_ready) fail('Technical production release required before advancing this job.',409)
}
...
export function productionQueue({ department = '', mine = false, memberId = null, ...options } = {}) {
  pageOptions(options)
  const jobs = all(
    "SELECT j.*,p.revision workflow_revision,p.timing workflow_timing,(SELECT a.status FROM art_versions a WHERE a.job_id=j.id ORDER BY a.version DESC,a.id DESC LIMIT 1) latest_art_status,(SELECT a.purpose FROM art_versions a WHERE a.job_id=j.id ORDER BY a.version DESC,a.id DESC LIMIT 1) latest_art_purpose FROM jobs j JOIN production_jobs p ON p.job_id=j.id WHERE j.status='active' ORDER BY j.rush DESC,j.due_date IS NULL,j.due_date,j.id"
  )
  const tasks = new Map()
  ...
  const rows = []
  const artStates = readinessForJobs(jobs.filter(j => j.art_release_required || j.latest_art_purpose === 'appearance_mockup' || (tasks.get(j.id) || []).some(t => t.gate === 'preflight')))
  for (const j of jobs) {
    const pending = planDates(JSON.parse(j.workflow_timing), tasks.get(j.id) || []).tasks
    const state = {
      latest_art_status: j.latest_art_status,
      art_production: artStates.get(j.id),
