...
export function productionQueue({ department = '', mine = false, memberId = null, ...options } = {}) {
  pageOptions(options)
  const jobs = all(
    "SELECT j.*,p.revision workflow_revision,p.timing workflow_timing,(SELECT a.status FROM art_versions a WHERE a.job_id=j.id ORDER BY a.version DESC,a.id DESC LIMIT 1) latest_art_status FROM jobs j JOIN production_jobs p ON p.job_id=j.id WHERE j.status='active' ORDER BY j.rush DESC,j.due_date IS NULL,j.due_date,j.id"
  )
  const tasks = new Map()
