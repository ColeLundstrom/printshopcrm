...
import { postalAddress } from './addresses.mjs'
...
const COLS = {
  ...
  billing_address: ['billing_address', 'billing address', 'billing address 1', 'billing address1', 'billing street', 'address', 'address 1', 'street address', 'street'],
  billing_address_line2: ['billing address 2', 'billing address2', 'address 2', 'address2', 'suite'],
  billing_city: ['billing city', 'city'],
  billing_region: ['billing state', 'billing province', 'state', 'province', 'region'],
  billing_postal_code: ['billing zip', 'billing zip code', 'billing postal code', 'zip', 'zip code', 'postal code', 'postcode'],
  billing_country: ['billing country', 'country'],
  shipping_address: ['shipping_address', 'shipping address', 'shipping address 1', 'shipping address1', 'shipping street', 'ship to address'],
  shipping_address_line2: ['shipping address 2', 'shipping address2', 'ship to address 2'],
  shipping_city: ['shipping city', 'ship to city'],
  shipping_region: ['shipping state', 'shipping province', 'ship to state'],
  shipping_postal_code: ['shipping zip', 'shipping zip code', 'shipping postal code', 'ship to zip'],
  shipping_country: ['shipping country', 'ship to country'],
}
...
function contactPostal(row, kind) {
  const raw = key => {
    for (const alias of COLS[key]) if (row[alias] != null && String(row[alias]).trim()) return String(row[alias])
    return ''
  }
  const city = raw(kind + '_city'), region = raw(kind + '_region'), postal = raw(kind + '_postal_code')
  const locality = [city, [region, postal].filter(Boolean).join(' ')].filter(Boolean).join(', ')
  return postalAddress([raw(kind + '_address'), raw(kind + '_address_line2'), locality, raw(kind + '_country')].filter(Boolean).join('\n'), kind === 'billing' ? 'Billing address' : 'Shipping address')
}
...
export function mapContactRow(row) {
  ...
  if (!name) return null
  const billing = contactPostal(row, 'billing'), shipping = contactPostal(row, 'shipping')
  return {
...
export function detectColumns(headers) {
  ...
  return {
    ...
    ...(Object.entries(COLS).some(([key, aliases]) => key.startsWith('billing_') && has(aliases)) ? { billing_address: true } : {}),
    ...(Object.entries(COLS).some(([key, aliases]) => key.startsWith('shipping_') && has(aliases)) ? { shipping_address: true } : {}),
  }
}
...
export function interruptedImportReport({ written = 0, total = 0, noun = 'row', resume = '', err } = {}) {
  ...
  const checkpointBacklog = err?.code === 'import_checkpoint_backlog'
  const checkpointRestore = err?.code === 'import_checkpoint_restore_failed'
  const cause = full
    ...
    : readOnly
      ...
      : checkpointBacklog
        ? ' because an active database reader is keeping too much import history pending. Let any running export or backup finish, then retry'
        : checkpointRestore
          ? ' because database maintenance could not restore its normal settings. Ask the server operator to check the database and restart the service before retrying'
          : ` — ${msg.slice(0, 160)}`
  ...
  const landed = written > 0
    ...
    : 'Nothing was written.'
  return {
    code: full ? 'import_stopped_disk_full' : readOnly ? 'import_stopped_readonly' : checkpointBacklog ? 'import_checkpoint_backlog' : checkpointRestore ? 'import_checkpoint_restore_failed' : 'import_interrupted',
    imported: written,
    of: total,
