...
// Header synonyms for order columns. First match wins.
const ORDER_COLS = {
...
export function coerceMoney(v) {
  ...
  if (lastComma > -1 && lastDot > -1) {
    ...
    // Only commas: decimal if it looks like a cents group (exactly 2 trailing digits, single comma).
    if (/^\d{1,3},\d{2}$/.test(s)) s = s.replace(',', '.')
    else s = s.replace(/,/g, '')
  } else if (lastDot > -1) {
...
export function mapOrderRows(rows, opts = {}) {
  ...
  list.forEach((row, i) => {
    try {
      ...
      const vals = {}       // field → first non-empty cleaned value
      const sizes = {}
      let sizeQty = 0
      for (const [h, cell] of Object.entries(row)) {
        ...
        if (field && val !== '' && vals[field] == null) vals[field] = val
      }
