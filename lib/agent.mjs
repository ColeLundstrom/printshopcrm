...
import { all, get, run, now, round2, getSettings, shopFormat, getUpcharges, freezeUpcharges, computeTotals, nextEstimateNumber, logActivity, sizeTotal, taxRateFor, emitContactCreated } from './db.mjs'
import { priceIntakeLive } from './quickquote.mjs'
import { parseIntakeHeuristic, parseSizeRun, generate, aiStatus, phoneCandidate, countableText } from './ai.mjs'
import { parseGarmentText } from './suppliers.mjs'
import { normStage } from './pipeline.mjs'

// In the shop's own currency — the receptionist quotes the shop's customers, who are wherever the
// shop is. Read per call: the bot runs inside the tenant context of whichever shop it belongs to.
...
export function initAgent(db) {
  ...
  `)
  // chat_sessions.state carries the session's opportunity and estimate rowids as JSON, and both
  // are reused rowids. captureLead() blind-UPDATEs by _oppId on every later turn, so:
  //   1. a visitor chats on the public widget; captureLead opens opportunity 57 and stores it;
  //   2. the shop's manager tidies the pipeline and deletes 57 — exactly the junk-bot-lead row
  //      this path produces;
  //   3. the shop writes its next real quote, and syncFromEstimate lands it on rowid 57;
  //   4. the original visitor's widget session is still live, they type one more message, and
  //      the $18,400 school-district deal is retitled, revalued to $1,240 and has a stranger's
  //      chat transcript, email and phone written into its notes. Nothing logs it, no screen shows
  //      what the value used to be, and Open Pipeline and the Weighted Forecast both move.
  // Same hazard and same fix as automation_pending.ctx in lib/automations.mjs and
  // art_versions.estimate_id in lib/db.mjs: a trigger is the foreign key a JSON column cannot
  // declare. json_valid() guards it, so a state that is somehow not JSON never makes a DELETE
  // throw — deleting a deal must not become impossible because of a malformed chat row.
  // Lives here rather than in applyMigrations because initAgent creates chat_sessions, and
  // initDb runs before it.
  db.exec(`
    UPDATE chat_sessions SET state = json_remove(state, '$._oppId')
      WHERE json_valid(state) AND json_extract(state, '$._oppId') IS NOT NULL
        AND json_extract(state, '$._oppId') NOT IN (SELECT id FROM opportunities);
    UPDATE chat_sessions SET state = json_remove(state, '$._estId')
      WHERE json_valid(state) AND json_extract(state, '$._estId') IS NOT NULL
        AND json_extract(state, '$._estId') NOT IN (SELECT id FROM estimates);
    CREATE TRIGGER IF NOT EXISTS trg_chat_state_opp_delete AFTER DELETE ON opportunities
      BEGIN UPDATE chat_sessions SET state = json_remove(state, '$._oppId')
              WHERE json_valid(state) AND json_extract(state, '$._oppId') = OLD.id; END;
    CREATE TRIGGER IF NOT EXISTS trg_chat_state_est_delete AFTER DELETE ON estimates
      BEGIN UPDATE chat_sessions SET state = json_remove(state, '$._estId')
              WHERE json_valid(state) AND json_extract(state, '$._estId') = OLD.id; END;
  `)
...
export const listSessions = (limit = 60) => all(`SELECT s.*, c.name AS contact_name,
  ...
  FROM chat_sessions s LEFT JOIN contacts c ON c.id=s.contact_id
  -- 'handoff' means the visitor asked to speak to a person. The Receptionist list is the ONLY
  -- place a session can be opened and the only caller of the takeover reply route, so a handoff
  -- that falls off this window is a customer nobody in the shop can answer — their transcript,
  -- name, email and phone are all in the table and no screen can produce the public_id.
  -- Waiting first, then most recent. Same rule as /api/qbo/queue and the three lists beside it.
  ORDER BY (s.status != 'handoff'), s.updated_at DESC LIMIT ?`, limit)
...
export function extract(text, state) {
  ...
  // Only take a phone number if it isn't actually the email's digits — and only if it has the
  // SHAPE of one. "sizes are 24 12 36 48 60 72" and "by 2026-09-08 2026-10-12" were both stored
  // as the visitor's phone number, which then made nextQuoteGoal skip the "how do we reach you"
  // ask entirely: the shop got a captured lead, a quoted opportunity and a draft estimate for
  // somebody it has no way to contact, and the transcript said the bot had their details.
  const phoneRaw = phoneCandidate(t.replace(EMAIL_RE, ''))
  if (phoneRaw && !st.phone) st.phone = phoneRaw
  ...
  if (!st.qty) {
    ...
    //
    // And the gap below is exactly the width of the rest of a phone number, so
    // "call us at 714-555-1234 about 48 tees" quoted 714 pieces. Take the contact details out of
    // the sentence before counting anything in it — the same shape as the style-number scrub. A
    // date is the same trap: "by 2026-09-08 2026-10-12 for 200 tees" quoted EIGHT pieces.
    ...
    let n = null
    const explicit = tq.match(new RegExp(String.raw`\b(?:qty|quantity)\s*:?\s*${NUM}\b`, 'i'))
    if (explicit) n = Number(explicit[1].replace(/,/g, ''))
    else {
      const { style } = parseGarmentText(t)
      const scrub = /^\d+$/.test(String(style)) ? tq.replace(new RegExp(String.raw`\b${style}\b`, 'g'), ' ') : tq
  ...
  // The WHOLE location list, not only the first colour count. state.colors alone meant computeQuote
  // hardcoded `locations: [{ name: 'Front', colors: state.colors }]`, so "2 color front and 1
  // color back" lost the back print entirely — out of the price, out of the screen count, and out
  // of the line description, so the estimate, the work ticket and the press all lost it too.
  // st.colors stays because the "how many colours?" follow-up ask reads it.
  if (parsed.locations?.length && !st.locations) { st.locations = parsed.locations; st.colors = st.colors || parsed.locations[0].colors }
  // Rush, stitches and print area all reached the parser and were then dropped on the floor: a
  // three-day rush quoted LOWER than the standard run, and embroidery was priced on ink colours
  // with no digitizing line. priceIntakeLive puts each service on its own axis.
  if (parsed.rush && !st.rush) st.rush = true
  if (parsed.stitches && !st.stitches) st.stitches = parsed.stitches
  if (parsed.print_area && !st.print_area) st.print_area = parsed.print_area
...
export async function respond(session, text, cfg = getBotConfig()) {
  ...
  if (caps.handoff && HUMAN_RE.test(t)) {
    ...
    // numeric range, garment regex, DECOS allowlist, EMAIL_RE plus phoneCandidate's shape test) before it can enter state.
    ...
    if (!goal) {
      if (wantsQuote || state._flow === 'quote') {
        ...
        // …and the bot's OWN chip. After a ballpark it asks "Anything you want to change?" and
        // offers "Change the quantity" — which matches none of the words above, so the reset never
        // fired, nextQuoteGoal found every slot filled and answered "You are all set". The visitor
        // then said "make it 200 tees" to the same wall, because extract() only writes st.qty when
        // it is empty, and the shop's draft stayed at 48 pieces for an order of 200.
        //
        // Only the count is cleared: this is a correction, not a restart, and re-asking for the
        // garment and decoration they already gave is how a visitor leaves.
        } else if (/\bchange\b[^.]{0,24}\b(quantity|qty|count|number|how many|pieces?|pcs)\b/i.test(t)) {
          state.qty = null; state.sizes = null; state.quoted = null; state.captured = false
        }
        state._flow = 'quote'
  ...
  if (goal === 'answer') {
    ...
    const q = await computeQuote(state)
    // priceIntakeLive refuses rather than guessing when it has no quantity to price. nextQuoteGoal
    // already rules that out before quote_ready, so this is a belt: ask the one question that
    // unblocks it instead of rendering a sentence full of NaN.
    if (!q) {
      state._expecting = 'qty'
      substance = QUOTE_ASKS.qty
      quick = ['24', '72', '144']
    } else {
      state.quoted = q
      if (state.email || state.phone) {
        // We already have a way to reach them — capture now and confirm (this path is the first
        // quote when contact was given up front). captured guards against re-capturing later.
    ...
    const q = state.quoted || await computeQuote(state)
    const cap = captureLead(session, state, cfg, { reason: 'quote', quote: q })
    if (cap) events.push(cap)
    state.captured = true
    state._expecting = null
...
function quoteItems(state, q) {
  ...
  if (state.decoration === 'Screen Print' && Number(q.screens) > 0) {
    items.push({
      ...
      // taxable: false, like every other writer of a setup line in the product — priceIntake
      // (the engine behind Autopilot, Slack and the assistant), the manual Price Calculator, the
      // estimate editor's "+ Fee / setup line" and the seed shop all mark it exempt. A one-time
      // service charge is not taxable goods, and this was the only path billing tax on it — on a
      // document the receptionist writes to the books itself.
      qty: colors, unit_price: round2(Number(q.screens) / colors), taxable: false,
    })
  }
  return items
}
...
/**
 * One order is one price, and this is the panel a STRANGER sees.
 *
 * This function used to be a second pricing engine: quoteScreenPrint plus a service multiplier,
 * against ai.mjs's ten-row blank shortlist. Round 24 removed exactly this from lib/assistant.mjs
 * and left the copy here, on the surface that runs on the shop's own website. Driven, one
 * sentence, one shop — "300 Comfort Colors 1717 tees, screen print, 2 color front":
 *
 *   Read-an-email / Autopilot / Assistant / Slack   $16.00/pc   $4,850.00
 *   the AI receptionist                              $7.99/pc   $2,447.00
 *
 * and it said the low number out loud to the visitor and then wrote EST-1012 for $2,632.77
 * against a canonical $5,222.00. Three divergences compounded, each measured on its own:
 * quoteScreenPrint instead of the shop's price book ($423 on an identical blank), the ten-row
 * shortlist's $3.20 blank instead of the catalog's $6.50, and locations/rush/stitches thrown away
 * upstream. A three-day rush came out CHEAPER than the standard run. Every one of six test orders
 * disagreed, and the receptionist was under on every one.
 *
 * priceIntakeLive is what every other entry point already uses. It costs nothing extra with no
 * distributor connected, and it swallows a supplier outage rather than failing the quote.
 */
async function computeQuote(state) {
  const s = getSettings()
  const qty = Math.max(1, Number(state.qty) || 24)
  const p = await priceIntakeLive({
    garment: state.product || 'Gildan 5000 Tee',
    garment_cost: state.garment_cost || null,
    garment_color: state.color || '',
    decoration: state.decoration || 'Screen Print',
    total_pieces: qty,
    sizes: state.sizes && Object.keys(state.sizes).length ? state.sizes : {},
    locations: state.locations?.length ? state.locations : [{ name: 'Front', colors: state.colors || 1 }],
    dark_garment: !!state.dark,
    rush: !!state.rush,
    stitches: state.stitches || null,
    print_area: state.print_area || null,
  }, s, { timeoutMs: 6000 })
  // Null only when there is no quantity at all, which nextQuoteGoal already rules out before
  // quote_ready — but a caller must never be handed a quote object with NaN in it.
  if (!p) return null
  const subtotal = p.totals.subtotal
  return {
    perPiece: p.quote.perPiece, qty: p.pieces, subtotal,
    screens: p.quote.screens, totalColors: p.quote.totalColors, items: p.items,
    low: round2(subtotal * 0.9), high: round2(subtotal * 1.12),
  }
}
...
function captureLead(session, state, cfg, { reason, quote } = {}) {
  /**
   * A preview is a rehearsal. It was a live run.
   *
   * The "Live preview" box on the AI Receptionist settings screen mints a session stamped
   * 'preview', and this function treated that stamp as fully trusted — which is right for the
   * question of whether the chatter may be LINKED to an existing customer (a manager is typing),
   * and wrong for everything else. Two turns of an owner trying out their own bot produced: a real
   * contact tagged `bot-lead`, a real deal in the Pipeline, EST-1001 for $8,807.50 burning the
   * shop's estimate sequence — and a real customer email queued to whatever address was typed into
   * the box, opening "Thanks for reaching out to Northgate Ink". Reset only clears the chat
   * bubbles; so does Save. And because the preview deliberately bypasses the receptionist's on/off
   * switch (so a disabled bot can still be tested), all of it happened with the bot switched OFF.
   *
   * The engine above this line is untouched, so the preview still exercises the real parser, the
   * real pricing and the real reply — which is the entire point of previewing it. What it no
   * longer does is leave anything behind.
   */
  // null, not an event object: `events` are the side-effects the caller broadcasts and notifies
  // on, and a rehearsal has none to broadcast.
  if (session.channel === 'preview') return null
  ...
  if (oppId) {
    // Scoped, not a bare `WHERE id = ?`. The trigger in initAgent clears this pointer when the
    // deal is deleted, but it cannot help a session whose pointer was already reused by an older
    // build — and this UPDATE is driven by an anonymous website visitor, so it gets the belt as
    // well as the braces. If it matches nothing that is ours, open a fresh deal rather than
    // writing a stranger's chat over whatever inherited the rowid.
    const hit = run('UPDATE opportunities SET title=?, value=?, notes=?, updated_at=? WHERE id=? AND contact_id=? AND source=?',
      summary, val, chatDigest(session, state), now(), oppId, contactId, 'ai-receptionist')
    if (!hit.changes) { oppId = null; state._oppId = null }
  }
  if (!oppId) {
    ...
    // 'quoted', not 'qualified'. 'qualified' is in no STAGE_KEYS and never has been, so every deal
    // the receptionist opened on its happy path was drawn in NO column on the Pipeline board — it
    // could not be opened, edited, dragged or corrected from any screen — while the board's own
    // header counted its value and the Dashboard's KPI did not. Measured: one $8,400 receptionist
    // deal beside one $670 quote read as $9,070 open on the board and $670 on the Dashboard, with
    // the $8,400 card visible nowhere. 'quoted' is what syncFromEstimate uses for exactly this —
    // a priced enquiry that has not been sent yet — and it is a real column the shop can drag.
    const stage = reason === 'handoff' || (matched && unverified) ? 'lead' : 'quoted'
    ...
    oppId = Number(run('INSERT INTO opportunities (contact_id, title, stage, value, source, notes, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)',
      contactId, summary, normStage(stage), val, 'ai-receptionist', digest, now(), now()).lastInsertRowid)
    state._oppId = oppId
  }
...
export function applyValidated(state, j, text = '') {
  ...
  if (!st.phone && j.phone) {
    const raw = phoneCandidate(String(j.phone).replace(EMAIL_RE, ''))
