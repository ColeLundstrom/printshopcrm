...
import { all, get, run, now, round2, getSettings, getUpcharges, computeTotals, nextEstimateNumber, logActivity, sizeTotal, taxRateFor, emitContactCreated } from './db.mjs'
...
export function extract(text, state) {
  ...
  if (!st.qty) {
    ...
    const m = t.match(/\b(?:qty|quantity)\s*:?\s*(\d{1,6})\b/i)
      || t.match(/\b(\d{1,6})\s*(?:x\b|pcs?\b|pieces?\b|units?\b|shirts?\b|tees?\b|t-?shirts?\b|hoodies?\b|hats?\b|caps?\b|totes?\b|bags?\b|polos?\b|tanks?\b|jerseys?\b|of them\b|of these\b)/i)
