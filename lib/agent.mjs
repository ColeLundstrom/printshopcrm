...
import { all, get, run, now, round2, getSettings, getUpcharges, computeTotals, nextEstimateNumber, logActivity, sizeTotal } from './db.mjs'
...
function addMsg(sid, role, body, meta = {}) {
  run('INSERT INTO chat_messages (session_id, role, body, meta, created_at) VALUES (?,?,?,?,?)', sid, role, String(body || ''), JSON.stringify(meta), now())
...
function extract(text, state) {
  ...
  if (!st.qty) {
    const m = t.match(/\b(\d{2,4})\b\s*(?:pcs?|pieces?|units?|shirts?|tees?|t-?shirts?|hoodies?|hats?|caps?|totes?|bags?|polos?|tanks?|jerseys?|of them|of these)?/i)
    if (m && Number(m[1]) >= 6 && Number(m[1]) <= 100000) st.qty = Number(m[1])
  }
...
function captureLead(session, state, cfg, { reason, quote } = {}) {
  ...
  // Enrich the contact with any details we learned after creating it.
  if (state.email) run(`UPDATE contacts SET email=COALESCE(NULLIF(email,''),?) WHERE id=?`, state.email, contactId)
  if (state.phone) run(`UPDATE contacts SET phone=COALESCE(NULLIF(phone,''),?) WHERE id=?`, state.phone, contactId)
  if (state.name) run(`UPDATE contacts SET name=CASE WHEN name IN ('Website Visitor','') OR name LIKE '%@%' THEN ? ELSE name END WHERE id=?`, state.name, contactId)
  ...
  if (!oppId) {
    oppId = Number(run('INSERT INTO opportunities (contact_id, title, stage, value, source, notes, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)',
      contactId, summary, reason === 'handoff' ? 'lead' : 'qualified', val, 'ai-receptionist', chatDigest(session, state), now(), now()).lastInsertRowid)
    state._oppId = oppId
  } else {
  ...
  let estText = ''
  if (q && state.product && state.qty && !state._estId && cfg.capabilities.quote) {
    const s = getSettings()
    const items = q.items || quoteItems(state, q) // sessions quoted before the setup line existed
    const totals = computeTotals(items, s.tax_rate, getUpcharges())
    const num = nextEstimateNumber()
    const estId = Number(run('INSERT INTO estimates (contact_id, estimate_number, status, items, subtotal, tax, total, notes, created_at) VALUES (?,?,?,?,?,?,?,?,?)',
      contactId, num, 'draft', JSON.stringify(items), totals.subtotal, totals.tax, totals.total, 'Auto-drafted from a website chat.', now()).lastInsertRowid)
    state._estId = estId
...
async function maybePhrase({ session, cfg, state, goal, substance, faqAnswer, assist, status }) {
  ...
  const transcript = sessionMessages(session.id).slice(-8).map((m) => `${m.role === 'visitor' ? 'Customer' : 'You'}: ${m.body}`).join('\n')
...
/** Last N lines of the chat, one per line, for grounding a model prompt. */
function recentTranscript(session, n = 8) {
  return sessionMessages(session.id).slice(-n)
    .map((m) => `${m.role === 'visitor' ? 'Customer' : m.role === 'bot' ? 'You' : 'Agent'}: ${m.body}`).join('\n')
}
...
Rules: Reply as ${cfg.name} in first person. Under 70 words. Warm and human, not salesy. No em dashes. No markdown. Reply with ONLY the message text.`
  const out = await generate(prompt, { timeoutMs: 20000, max: 700 })
  return out.text && out.text.trim().length > 1 ? out.text.trim() : ''
}
...
"""`
  ...
  return applyValidated(state, j)
}

/**
 * Fold model-proposed fields into state, but ONLY after each clears the same deterministic
 * validator the parser uses, and ONLY into a slot the deterministic extract left empty. An
 ...
 */
export function applyValidated(state, j) {
  const st = { ...state }
  // Quantity: must be a positive integer in the same range extract() accepts.
  if (!st.qty && j.qty != null) {
    const n = Number(j.qty)
    if (Number.isFinite(n) && n >= 6 && n <= 100000) st.qty = Math.round(n)
  }
  // Sizes: only trusted through parseSizeRun — a proposed grid string is re-parsed, never taken raw.
  ...
  // Product: must read as a real garment (same word set extract() gates on); cost comes from the
  // deterministic heuristic, never the model.
  if (!st.product && j.product) {
    const p = String(j.product)
    if (/\b(tee|t-?shirt|shirt|hoodie|hoody|polo|tank|hat|cap|beanie|crew|apron|jersey|sweat|tote|bag)/i.test(p)) {
      const parsed = parseIntakeHeuristic(p)
      st.product = parsed.garment
      st.garment_cost = parsed.garment_cost
      if (parsed.dark_garment) st.dark = true
    }
  }
  // Decoration: strict allowlist against the shop's known decoration names.
  if (!st.decoration && j.decoration) {
    const d = DECOS.find((x) => x.name.toLowerCase() === String(j.decoration).toLowerCase())
    if (d) st.decoration = d.name
  }
  // Email / phone: must survive the same regex validators as the deterministic extractor.
  if (!st.email && j.email) {
  ...
  if (!st.phone && j.phone) {
    ...
    if (raw && raw.replace(/\D/g, '').length >= 10) st.phone = raw.trim()
  }
  return st
}
