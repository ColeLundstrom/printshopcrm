...
export async function writeContactImport(db, contacts, { batchSize = 500, checkpoint = async () => {} } = {}) {
  ...
  try {
    ...
    const insert = db.prepare('INSERT INTO contacts (name, email, phone, company, notes, tags, billing_address, shipping_address, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)')
      ...
      try {
        ...
        for (const c of contacts.slice(offset, offset + batchSize)) {
          ...
          try {
            ...
            const result = insert.run(c.name, c.email, c.phone, c.company, c.notes, c.tags, c.billing_address || '', c.shipping_address || '', stamp, stamp)
