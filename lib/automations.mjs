 ...
 */
import { all, get, run, now, round2, getSettings, logActivity, scheduleFor } from './db.mjs'
...
export const TRIGGERS = [
  ...
  { key: 'job.stage', label: 'Job reaches a stage', entity: 'job', param: { key: 'stage', label: 'Stage', default: 'shipping' } },
  ...
  { key: 'separation.saved', label: 'Art is separated into screens', entity: 'job' },
]
...
export const ACTIONS = [
  ...
  { key: 'job.move', label: 'Move the job to a stage', fields: [{ key: 'stage', label: 'Stage' }] },
...
function conditionsPass(conds, ctx) {
  for (const c of conds || []) {
    ...
    switch (c.key) {
      ...
      case 'is_rush': if (!ctx.job?.rush !== (String(v) !== 'true')) return false; break
...
function fill(text, ctx) {
  ...
  const vars = {
    ...
    total: ctx.total != null ? `$${Number(ctx.total).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '',
    version: ctx.version ?? '',
    days: ctx.days ?? '',
    stage: String(ctx.job?.stage || '').replace('_', ' '),
  }
  return String(text || '').replace(/\{\{(\w+)\}\}/g, (_, k) => vars[k] ?? s[k] ?? '')
}
...
function runAction(action, ctx, deps) {
  ...
  switch (action.key) {
    case 'email.customer': {
      ...
      deps.queueEmail({ contact: ctx.contact, kind: 'automation', subject: fill(cfg.subject, ctx), template: cfg.body, vars: ctxVars(ctx) })
      return `emailed ${ctx.contact.email}`
    }
    case 'sms.customer': {
      if (!ctx.contact?.phone) return 'skipped — customer has no phone'
      deps.queueSms({ contact: ctx.contact, body: fill(cfg.body, ctx) })
    ...
    case 'job.move': {
      ...
      run('UPDATE jobs SET stage = ?, updated_at = ? WHERE id = ?', cfg.stage, now(), ctx.job.id)
 ...
 */
function executeActions(a, actions, ctx, deps, startIdx = 0) {
  const results = []
...
export function runAutomation(a, trigger, ctx, deps) {
  ...
  try {
    const { results } = executeActions(a, a.actions, ctx, deps)
    if (results?.length) run('UPDATE automation_runs SET detail = ? WHERE id = ?', results.join(' · '), runId)
  } catch (e) {
    console.error('automation action failed:', a.name, String(e.message).slice(0, 200))
  }
}
...
export function fire(trigger, ctx, deps) {
  for (const a of listAutomations()) {
    ...
    if (a.trigger === 'job.stage' && a.params.stage && ctx.job?.stage !== a.params.stage) continue
...
export function tick(deps) {
  ...
  // 'ran' OR 'error' both block a re-fire: a timed rule that errored mid-send must not retry every
  // 5 minutes and re-send to the customer. (A shop can re-enable the rule to try again.)
  ...
  // Resume drip sequences whose wait has expired. Delete-before-run so a crash can't loop
  // the same step forever; executeActions re-queues if another wait follows.
  const exists = (type, id) => id == null || !!get(`SELECT 1 AS x FROM ${type} WHERE id = ?`, id)
  for (const p of all(`SELECT * FROM automation_pending WHERE due_at <= datetime('now') ORDER BY id`)) {
    run('DELETE FROM automation_pending WHERE id = ?', p.id)
    try {
      // The rule may have been disabled or deleted during the wait — respect that, don't send.
      ...
      // drop the drip quietly rather than message about a record that no longer exists.
      if (!exists('contacts', ctx.contact?.id)) continue
      if (!exists('estimates', ctx.estimate?.id) || !exists('invoices', ctx.invoice?.id) || !exists('jobs', ctx.job?.id)) continue
    ...
    } catch (e) { console.error('pending resume failed:', e.message) }
  }
  ...
  for (const a of listAutomations()) {
    ...
    if (a.trigger === 'estimate.stale') {
      for (const e of all(`SELECT * FROM estimates WHERE status = 'sent' AND sent_at IS NOT NULL
        ...
        runAutomation(a, 'estimate.stale', { estimate: e, total: e.total, days: days || 3, contact: get('SELECT * FROM contacts WHERE id = ?', e.contact_id) }, deps)
        fired.push(`estimate.stale ${e.estimate_number}`)
      }
    }

    if (a.trigger === 'invoice.overdue') {
      for (const i of all(`SELECT * FROM invoices WHERE status != 'paid' AND due_date IS NOT NULL
          AND imported_at IS NULL
          AND due_date <= date('now', ?)`, `-${days || 1} days`)) {
        if (overBudget()) break
        if (already('invoice.overdue', i.id, a.id)) continue
        runAutomation(a, 'invoice.overdue', { invoice: i, total: round2(i.amount_due - i.amount_paid), days: days || 1, contact: get('SELECT * FROM contacts WHERE id = ?', i.contact_id) }, deps)
    ...
    if (a.trigger === 'art.waiting') {
      for (const v of all(`SELECT av.*, j.id AS jid FROM art_versions av JOIN jobs j ON j.id = av.job_id
          WHERE av.status = 'sent' AND av.sent_at <= datetime('now', ?)`, `-${days || 2} days`)) {
        if (overBudget()) break
        if (already('art.waiting', v.jid, a.id)) continue
        const job = get('SELECT * FROM jobs WHERE id = ?', v.jid)
        runAutomation(a, 'art.waiting', { job, version: v.version, days: days || 2, contact: get('SELECT * FROM contacts WHERE id = ?', job.contact_id) }, deps)
    ...
    if (a.trigger === 'job.due_soon') {
      for (const j of all(`SELECT * FROM jobs WHERE status = 'active' AND due_date IS NOT NULL
        ...
        runAutomation(a, 'job.due_soon', { job: j, days: days || 2, contact: get('SELECT * FROM contacts WHERE id = ?', j.contact_id) }, deps)
    ...
    if (a.trigger === 'job.at_risk') {
      for (const j of all(`SELECT * FROM jobs WHERE status = 'active' AND approval_gated = 1
        ...
        runAutomation(a, 'job.at_risk', { job: j, days: s.slip, contact: get('SELECT * FROM contacts WHERE id = ?', j.contact_id) }, deps)
...
export function seedAutomations() {
  if (get('SELECT COUNT(*) AS c FROM automations').c > 0) return 0
