...
export function initAutomations(db) {
  ...
  for (const [type, table] of [['estimate', 'estimates'], ['invoice', 'invoices'], ['job', 'jobs'], ['contact', 'contacts']]) {
    ...
    db.exec(`
      UPDATE automation_runs SET entity_id = NULL
 ...
 */
function executeActions(a, actions, ctx, deps, startIdx = 0, done = []) {
  const results = done
...
export function runAutomation(a, trigger, ctx, deps) {
  ...
  const done = []
  try {
    const { results } = executeActions(a, a.actions, ctx, deps, 0, done)
    run('UPDATE automation_runs SET detail = ? WHERE id = ?', (results || []).join(' · '), runId)
  } catch (e) {
    ...
    run('UPDATE automation_runs SET status = ?, detail = ? WHERE id = ?', 'error',
      `${done.length} of ${(a.actions || []).length} step(s) ran${done.length ? ` · ${done.join(' · ')}` : ''} · then failed: ${msg}`, runId)
  }
}
