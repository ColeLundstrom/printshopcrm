...
import { recipientForContext } from './billing-recipients.mjs'
/**
...
function fill(text, ctx) {
  const s = getSettings()
  const vars = {
    first_name: String(ctx._recipient?.name ?? ctx.contact?.name ?? '').split(' ')[0],
    recipient_name: ctx._recipient?.name || '',
    recipient_first_name: String(ctx._recipient?.name || '').split(' ')[0],
...
function runAction(action, ctx, deps) {
  ...
  switch (action.key) {
    case 'email.customer': {
      const recipient=recipientForContext(ctx) || {name:ctx.contact?.name || '',email:ctx.contact?.email || ''}
      if(recipient.blocked || !recipient.email)return `skipped — ${recipient.blocked || 'customer has no email'}`
      const deliveryCtx={...ctx,_recipient:recipient}
      deps.queueEmail({ contact: ctx.contact, invoice_id:ctx.invoice?.id, estimate_id:ctx.invoice?.id?null:ctx.estimate?.id,recipient_revision:recipient.revision,kind: 'automation', subject: fill(cfg.subject, deliveryCtx), template: cfg.body, vars: {...ctxVars(ctx),recipient_name:recipient.name,recipient_first_name:String(recipient.name).split(' ')[0]} })
      return `queued to ${recipient.email}`
    }
    case 'sms.customer': {
...
function invoiceContextIssue(ctx,trigger) {
  delete ctx._recipient_issue
  try { recipientForContext(ctx) } catch(error) {
    if(error.code!=='not_found') {ctx._recipient_issue=true;return error.message+' Cancel the old sequence and create a fresh document message after review.'}
  }
...
export function tick(deps) {
  ...
  for (const p of all(`SELECT * FROM automation_pending WHERE due_at <= datetime('now') AND status IS NULL ORDER BY id LIMIT ?`, TICK_BUDGET)) {
    ...
    try {
      ...
      if(issue) {park(p.id, ctx._recipient_issue?'recipient_review':'payment_review', issue);continue}
...
export const STARTER_AUTOMATIONS = [
  ...
  {
    name: 'Notify the customer when packing starts',
    trigger: 'job.stage', params: { stage: 'shipping' }, conditions: [],
    actions: [{ key: 'sms.customer', config: { body: 'Hi {{first_name}} — {{job_title}} is with our shipping team for packing. We will confirm dispatch or collection separately. — {{shop_name}}' } }],
  },
  {
