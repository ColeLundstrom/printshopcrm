...
export function jobRoi(job, settings = getSettings(), garmentRows = null) {
  ...
  // Revenue is what the customer agreed to: the invoice amount if invoiced, else the estimate.
  const revenue = round2(inv ? inv.amount_due : est ? est.total : 0)
...
const colorsOf = (it) => {
  if (it.locations) return Math.max(...it.locations.map((l) => Number(l.colors) || 1), 1)
  const m = String(it.detail || '').match(/(\d+)\s*colou?r/i)
