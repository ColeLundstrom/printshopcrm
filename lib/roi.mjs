...
// The shop's real cost model for the five decorations that are not screen print — vendor cost
// matrices, service margins and the patch price breaks. It was written for the browser price
// calculator and, until this round, `grep -rn servicecost` over the whole repo returned exactly
// one importer: public/js/views/quote.js. Nothing server-side had ever read it. It is pure
// arithmetic with no DOM and no dependencies, so this is not a new runtime dependency.
import { servicePerPiece, SERVICE_METHODS } from '../public/js/shared/servicecost.js'
 ...
 */
export function laborActualMinutes(jobId, { now = Date.now(), scans: prefetched = null } = {}) {
  // `scans` lets the shop-wide sweep pass this job's rows in from one query instead of running
  // one per job — see shopRoi(). Same rows, same order.
  const scans = prefetched || all("SELECT from_stage, to_stage, created_at FROM job_scans WHERE job_id = ? AND source = 'scan' ORDER BY created_at, id", jobId)
  let enteredAt = null
  let minutes = 0
  let suspect = 0
...
/**
 * `pre` is the shop-wide sweep's prefetch — the estimates, invoices, sibling runs and press scans
 * for EVERY job, in four queries instead of four per job. shopRoi() runs over every job a shop has
 * ever had, so per-job lookups were 60,003 statements and 18.7 seconds of a blocked event loop on
 * a five-year shop, with /health failing while it ran. Omitted (the single-job /api/roi/:id route)
 * everything falls back to its own query, unchanged.
 */
export function jobRoi(job, settings = getSettings(), garmentRows = null, pre = null) {
  ...
  // When `pre` is present its maps are loaded from the whole table, so a miss means the row really
  // does not exist — falling back to a query there would put the per-job cost straight back for
  // every job that has no invoice yet, which is most of an active shop's board.
  const est = !job.estimate_id ? null
    : pre ? (pre.estimates.get(job.estimate_id) || null) : get('SELECT * FROM estimates WHERE id = ?', job.estimate_id)
  const inv = job.invoice_id
    ? (pre ? (pre.invoicesById.get(job.invoice_id) || null) : get('SELECT * FROM invoices WHERE id = ?', job.invoice_id))
    : job.estimate_id
      ? (pre ? (pre.invoicesByEstimate.get(job.estimate_id) || null) : get('SELECT * FROM invoices WHERE estimate_id = ?', job.estimate_id))
      : null
  ...
  // One query per job is one full sweep of the jobs table per job when this is not prefetched —
  // shopRoi() hands the whole map down, the same way it already hands down the garment catalog.
  const runs = !job.estimate_id ? []
    : pre ? (pre.runs.get(job.estimate_id) || [])
  ...
  let garment = 0, labor = 0, screensCost = 0, decoCost = 0, pieces = 0, colors = 0, matched = true
  const up = (() => { try { return JSON.parse(settings.size_upcharges) } catch { return {} } })()

  /**
   * What this line's decoration actually costs the shop.
   *
   * Profitability booked press-time labour for EVERY job and no consumable cost at all, because
   * this file never contained the word "decoration". Five of the seven methods in the shipped
   * price book — DTF, UV DTF, embroidery, patch and laser — have no press, no screens and a real
   * per-piece material cost, and all of it was invisible. Measured on a 500-piece DTF order at
   * $8.00/pc: the shop spends $4,182.00 and LOSES $182.00, and the screen reported $2,005.50
   * profit at 50.1% margin, "Healthy", with `losers: 0` and the page headline reading
   * "Every job is profitable."
   *
   * Returns null for screen print, whose press-time model below is correct and unchanged.
   * The print-size defaults must match quote.js's, or the calculator and this screen disagree
   * about the same order.
   */
  const decorationCostFor = (it, qty) => {
    const method = it?.decoration || job.decoration || 'Screen Print'
    if (!SERVICE_METHODS.includes(method)) return null
    const svc = servicePerPiece(method, qty, {
      widthIn: it?.print_w, heightIn: it?.print_h,
      patchSizeIn: it?.patch_size, liveEmbroidery: !!it?.live_embroidery,
    })
    if (!svc) return null
    // Laser has no material cost line, only a sell tier. quote.js:130 halves the sell price for
    // exactly this case; use the same fallback so the two screens agree.
    const per = svc.cost != null ? svc.cost : round2((svc.sell || 0) * 0.5)
    return { method, cost: round2(qty * per), fee: svc.fee || 0 }
  }
  ...
  if (items.length) {
    ...
    for (const it of items) {
      ...
      if (it.sizes) {
        ...
        // garment_cost -> blank_cost -> read the text, which is the precedence lineBlankCost()
        // defines in public/js/shared/pricing.js. This read `it.garment_cost` alone, so it honoured
        // only the Price Calculator. lib/quickquote.mjs stamps `blank_cost` instead — and that is
        // the LIVE distributor price it just looked up, on every line Autopilot, the Slack
        // quick-quote, the receptionist and the assistant write. Ignoring it sent those jobs to the
        // catalogue for a different, usually cheaper number: a 300-piece run quoted off a live
        // $6.42 blank was costed at the catalogue's $4.10 and reported a 48% "Healthy" margin on a
        // job actually running at 31.9%, under the shop's own 45% floor.
        const stamped = Number(it.garment_cost) > 0 ? Number(it.garment_cost)
          : Number(it.blank_cost) > 0 ? Number(it.blank_cost)
          : 0
        const c = stamped > 0 ? { cost: stamped, matched: true } : costFor(it.description, garmentRows)
        if (!c) matched = false
        if(!it.matrix?.customerSupplied)garment += round2(qty * (c?.cost || estimateBlank(it)) * spoil)
        const lineColors = colorsOf(it, jobColors)
        colors = Math.max(colors, lineColors)
        pieces += qty
        // A DTF transfer, a patch or a stitched cap does not go on a press, so press minutes are
        // the wrong model for it AND the material it consumes was being costed at zero. Book the
        // decoration; book press labour only for the method the press formula describes.
        const deco = decorationCostFor(it, qty)
        if (deco) decoCost += round2(deco.cost + deco.fee)
        else labor += (pressMinutes(qty, lineColors, press) / 60) * (shopRate / util)
    ...
    // Same rule on the no-estimate path, which reads the job's own decoration column directly.
    const deco = decorationCostFor(null, pieces)
    if (deco) {
      decoCost = round2(deco.cost + deco.fee)
      screensCost = 0                    // there are no screens on a DTF or embroidery job
    } else {
  ...
  if (share !== 1) {
    ...
    decoCost = round2(decoCost * share)
  ...
  const measured = laborActualMinutes(job.id, pre ? { scans: pre.scans.get(job.id) || [] } : {})
  ...
  const cost = round2(garment + labor + screensCost + decoCost)
  ...
  return {
    ...
    breakdown: { garment: round2(garment), labor: round2(labor), screens: screensCost, decoration: round2(decoCost) },
...
export function shopRoi({ completedOnly = false } = {}) {
  ...
  const { garmentRows, pre } = loadRoiContext()
  const rows = jobs.map((j) => jobRoi(j, settings, garmentRows, pre)).filter((r) => r.revenue > 0)
...
/** Shared read snapshot for bulk cost models; no per-job SQL in a shop sweep. */
export function loadRoiContext() {
  const garmentRows = all('SELECT * FROM garments')   // load the catalog ONCE for the whole sweep
  // Which runs share an order, in ONE query rather than one per job. Same reason as the catalog:
  // this sweep runs over every job the shop has ever had.
  const group = (rows, key) => {
    const m = new Map()
    for (const r of rows) { const list = m.get(r[key]); if (list) list.push(r); else m.set(r[key], [r]) }
    return m
  }
  const byId = (rows) => new Map(rows.map((r) => [r.id, r]))
  // Four queries for the whole shop, not four per job. This sweep runs over every job a shop has
  // ever had: per-job lookups cost 60,003 statements and blocked the event loop for 18.7 seconds
  // on a five-year shop, with /health failing while it ran — a page nobody can wait for.
  const invoices = all('SELECT * FROM invoices ORDER BY id')
  const pre = {
    estimates: byId(all('SELECT * FROM estimates')),
    invoicesById: byId(invoices),
    invoicesByEstimate: new Map(invoices.filter((i) => i.estimate_id != null).reverse().map((i) => [i.estimate_id, i])),
    runs: group(all('SELECT id, sizes, estimate_id FROM jobs WHERE estimate_id IS NOT NULL ORDER BY id'), 'estimate_id'),
    scans: group(all("SELECT job_id, from_stage, to_stage, created_at FROM job_scans WHERE source = 'scan' ORDER BY job_id, created_at, id"), 'job_id'),
  }
  return { settings: getSettings(), garmentRows, pre }
}
