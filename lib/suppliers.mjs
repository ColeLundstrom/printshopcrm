...
export async function submitPurchaseOrder(po, settings, { dryRun = false, poNumber } = {}) {
  ...
  if (st.ss) {
    ...
    // A 2xx is not a placed order. `res.json().catch(() => ({}))` turns any body that is not the
    // expected JSON — a WAF interstitial, an nginx error page served with 200, a TLS-inspecting
    // corporate proxy — into {}, and this used to return ok:true with order_id null. The route
    // reads `ok` alone, writes status 'submitted', logs "recorded for S&S Activewear", and
    // board.js toasts "Order placed". Nothing was ordered, and after that no screen in the
    // product can send it again: Submit answers already:true without a network call, short-close
    // moves it to 'closed' which is also in poAlreadySent(), and there is no delete.
    //
    // lib/gdrive.mjs:214 already carries this guard, in the same words, for the same reason. This
    // is the last call in the tree that spends the shop's money on a 2xx alone.
    const orderId = data.orderNumber || data.orderId || data.id || null
    if (!orderId) {
      throw new Error(`S&S accepted the request but returned no order number (HTTP ${res.status}) — the order was NOT placed. Check your S&S portal before sending it again.`)
    }
    return { ok: true, supplier: 'S&S Activewear', order_id: orderId, lines, raw: data }
  }
