 ...
 */
import { all, get, run, now, tx, shopFormat } from './db.mjs'
import { countableText } from './text.mjs'
...
export function initSuppliers(db) {
  ...
  // Additive evidence only. A legacy status is never migrated into a human acknowledgement.
  db.exec(`CREATE TABLE IF NOT EXISTS po_manual_confirmations (
    po_id INTEGER PRIMARY KEY REFERENCES purchase_orders(id) ON DELETE CASCADE,
    supplier TEXT NOT NULL, reference TEXT NOT NULL, manifest TEXT NOT NULL,
    recorded_by TEXT NOT NULL, confirmed_at TEXT NOT NULL);
    CREATE TRIGGER IF NOT EXISTS po_manual_confirmation_immutable BEFORE UPDATE ON po_manual_confirmations
    BEGIN SELECT RAISE(ABORT,'Manual purchase acknowledgement is immutable'); END;`)
...
export function costFor(garmentText, preloadedRows = null) {
  ...
  //
  // The style test used to be a bare `t.includes(style)`, taking the FIRST row that matched by
  // rowid. A style number is a prefix of other style numbers, so a youth or premium garment was
  // costed and ORDERED as the adult basic sitting earlier in the table:
  //
  //   "Gildan 5000B youth tee"  -> G500     (5000B is in the catalogue, as G500B)
  //   "Bella+Canvas 3001CVC"    -> BC3001   (3001CVC is in the catalogue)
  //   "Bella 3001Y"             -> BC3001
  //   "Gildan 42000"            -> G200     ("42000" contains "2000")
  //
  // Nothing warned, because `matched` was true. This is the call that spends real money at the
  // distributor: the PO went out for the wrong blank at the wrong price. So a style must now match
  // on its own — not inside a longer alphanumeric run — and the most specific catalogue entry wins
  // rather than the earliest one.
  const styled = rows.filter((g) => g.style).sort((a, b) => String(b.style).length - String(a.style).length)
  const hit = styled.find((g) => styleMatches(t, g.style))
  // A style hit names THIS garment, so it may hand back a SKU.
  if (hit) return { cost: hit.cost, sku: hit.sku, name: `${hit.brand} ${hit.style}`, matched: true }
  // The brand+type and bare-type fallbacks below do NOT name this garment — they find something
  // vaguely like it. They were returning a real SKU with `matched: true`, so every brand outside
  // the shipped 43-row catalogue was quietly ordered as a Gildan or Bella basic at that basic's
  // price: "Nike DV7299 Dri-FIT Tee" ordered 300 × G500 at $3.20 with warnings: [], and ROI
  // reported an 84.9% margin on it. "Carhartt K87", "Adidas A430", "Under Armour 1376843" and
  // "Custom cut-and-sew heavyweight hoodie" all did the same thing.
  //
  // They remain useful as a BALLPARK for costing a blank we do not stock — that is all they ever
  // were — so they keep the cost and lose the SKU, and say they are approximate. A null SKU is
  // what buildPurchaseOrder already warns on and what /po/submit already refuses.
  const approx = rows.find((g) => t.includes(g.brand.toLowerCase()) && t.includes(g.type))
    || rows.find((g) => t.includes(g.type))
  return approx
    ? { cost: approx.cost, sku: null, name: `${approx.brand} ${approx.style}`, matched: false, approximate: true }
    : null
}

/**
 * Does `text` name this style as a style, rather than merely contain its characters?
 *
 * "5000" appears inside "5000B" and "2000" inside "42000"; neither is the same garment. The style
 * has to sit on its own, so anything alphanumeric either side disqualifies it. Exported so the
 * gate can pin the four real mismatches this had been producing.
 */
export function styleMatches(text, style) {
  const st = String(style || '').toLowerCase()
  if (!st) return false
  const i = String(text || '').toLowerCase().indexOf(st)
  if (i < 0) return false
  const t = String(text || '').toLowerCase()
  const before = i > 0 ? t[i - 1] : ''
  const after = i + st.length < t.length ? t[i + st.length] : ''
...
export function parseGarmentText(text) {
  // A phone number and a date are both punctuated runs of digits, and both were read as a style
  // code: "call us at 714-555-1234 about 48 tees" parsed style 714, which is the number a
  // supplier lookup would then have been sent. Neither is ever a style. countableText takes them
  // out; it leaves everything else exactly as typed, so "Gildan 5000" is untouched.
  const raw = countableText(String(text || '')).trim()
...
export async function psSoap({ endpoint, action, xml, timeoutMs = 10000 }) {
  ...
  // Bound the streamed body, not just Content-Length (which a remote service may omit).
  const chunks=[];let bytes=0
  for await (const chunk of res.body) {bytes+=chunk.length;if(bytes>4e6)throw new Error('Supplier response exceeds the supported size');chunks.push(chunk)}
...
/**
 * The colour of the BLANK — not the colour of the ink.
 *
 * This used to be `PO_COLORS.find(c => text.includes(c))`, which returns the first colour in the
 * ARRAY rather than the first in the TEXT. The estimate writes the blank and the imprint on one
 * line, so the ink spec won and a whole run was ordered in the wrong colour with no warning
 * anywhere — a colour *was* found, so `if (!color)` never fired:
 *
 ...
 *
 * Two rules, both about WHERE the colour lives rather than which colours exist:
 *  1. Parentheses are an ink spec. Never read a blank's colour out of them.
 *  2. The garment's colour sits after the style, in the "Style — Colour" shape the estimate
 *     writes and that `buildPurchaseOrder` already splits on. Scan that segment first; fall back
 *     to the whole string when it names none, so "Black Gildan 5000 — 3/0 front" still resolves
 *     rather than trading a wrong order for a missing one.
 * Within a scope the EARLIEST colour in the text wins, so "Charcoal Heather" is charcoal.
 */
const scanColor = (scope) => {
  let best = null
  for (const c of PO_COLORS) {
    const m = new RegExp(`\\b${c}\\b`).exec(scope)
    if (m && (best === null || m.index < best.at)) best = { color: c, at: m.index }
  }
  return best?.color || ''
}
const colorFrom = (text) => {
  const raw = String(text || '').toLowerCase().replace(/\([^)]*\)/g, ' ') // rule 1: ink is not the blank
  const dash = raw.indexOf('—')
  return (dash >= 0 && scanColor(raw.slice(dash + 1))) || scanColor(raw) // rule 2: colour segment first
}
...
export function buildPurchaseOrder(job, sizes, garmentText, settings) {
  ...
  const st = supplierStatus(settings)
  const warnings = []
  // `!match` was too narrow: an approximate hit is not a match, and it used to arrive carrying a
  // real SKU and matched:true, so the shop saw no warning at all before ordering the wrong blank.
  if (!match || !match.matched) {
    warnings.push(match
      ? `No catalog SKU matched "${style}" — costed as ${match.name} for planning only. Set the exact style before submitting.`
      : 'No catalog SKU matched — set the exact style before submitting.')
  }
  ...
  return {
    ...
    // This builder resolves style-level catalog codes, not supplier size/color identifiers.
    // Even a single-size run must not send an unverified style code as an order identifier.
    submission_ready: false,
    submission_note: st.ss
      ? 'Exact supplier SKUs for each size and color are not connected to this PO. Download it and place the order in the supplier portal, then record the supplier confirmation here.'
      : 'Download this PO and place the order with your supplier. Record it here only after the supplier has confirmed the order. This software does not submit SanMar or AlphaBroder orders.',
    warnings,
  }
}

/**
 * The purchase order for a WHOLE job — every garment on it, not just the first one.
 *
 * buildPurchaseOrder above takes one style and one grid, which is all a job used to carry: the
 * convert step merged every line's sizes into a single flat grid and kept the first line's
 * description as `garment`. A tees-and-hoodies order therefore bought the tee style for the
 * whole quantity and never ordered the hoodies at all — the shop found out on press day.
 *
 * `lines` is jobs.line_sizes: [{ description, garment, sizes }], one entry per sized quote line.
 * Each is costed against its OWN style, then consolidated together, so two styles stay two
 * groups (consolidatePoLines keys on sku|style + colour + size) and the no-split-shipment
 * guarantee is unchanged. Warnings are prefixed with the garment they belong to, because
 * "No catalog SKU matched" is useless when a PO covers three styles.
 */
export function buildJobPurchaseOrder(job, lines, settings) {
  const sized = (lines || []).filter((l) => l && l.sizes && Object.values(l.sizes).some((n) => Number(n) > 0))
  if (sized.length <= 1) {
    const one = sized[0]
    return buildPurchaseOrder(job, one?.sizes || {}, one?.description || one?.garment || '', settings)
  }
  const parts = sized.map((l) => buildPurchaseOrder(job, l.sizes, l.description || l.garment, settings))
  const merged = consolidatePoLines(parts.flatMap((p) => p.lines))
  return {
    ...parts[0],
    garment: parts.map((p) => p.garment).filter(Boolean).join(' + '),
    color: null, // multiple garments, multiple colours — the per-line `color` is the truth
    lines: merged,
    total_units: merged.reduce((s, l) => s + l.qty, 0),
    est_cost: Math.round(merged.reduce((s, l) => s + l.qty * (l.unit_cost || 0), 0) * 100) / 100,
    warnings: parts.flatMap((p, i) => p.warnings.map((w) => `${parts[i].garment || `line ${i + 1}`}: ${w}`)),
  }
}
 ...
 *
 * A missing receipt does not prove failure. In-flight and uncertain records remain fenced
 * across process death; a manager can record the existing supplier confirmation after review.
 */
export function poAlreadySent(po) {
  // A crashed/expired request may already have reached the supplier. Time is not evidence
  // that another chargeable order is safe. This is a retry fence, not a success verdict.
  return ['submitting', 'submission_uncertain', 'submitted', 'placed_manually', 'partial', 'received', 'closed'].includes(String(po?.status || ''))
}
...
export async function submitPurchaseOrder(po, settings, { dryRun = false, poNumber } = {}) {
  ...
  const st = supplierStatus(settings)
  const handoff = {
    ok: false, pending: true, code: 'po_manual_required', supplier: po.supplier || null, lines,
    note: po.submission_note || 'Place the order with the supplier, then record their confirmation here. Nothing was submitted.'
  }
  if (!dryRun && po.submission_ready === false) return handoff
  ...
  // And every line needs its OWN SKU. consolidatePoLines keys on (sku|style, colour, size) so a
  // size run correctly stays four rows — but the shipped catalogue is style-level (G500 is "Gildan
  // 5000", not "Gildan 5000 White M"), so all four rows carry the same one. The S&S body below is
  // `{identifier, qty}` and has no size or colour field at all, so a 226-piece S/M/L/XL run left
  // the building as four identical lines of "G500": 226 pieces of one unknown size, charged, while
  // the local PO record still showed a correct grid so nothing looked wrong until the boxes came.
  //
  // A per-size distributor SKU only exists on a live lookup, which this synchronous builder does
  // not have, so the honest move is to refuse. It is the same rule the brand fallbacks follow one
  // screen up: a visible dead end is recoverable, a confidently wrong order is not. The PO
  // DOCUMENT is untouched — it has always carried size and colour per line — so "download it and
  // place it by hand" is a real answer, and it is the one the message gives.
  const cells = new Map()
  for (const l of lines) {
    const k = String(l.sku).toUpperCase()
    if (!cells.has(k)) cells.set(k, new Set())
    cells.get(k).add(`${String(l.color || '').toLowerCase()}|${String(l.size || '').toUpperCase()}`)
  }
  const collision = [...cells.entries()].find(([, set]) => set.size > 1)
  if (collision) {
    throw new Error(`SKU ${collision[0]} covers ${collision[1].size} different size/colour cells — the distributor needs one SKU per size and colour, and this order would arrive as ${collision[1].size} lots of the same item. Match the exact sizes against the distributor, or download the PO and place it by hand.`)
  }
  if (dryRun) return { ok: true, dry_run: true, supplier: po.supplier, lines, total_units: lines.reduce((s, l) => s + l.qty, 0) }
  if (!st.ss) return handoff
  ...
  if (st.ss) {
    ...
    let res, data
    try {
      res = await fetch(`${SS_BASE}/orders/`, {
        method: 'POST',
      ...
      // Error text can contain credentials supplied to the provider. Never echo it into
      // activity, the purchase record or the browser, and never call an uncertain send failed.
      if (!res.ok) { await res.body?.cancel(); throw new Error('Provider did not confirm') }
      const reader = res.body?.getReader()
      if (!reader) throw new Error('Missing response body')
      const bytes=Buffer.alloc(1024*1024); let length=0
      try {
        while (true) {
          const {done,value}=await reader.read(); if(done)break
          if(length+value.byteLength>bytes.length)throw new Error('Response too large')
          bytes.set(value,length);length+=value.byteLength
        }
        data=JSON.parse(bytes.subarray(0,length).toString('utf8'))
      } finally { await reader.cancel().catch(()=>{}); reader.releaseLock() }
    } catch {
      throw Object.assign(new Error('The supplier has not confirmed the outcome. Check your supplier order history before placing anything again.'), {code:'po_submission_uncertain',expose:true})
    }
    // A success status without a usable supplier receipt still has an unknown outcome.
    const orderId = data?.orderNumber || data?.orderId || data?.id || null
    if (!['string','number'].includes(typeof orderId) || !/^[A-Za-z0-9][A-Za-z0-9._/-]{0,119}$/.test(String(orderId))) {
      throw Object.assign(new Error('S&S answered without an order number. The outcome is unknown; check your S&S order history before placing anything again.'), {code:'po_submission_uncertain',expose:true})
    }
    return { ok: true, supplier: 'S&S Activewear', order_id: orderId, lines }
  }
...
export async function blankCost(garmentText, settings, { color = '', size = '', timeoutMs = 6000, allowLive = true } = {}) {
  ...
  // `matched` was hardcoded true here, so even an explicitly approximate catalogue guess came back
  // to ROI and the PO as a confirmed blank. Carry what costFor actually decided.
  const cat = costFor(garmentText)
  if (cat) return { cost: cat.cost, source: 'catalog', live: false, sku: cat.sku, name: cat.name, matched: cat.matched !== false, approximate: !!cat.approximate, error }
...
export function blankCostLabel(b) {
  if (!b) return ''
  const bits = [b.name || 'Blank', shopFormat().money(b.cost)]
...
const poError = (message, code, status=400) => Object.assign(new Error(message), {code,status,expose:true})
const poManifest = po => JSON.stringify({job:po.job,po_number:po.po_number,supplier:po.supplier||'',lines:consolidatePoLines(po.lines).map(l=>({sku:l.sku,style:l.style,color:l.color,size:l.size,qty:l.qty,unit_cost:l.unit_cost}))})
export function purchaseOrderPlacement(po) {
  if (po?.manual_confirmation) return 'confirmed_manual'
  if (po?.order_id && po?.submitted_at) return 'confirmed_supplier'
  if (po?.status === 'submitting') return 'submitting'
  if (po?.status === 'submission_uncertain' || po?.status === 'failed') return 'unknown'
  if (po && ['placed_manually','submitted','partial','received','closed'].includes(po.status)) return 'unverified_legacy'
  return 'manual_required'
}

/** The preview and acknowledgement use the same persisted ordered quantities, never current
 * job edits masquerading as an old PO. The digest binds a stale browser to exactly this review. */
export function purchaseOrderReview(job, lines, settings) {
  const built=buildJobPurchaseOrder(job,lines,settings)
  built.po_number=`PSC-${job.job_number}`
  const prior=get('SELECT id FROM purchase_orders WHERE job_id=? AND po_number=? ORDER BY id DESC LIMIT 1',job.id,built.po_number)
  const stored=prior ? getPurchaseOrder(prior.id) : null
  const po=stored ? {...built,supplier:stored.supplier,lines:stored.lines.map(l=>({sku:l.sku,style:l.style,color:l.color,size:l.size,qty:l.qty_ordered,unit_cost:l.unit_cost})),total_units:stored.ordered,est_cost:stored.est_cost} : built
  return {...po,purchase_order:stored,placement_state:purchaseOrderPlacement(stored),review_key:createHash('sha256').update(poManifest(po)).digest('hex')}
}

/** Record a manager's supplier confirmation; this function never orders goods. */
export function confirmManualPurchaseOrder(job, review, body, actor='Manager') {
  return tx(()=>{
    if(body?.confirmed!==true)throw poError('Confirm that you checked the supplier order and its quantities.','po_confirmation_required')
    const supplier=typeof body.supplier==='string'?body.supplier.trim():''
    const reference=typeof body.reference==='string'?body.reference.trim():''
    if(!supplier || supplier.length>100 || !reference || reference.length>120 || /[\u0000-\u001f\u007f]/.test(supplier+reference))throw poError('Enter the supplier and their order or confirmation reference.','po_confirmation_reference')
    if(body.review_key!==review.review_key)throw poError('This purchase order changed. Reopen it and review the quantities.','po_review_conflict',409)
    const found=get('SELECT id FROM purchase_orders WHERE job_id=? AND po_number=? ORDER BY id DESC LIMIT 1',job.id,review.po_number)
    const current=found ? getPurchaseOrder(found.id) : null
    if(current){
      const actual={...review,supplier:current.supplier,lines:current.lines.map(l=>({sku:l.sku,style:l.style,color:l.color,size:l.size,qty:l.qty_ordered,unit_cost:l.unit_cost}))}
      if(poManifest(actual)!==poManifest(review))throw poError('This purchase order changed. Reopen it and review the quantities.','po_review_conflict',409)
    }
    if(current?.manual_confirmation){
      if(current.manual_confirmation.supplier!==supplier || current.manual_confirmation.reference!==reference || current.manual_confirmation.manifest!==poManifest(review))throw poError('This PO already has a different recorded confirmation.','po_confirmation_conflict',409)
      return {ok:true,already:true,purchase_order:current}
    }
    if(current?.order_id && current?.submitted_at)throw poError('This PO already has a supplier submission receipt.','po_already_submitted',409)
    if(current?.status==='submitting'){
      const stamp=Date.parse(String(current.updated_at||'').replace(' ','T')+'Z')
      if(!Number.isFinite(stamp) || Date.now()-stamp<5*60*1000)throw poError('A supplier submission may still be running. Wait for it to finish, then check the supplier order history.','po_submitting',409)
    }
    if(!review.lines.length)throw poError('This job has no sized quantities to order.','po_empty')
    const po=current || insertPurchaseOrder(job,review,{status:'manual_required'})
    run('INSERT INTO po_manual_confirmations(po_id,supplier,reference,manifest,recorded_by,confirmed_at) VALUES(?,?,?,?,?,?)',po.id,supplier,reference,poManifest(review),String(actor||'Manager').slice(0,120),now())
    // Receiving history and a short-close stay intact. Recording placement does not count stock.
    run("UPDATE purchase_orders SET status=CASE WHEN status IN ('partial','received','closed') THEN status ELSE 'placed_manually' END,updated_at=? WHERE id=?",now(),po.id)
    return {ok:true,already:false,purchase_order:getPurchaseOrder(po.id)}
  })
}
...
export function createPurchaseOrder(job, po, { status } = {}) {
  return tx(() => insertPurchaseOrder(job,po,{status}))
}
function insertPurchaseOrder(job, po, {status} = {}) {
...
export function getPurchaseOrder(poId) {
  ...
  const manual_confirmation=get('SELECT * FROM po_manual_confirmations WHERE po_id=?',poId)||null
  return { ...po, lines, ordered, received, short: Math.max(0, ordered - received),manual_confirmation,
    placement_state:purchaseOrderPlacement({...po,manual_confirmation}),fully_received: ordered > 0 && received >= ordered }
}
...
export function receivePurchaseOrder(poId, receipts, { by } = {}) {
  ...
  tx(() => {
    for (const r of receipts || []) {
      // A row the UI failed to build, or any caller posting a sparse array, arrives as null — and
      // `r.line_id` on null threw, so the receiving screen answered a bare 500 on the one action
      // that books goods in. Skip what is not a receipt; book what is.
    ...
    // A negative `add` walks a mis-receipt back — the clamp above has always allowed it, and the
    // receiving dialog now sends it. So the status has to be able to walk back too. It used to
    // fall through to `po.status`, which was read BEFORE the correction: an order emptied back to
    // zero still said 'received', the card still printed "✓ Fully received" over nothing, and
    // short-close still 409'd on it. The shop was out of exits a second time.
    //
    // It must never fall back to 'draft'. 'draft' is not in poAlreadySent(), so the idempotency
    // guard on /po/submit would wave a second REAL, chargeable order through. An order that has
    // been placed stays placed: 'submitted' if the distributor confirmed it, 'placed_manually'
    // if a human typed it into the portal.
    const status = rolled.fully_received ? 'received'
      : rolled.received > 0 ? 'partial'
      : ['received', 'partial'].includes(po.status) ? (po.submitted_at ? 'submitted' : 'placed_manually')
      : po.status
