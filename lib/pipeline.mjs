 ...
 */
import { all, get, run, now, round2, emitOpportunityStage } from './db.mjs'
...
export function setStage(id, stage, { lost_reason } = {}) {
  ...
  run(`UPDATE opportunities SET stage=?, won_at=?, lost_at=?, lost_reason=?, updated_at=? WHERE id=?`,
    ...
    now(), id)
  const updated = get('SELECT * FROM opportunities WHERE id = ?', id)
  // Only on a real move, and only into the two stages anything subscribes to. Guarding on
  // `stage !== o.stage` is what makes this idempotent: the approve doors legitimately run twice
  // (convert, then the portal callback), and the customer must be thanked once.
  if (stage !== o.stage && (stage === 'won' || stage === 'lost')) {
    emitOpportunityStage({ opportunity: updated, stage, from: o.stage })
  }
  return updated
}
...
export function syncFromEstimate(estimate, event) {
  ...
  // A newly revised commercial offer has no current acceptance. This explicit event is the
  // exception to forward-only sync; ordinary edits/no-ops still preserve the deal's stage.
  if (event === 'revised') {
    run('UPDATE opportunities SET contact_id=? WHERE id=?', estimate.contact_id, opp.id)
    setStage(opp.id, 'quoted')
  }
 ...
 *
 * Exported, because "what is this quote worth to the shop" was being answered independently in
 * four more places and two of them still summed `e.total`: the Follow-ups screen's "quotes gone
 * quiet" headline and the in-app assistant's answer to the same question both read $10,089.70
 * against this expression's $9,572.00 — a tenth high, growing with the tax rate, on the number a
 * shop uses to decide who to call first. One definition, imported.
 */
export const dealValue = (estimate) => round2(Number(estimate?.subtotal) || Number(estimate?.total) || 0)
