 ...
 */
import { get, run, now, getSettings, getUpcharges, computeTotals, nextEstimateNumber, nextJobNumber, logActivity, sizeTotal, rollupSizes, sizeSummary, round2, addBusinessDays, businessDaysBetween, emitContactCreated } from './db.mjs'
...
export function priceIntake(order, settings = getSettings(), { blank = null, taxRate = null } = {}) {
  ...
  const q = { ...quoteScreenPrint({ garmentCost: blankCostValue, markup, qty: pieces, locations, screenFee, darkGarment: order.dark_garment }),
    perPiece: Math.round((garment + priced.perPiece) * 100) / 100, imprintPerPiece: priced.perPiece, service: svc, setup: priced.setup,
  ...
  const items = [{
    ...
    detail: `${order.dark_garment ? 'Dark garment, underbase incl. ' : ''}${order.rush ? 'RUSH. ' : ''}${stated > grid && grid > 0 ? `Sizes given for ${grid} of ${pieces} — confirm the breakdown. ` : ''}`.trim(),
  ...
  return { pieces, quote: q, items, totals, taxRate: effTaxRate }
}
...
export async function quickQuote({ text, contact_name, contact_email, contact_phone, source = 'quote', createJob = false }) {
  ...
  if (createJob) {
    ...
    const dueDate = hint || addBusinessDays(today, order.rush ? 3 : 10)
    const turnaround = hint ? Math.max(1, businessDaysBetween(today, dueDate)) : (order.rush ? 3 : 10)
    const grid = rollupSizes(priced.items)
    const jobId = Number(run(
      'INSERT INTO jobs (contact_id, estimate_id, job_number, title, status, stage, decoration, garment, sizes, quantities, due_date, turnaround_days, approval_gated, notes, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      contact.id, estId, nextJobNumber(), `${priced.pieces} ${order.garment}`, 'active', 'new',
      order.decoration, order.garment, JSON.stringify(grid), sizeSummary(grid),
