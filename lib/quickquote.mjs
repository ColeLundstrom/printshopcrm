 ...
 */
import { get, run, now, getSettings, shopFormat, getUpcharges, freezeUpcharges, computeTotals, nextEstimateNumber, nextJobNumber, logActivity, sizeTotal, rollupSizes, garmentLines, sizeSummary, round2, addBusinessDays, businessDaysBetween, emitContactCreated } from './db.mjs'
...
const money = (n) => shopFormat().money(round2(n))
 ...
 */
export function resolveContact({ name, email, phone, notes, source = 'quote', emitAutomation = true }) {
  ...
  if (emitAutomation) emitContactCreated(created)
...
export function priceIntake(order, settings = getSettings(), { blank = null, taxRate = null } = {}) {
  ...
  // Stamp the upcharge table these lines are priced with onto the lines, before the totals are
  // taken from them — otherwise the stored subtotal and the printed lines part company the next
  // time the shop edits a rate. See freezeUpcharges in lib/db.mjs.
  items = freezeUpcharges(items)
 ...
 */
export async function quickQuote({ text, contact_name, contact_email, contact_phone, source = 'quote', createJob = false, beforeCommit = () => {}, emitAutomation = true }) {
  ...
  // Finish all external parsing/pricing before creating records. Slack rechecks the employee
  // at this boundary, so revoking access while a supplier/model is pending prevents the write.
  const priced = await priceIntakeLive(order, settings)
  ...
  const { contact, created, unassigned } = resolveContact({
    ...
    notes: order.notes || '', source, emitAutomation,
  })

  if (contact.tax_exempt) {
    priced.taxRate = 0
    priced.totals = computeTotals(priced.items, 0, getUpcharges())
  }

  const estNum = nextEstimateNumber()
  // rushDays travels with the price (see priceIntake) so the JOB is scheduled and flagged on the
  // same tier the customer was billed for. Storing it on the estimate is what lets the convert
  // route do the same for a quote a human sends and the customer approves later.
  const estId = Number(run(
    'INSERT INTO estimates (contact_id, estimate_number, status, items, subtotal, tax, total, tax_rate, notes, rush_days, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
    contact.id, estNum, 'draft', JSON.stringify(priced.items),
    priced.totals.subtotal, priced.totals.tax, priced.totals.total, priced.taxRate, order.notes || '',
  ...
  if (createJob) {
    ...
    const jobId = Number(run(
      'INSERT INTO jobs (contact_id, estimate_id, job_number, title, status, stage, decoration, garment, sizes, line_sizes, quantities, due_date, turnaround_days, approval_gated, rush, notes, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      ...
      dueDate, turnaround, 0, priced.rushDays > 0 ? 1 : 0, order.notes || '', now(), now()).lastInsertRowid)
