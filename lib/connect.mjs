...
import { requestStripe, createStripeCollectionClient } from './stripe.mjs'
...
/** Destination charges stay on the captured platform account; never add Stripe-Account. */
export function createConnectedCollectionClient(settings,{send=fetch,timeoutMs=15000,secret:injectedSecret}={}) {
  const secret=injectedSecret??platformSecret(),destination=String(settings?.stripe_account_id || '')
  const enabled=String(settings?.stripe_charges_enabled || '')
  if(!connectReady(settings))throw new Error('This shop has not finished connecting Stripe')
  return createStripeCollectionClient(settings,{send,timeoutMs,secret,destination,feePct:FEE_PCT,provider:'stripe_connect',
    matchesSettings:current=>String(current?.stripe_account_id || '')===destination && String(current?.stripe_charges_enabled || '')===enabled &&
      (injectedSecret!==undefined?injectedSecret:platformSecret())===secret})
}
...
async function stripe(path, params = {}, method = 'POST') {
  ...
  return requestStripe(path, { secret: sk, method, ...(method === 'POST' ? { body: form(params) } : {}) })
}
 ...
 */
export async function createConnectedCheckout({ accountId, lineItems, successUrl, cancelUrl, metadata = {}, customerEmail, currency = 'USD' }) {
  ...
  const fee = Math.round(stripeUnits(total,currency) * FEE_PCT)
  const params = {
    mode: 'payment',
  ...
  lineItems.forEach((li, i) => {
    ...
    params[`line_items[${i}][price_data][currency]`] = currencyCode(currency).toLowerCase()
