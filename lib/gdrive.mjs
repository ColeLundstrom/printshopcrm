...
export async function ensureFolder({ accessToken, name, parentId, apiBase, fetch }) {
  ...
  // Same rule as uploadFile: a 2xx with no id is not a folder we can put anything in.
  if (!created.data?.id) return { ok: false, status: created.status, error: 'Drive created the folder but returned no id', data: created.data }
...
export async function uploadFile({ accessToken, name, mimeType, buffer, folderId, uploadBase = DEFAULT_UPLOAD_BASE, fetch = globalThis.fetch }) {
  try {
    ...
    // A 2xx is not a stored file. `res.json().catch(() => ({}))` turns any body that is not the
    // expected JSON — a TLS-inspecting corporate proxy, an nginx error page served with 200, a
    // truncated response — into {}, and this used to return ok:true with id undefined. The caller
    // reads ok and DELETES the shop's only local copy of the customer's artwork, so a 200 that
    // stored nothing destroyed the file. Never report success without the id that proves it.
    if (!data.id) return { ok: false, status: res.status, error: 'Drive accepted the upload but returned no file id', data }
