...
const id = (value, prefix) => {
  ...
  return typeof raw === 'string' && new RegExp(`^${prefix}_[A-Za-z0-9_]{1,240}$`).test(raw) ? raw : null
}
...
function legacyProductMatches(subscription, plan) {
 ...
 */
export function createPlatformSubscriptionEventProcessor({ getTenantById, getTenantByStripeCustomer, setSubscription, retrieveSubscription, maxConcurrent = 4 }) {
  ...
  const unchanged = (original, customerId) => {
    ...
    if (!customerMatches(current, customerId) || ['stripe_subscription_id', 'stripe_customer_id', 'subscription_status', 'plan_tier']
  ...
  return async function processPlatformSubscriptionEvent(event) {
    ...
    if (event.type === 'checkout.session.completed' || event.type === 'checkout.session.async_payment_succeeded') {
      ...
      if (!alreadyBound) {
        ...
        const explicit = obj.metadata?.purpose === HOSTING_PURPOSE && subscription.metadata?.purpose === HOSTING_PURPOSE
        const legacy = missing(obj.metadata?.purpose) && missing(subscription.metadata?.purpose) && legacyProductMatches(subscription, plan)
        if (!explicit && !legacy) return ignored('unverified_hosting_product')
      ...
      if (original.stripe_subscription_id && !alreadyBound) {
        ...
        if (!TERMINAL.has(previous.status) || TERMINAL.has(subscription.status)) return ignored('existing_subscription')
      }
