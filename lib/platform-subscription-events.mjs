...
export function createPlatformSubscriptionEventProcessor({ getTenantById, getTenantByStripeCustomer, setSubscription, retrieveSubscription, hostingCheckouts, maxConcurrent = 4 }) {
  ...
  const reconcile = async (tenant, callback, options = {}) => {
    ...
    try {
      return await (hostingCheckouts?.withTenantBillingOperation
        ? hostingCheckouts.withTenantBillingOperation(tenant.id, callback, options) : callback(() => {}))
    } finally { inFlight.delete(tenant.id) }
  }
  ...
  return async function processPlatformSubscriptionEvent(event) {
    ...
    if (event.type === 'checkout.session.completed' || event.type === 'checkout.session.async_payment_succeeded') {
      ...
      return reconcile(original, async assertActive => {
      ...
      if (original.stripe_subscription_id && !alreadyBound) {
        ...
        if (!TERMINAL.has(previous.status) || TERMINAL.has(subscription.status)) {
          ...
          return { ...ignored('existing_subscription'), verificationResolved: true }
        }
      }
      ...
      }, { verification: { session_id: id(obj.id,'cs'), subscription_id: subscriptionId, customer_id: customerId,
        plan, purpose: obj.metadata?.purpose } })
    }
    ...
    return reconcile(tenant, async assertActive => {
