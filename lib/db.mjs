import { initShopCrmSchema } from './shop-crm-schema.mjs'
import { initPaymentCollectionSchema } from './payment-collection-schema.mjs'
import { API_RETRY_SCHEMA } from './api-retries.mjs'
import { initAddressSchema } from './addresses.mjs'
import { initBillingRecipientSchema } from './billing-recipient-schema.mjs'
import { initShippingSchema } from './shipping-schema.mjs'
import { initEstimateApprovalSchema, DEFAULT_ESTIMATE_TERMS } from './estimate-approval-schema.mjs'
import { initSlackOperator } from './slack-operator-schema.mjs'
import { initCostingSchema } from './costing-schema.mjs'
import { initProductionSchema } from './production-schema.mjs'
import { DatabaseSync } from 'node:sqlite'
import { mkdirSync, existsSync, statSync, openSync, readSync, closeSync } from 'node:fs'
...
/**
 * Is this file actually a database?
 *
 * Sixteen bytes off the front. Every SQLite file on earth begins with the same header string.
 *
 * This lived in lib/tenants.mjs, private to it, and guarded control.db and every per-shop
 * database — but not the default handle opened below, which in a single-shop install IS the whole
 * shop. tenants.mjs imports from this file, so it could never have imported back. A rule every
 * opener owes belongs where every opener can reach it, which is the same reason statusForStage
 * and stampShipDate live here.
 */
export const isSqliteFile = (path) => {
  let fd
  try {
    if (statSync(path).size === 0) return false
    fd = openSync(path, 'r')
    const b = Buffer.alloc(16)
    return readSync(fd, b, 0, 16, 0) === 16 && b.toString('latin1') === 'SQLite format 3\0'
  } catch { return false } finally { try { if (fd !== undefined) closeSync(fd) } catch { /* nothing to close */ } }
}
...
export const SCHEMA = API_RETRY_SCHEMA + `
...
-- Source artwork and machine handoff files are independent of customer-facing proof versions.
-- A technical release records staff review, not a claim that a machine has validated the file.
CREATE TABLE IF NOT EXISTS art_assets (
  id INTEGER PRIMARY KEY,
  job_id INTEGER NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
  role TEXT NOT NULL CHECK(role IN ('source','production')),
...
CREATE INDEX IF NOT EXISTS idx_art_assets_job ON art_assets(job_id,id);
CREATE TRIGGER IF NOT EXISTS trg_art_assets_immutable BEFORE UPDATE ON art_assets
  BEGIN SELECT RAISE(ABORT,'Artwork asset metadata is immutable; upload a new file'); END;

CREATE TABLE IF NOT EXISTS art_releases (
  id INTEGER PRIMARY KEY,
  job_id INTEGER NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
  art_revision INTEGER NOT NULL,
  proof_id INTEGER REFERENCES art_versions(id) ON DELETE SET NULL,
  proof_snapshot TEXT NOT NULL,
  source_manifest TEXT NOT NULL,
  production_manifest TEXT NOT NULL,
  specs TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_art_releases_job ON art_releases(job_id,id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_art_releases_active ON art_releases(job_id) WHERE revoked_at IS NULL;
CREATE TRIGGER IF NOT EXISTS trg_art_releases_immutable BEFORE UPDATE ON art_releases
  WHEN NEW.id IS NOT OLD.id OR NEW.job_id IS NOT OLD.job_id OR NEW.art_revision IS NOT OLD.art_revision
    OR (NEW.proof_id IS NOT OLD.proof_id AND NEW.proof_id IS NOT NULL)
    OR NEW.proof_snapshot IS NOT OLD.proof_snapshot OR NEW.source_manifest IS NOT OLD.source_manifest
    OR NEW.production_manifest IS NOT OLD.production_manifest OR NEW.specs IS NOT OLD.specs
    OR NEW.notes IS NOT OLD.notes OR NEW.reviewed_by IS NOT OLD.reviewed_by
    OR NEW.reviewed_by_id IS NOT OLD.reviewed_by_id OR NEW.reviewed_at IS NOT OLD.reviewed_at
    OR (OLD.revoked_at IS NOT NULL AND (NEW.revoked_at IS NOT OLD.revoked_at
      OR NEW.revoked_reason IS NOT OLD.revoked_reason OR NEW.revoked_by IS NOT OLD.revoked_by OR NEW.revoked_by_id IS NOT OLD.revoked_by_id))
  BEGIN SELECT RAISE(ABORT,'Artwork release snapshots are immutable'); END;
CREATE TRIGGER IF NOT EXISTS trg_art_release_proof_delete BEFORE DELETE ON art_versions
  BEGIN UPDATE art_releases SET revoked_at=CURRENT_TIMESTAMP,revoked_reason='Referenced customer proof deleted',revoked_by='System'
    WHERE proof_id=OLD.id AND revoked_at IS NULL; END;

-- A browser retry returns the existing draft instead of duplicating its files and proof.
-- Keep the receipt after proof deletion so a reused row id cannot adopt that request.
CREATE TABLE IF NOT EXISTS mockup_composition_receipts (
  job_id INTEGER NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_mockup_composition_proof ON mockup_composition_receipts(proof_id);
CREATE TRIGGER IF NOT EXISTS trg_mockup_composition_receipts_immutable BEFORE UPDATE ON mockup_composition_receipts
  WHEN NEW.job_id IS NOT OLD.job_id OR NEW.request_id IS NOT OLD.request_id OR NEW.request_hash IS NOT OLD.request_hash
    OR (NEW.proof_id IS NOT OLD.proof_id AND NEW.proof_id IS NOT NULL)
    OR NEW.proof_filename IS NOT OLD.proof_filename OR NEW.proof_sha256 IS NOT OLD.proof_sha256
    OR NEW.proof_version IS NOT OLD.proof_version OR NEW.revision IS NOT OLD.revision
    OR NEW.composition_json IS NOT OLD.composition_json OR NEW.created_at IS NOT OLD.created_at
  BEGIN SELECT RAISE(ABORT,'Mockup composition receipts are immutable'); END;
...
// Same shape, same reason, for money arriving. POST /api/invoices/:id/payments does four things
// after the INSERT that lib/ cannot reach — advanceOrder, fireAuto('invoice.paid'), the webhook
// dispatch inside it, and enqueueQbo — because all four are module-private to server.mjs. So a
// payment recorded from the in-app assistant was invisible to QuickBooks, to the shipped "Thank
// the customer when they pay" rule, to every invoice.paid subscriber, and to the Orders board.
// `before` is the invoice's status BEFORE the payment, so the listener can fire invoice.paid
// exactly once, on the payment that actually settled it.
let paymentRecordedHook = null
export const onPaymentRecorded = (fn) => { paymentRecordedHook = fn }
export const emitPaymentRecorded = (p) => { try { paymentRecordedHook?.(p) } catch (e) { console.error('payment.recorded hook:', e && e.message) } }

// And a third, for the same reason, on the stage a deal lands in. `opportunity.won` and
// `opportunity.lost` are declared triggers, published webhook events and two of the eleven starter
// automations — and their only firing site was the hand-drag route in server.mjs. Deals are
// actually won by APPROVING THE QUOTE, which goes through lib/pipeline.mjs setStage(), where
// fireAuto is out of reach. So setStage emits and server.mjs listens, and there is one path again.
// `from` is the stage the deal was in before, so a listener can tell a real move from a no-op.
let opportunityStageHook = null
export const onOpportunityStage = (fn) => { opportunityStageHook = fn }
export const emitOpportunityStage = (e) => { try { opportunityStageHook?.(e) } catch (err) { console.error('opportunity.stage hook:', err && err.message) } }
 ...
 */
// Node 22.13 has no DatabaseSync.isTransaction. Track transactions opened by this helper so
// mutation modules can enforce atomic participation without abandoning that supported runtime.
const activeTransactions = new WeakSet()
export const inTransaction = (dbh = getDb()) => typeof dbh.isTransaction === 'boolean' ? dbh.isTransaction : activeTransactions.has(dbh)
...
function applyMigrations(dbh) {
  // Do not enroll existing jobs or infer readiness from an old customer approval. Shops opt in
  // per job, and only a new staff release can authorize the separate production file manifest.
  addColumn(dbh, 'jobs', 'art_revision', 'INTEGER NOT NULL DEFAULT 0')
  addColumn(dbh, 'jobs', 'art_release_required', 'INTEGER NOT NULL DEFAULT 0')
  addColumn(dbh, 'art_versions', 'purpose', "TEXT NOT NULL DEFAULT 'legacy_proof'")
  addColumn(dbh, 'art_versions', 'source_asset_id', 'INTEGER REFERENCES art_assets(id) ON DELETE SET NULL')
  addColumn(dbh, 'art_versions', 'composition_json', 'TEXT')
  addColumn(dbh, 'price_matrices', 'customer_supplied', 'INTEGER NOT NULL DEFAULT 0')
  addColumn(dbh, 'price_matrices', 'decoration', "TEXT NOT NULL DEFAULT ''")
  ...
  addColumn(dbh, 'invoices', 'payment_review', "TEXT DEFAULT ''")
  addColumn(dbh, 'invoices', 'credit_base', 'TEXT')
  addColumn(dbh, 'email_log', 'invoice_id', 'INTEGER')
  addColumn(dbh, 'email_log', 'payment_stale', 'INTEGER DEFAULT 0')
  dbh.exec(`CREATE TABLE IF NOT EXISTS invoice_credits (
    reference TEXT PRIMARY KEY, invoice_id INTEGER NOT NULL REFERENCES invoices(id) ON DELETE RESTRICT,
    subtotal_cents INTEGER NOT NULL, tax_cents INTEGER NOT NULL, reason TEXT NOT NULL,
    currency TEXT NOT NULL, created_at TEXT NOT NULL, cancelled_at TEXT, cancel_reason TEXT
  ); CREATE INDEX IF NOT EXISTS idx_credit_invoice ON invoice_credits(invoice_id);`)
  dbh.exec(`CREATE TABLE IF NOT EXISTS payment_reversals (
    provider TEXT NOT NULL, provider_id TEXT NOT NULL, kind TEXT NOT NULL, attempt_ref TEXT NOT NULL,
    invoice_id INTEGER REFERENCES invoices(id) ON DELETE RESTRICT, amount_cents INTEGER NOT NULL,
    status TEXT NOT NULL, applied_cents INTEGER NOT NULL DEFAULT 0, is_test INTEGER NOT NULL DEFAULT 0,
    revision INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
    PRIMARY KEY(provider,provider_id)
  ); CREATE INDEX IF NOT EXISTS idx_reversal_invoice ON payment_reversals(invoice_id);
     CREATE INDEX IF NOT EXISTS idx_reversal_attempt ON payment_reversals(attempt_ref);`)
  ...
  // The deal an estimate opened must not outlive the estimate either. opportunities.estimate_id is
  // ON DELETE SET NULL, not CASCADE, so deleting a quote left the deal behind with a null pointer:
  // still 'quoted', still carrying the dead quote's value in Open Pipeline and Weighted Forecast,
  // and no longer findable by oppForEstimate — so it could never be re-valued, and a re-created
  // quote for the same customer minted a SECOND deal beside it.
  // BEFORE DELETE, because the FK's SET NULL erases the pointer this matches on.
  // Only source='estimate' — the deals lib/pipeline.mjs opened. 'manual' is a deal a person typed
  // and 'ai-receptionist' is one the website agent opened; neither is ours to bin.
  // No retroactive sweep: an orphan already in the field still draws in the Quoted column and
  // DELETE /api/opportunities/:id exists, so a human can clear it — and a blanket
  // `estimate_id IS NULL AND source='estimate'` sweep on every boot would eat a deal an API
  // caller had legitimately posted with that source.
  dbh.exec(`
    CREATE TRIGGER IF NOT EXISTS trg_opportunities_estimate_delete BEFORE DELETE ON estimates
      BEGIN DELETE FROM opportunities WHERE estimate_id = OLD.id AND source = 'estimate'; END;
  `)
  ...
  // qbo_sync.entity_id was the LAST parent pointer in the schema with no foreign key behind it,
  // and it is polymorphic ('invoice' + a rowid), so it cannot have one. Same rowid-reuse shape the
  // art_versions trigger above fixes, with a worse ending.
  //
  // invoices.contact_id cascades, and DELETE /api/contacts/:id lets a customer go when their only
  // invoice is VOIDED — which is the supported way to retract one raised in error. That deletes
  // the invoice and leaves its qbo_sync row pointing at a rowid SQLite hands straight to the next
  // invoice the shop raises. Three things then go wrong at once, all silent:
  //   - the QuickBooks queue screen LEFT JOINs invoices on entity_id, so the old failure is
  //     relabelled with the NEXT customer's invoice number and amount;
  //   - "Retry" on that row pushes an invoice nobody asked to push;
  //   - worst, enqueueQbo() returns early when a row for the id is already 'pending'/'retrying'/
  //     'syncing', so the new invoice is NEVER queued. The money moves, QuickBooks never hears
  //     about it, and the books diverge with no error anywhere — the exact silent gap the whole
  //     reconciliation queue exists to prevent.
  // A dead invoice's push receipt is not a receipt, it is a live landmine. Sweep the orphans an
  // existing database is already carrying, then let the trigger be the foreign key we cannot write.
  dbh.exec(`
    DELETE FROM qbo_sync WHERE entity = 'invoice' AND entity_id NOT IN (SELECT id FROM invoices);
    CREATE TRIGGER IF NOT EXISTS trg_qbo_sync_invoice_delete AFTER DELETE ON invoices
      BEGIN DELETE FROM qbo_sync WHERE entity = 'invoice' AND entity_id = OLD.id; END;
  `)
  ...
  // Rescue deals filed under a stage that does not exist.
  //
  // The website receptionist wrote stage 'qualified' on its happy path, and 'qualified' is in no
  // STAGE_KEYS. pipelineBoard() builds its columns from STAGES, so the card was drawn in NO column
  // at all: it could not be opened, edited, dragged or corrected from any screen in the product —
  // a state a human cannot get out of. Meanwhile the board's own header counted its value and the
  // Dashboard's KPI did not, so two screens reported different open pipeline out of one table.
  //
  // 'lead' is the honest landing place: it understates rather than overstates the forecast, and it
  // is a real column the shop can drag the card out of. Latched, because a stage a person has
  // since typed is theirs — and the fix upstream means nothing writes an unknown one again.
  once(dbh, 'opportunity_stage_is_a_real_stage', () => {
    dbh.exec("UPDATE opportunities SET stage = 'lead' WHERE stage NOT IN ('lead','quoted','sent','negotiation','won','lost')")
  })
  ...
  // The rush the PRICE was built from, carried on the document that charged for it.
  //
  // v1.18.0 and v1.19.0 taught every automated quoting path to charge the shop's published rush
  // tier — +50% on a 3-day, double on next-day. Not one of them was taught to PRODUCE a rush. The
  // job that came out of a rush quote carried `rush 0` and the default 10-day turnaround, so the
  // board's Rush filter was permanently empty, the work ticket had no RUSH banner, Floor Mode
  // showed no pill, and lib/capacity.mjs's rush tiebreak could never fire. On a 300-piece order
  // billed $4,280.00 instead of $2,870.00 for a 3-day turnaround, the job was promised 2026-09-02,
  // projected 2026-09-11 the moment it was created, and rewritten to 2026-09-14 at proof approval.
  // Eight working days past the date the customer paid extra for. 0 means standard.
  addColumn(dbh, 'estimates', 'rush_days', 'INTEGER DEFAULT 0')
  ...
  // POST /api/outbox/:id/send read `delivered`, then awaited SMTP, then wrote — so two Send
  // clicks (a double-click, a retried tap on a slow phone, two staff on the same queue) both
  // passed the already-sent check and the customer got the message twice from one email_log row.
  // This column is the claim the sibling drains all have: qbo_sync goes 'syncing', webhooks move
  // next_attempt_at, POs go 'submitting'. Time-stamped rather than a flag, so a process killed
  // mid-send releases the row instead of stranding it — nothing else can clear it.
  addColumn(dbh, 'email_log', 'sending_at', 'DATETIME')
  ...
  dbh.exec(`
    ...
    -- jobs.invoice_id was the one foreign key in the schema with no index behind it. invoices
    -- CASCADE from contacts, so deleting a customer runs its ON DELETE SET NULL over the WHOLE
    -- jobs table once per invoice, and POST /api/invoices/:id/void scans it on every void. Same
    -- reason idx_jobs_estimate and idx_opp_contact exist. node:sqlite is synchronous and every
    -- shop on the box shares the process, so that scan blocks the event loop for all of them.
    ...
    -- email_log had no index of any kind, and it is append-only with no retention: /api/outbox
    -- full-scans the whole table twice on every load. Measured on a three-year shop (16k rows,
    -- ~30 MB): 25.42 ms -> 0.033 ms. Partial, in the style of idx_msg_unread above, because the
    -- Outbox only ever asks for what has not gone out yet.
    CREATE INDEX IF NOT EXISTS idx_email_undelivered ON email_log(id) WHERE delivered = 0;
    -- opportunities had no index of any kind. oppForEstimate() runs on the write path of every
    -- quote in the product — create, send, approve, convert, and the quick-quote path — and it was
    -- a full SCAN of a table the app appends to for every estimate the shop ever writes: 1.18ms
    -- per call at 29,910 rows, about 10% of a POST /api/estimates, growing forever. contact_id is
    -- an FK too, so deleting a customer scanned it for the cascade.
    CREATE INDEX IF NOT EXISTS idx_opp_estimate ON opportunities(estimate_id);
    CREATE INDEX IF NOT EXISTS idx_opp_contact ON opportunities(contact_id);
    -- The dashboard's Pipeline card is now one GROUP BY over stage; covering, so it never touches
    -- the table.
    CREATE INDEX IF NOT EXISTS idx_opp_stage_value ON opportunities(stage, value);
  `)
}

/** Apply PRAGMAs + schema + migrations to a database handle. Idempotent. */
export function initDb(dbh) {
  // busy_timeout FIRST, and this order is the whole point.
  //
  // Wait up to 5s for a write lock instead of throwing SQLITE_BUSY — the 5-min automation tick and
  // a live request can briefly contend on the same tenant db. It was set one statement AFTER
  // `journal_mode = WAL`, and setting journal_mode is itself a locking operation: it needs an
  // exclusive lock to convert the file. `VACUUM INTO` — which is how bin/snapshot.mjs and every
  // backup this product takes write their copies — produces a file in `journal_mode=delete`, so
  // every RESTORED database needs that conversion on first open. With any second reader attached,
  // the conversion throws `database is locked` at 0 ms: no wait, no retry, the exact failure the
  // timeout exists to prevent, on the exact path a shop is on when it is already having its worst
  // day. Measured: throws at 0 ms in the shipped order, survives at 1366 ms with the timeout set
  // first.
  ...
  dbh.exec(SCHEMA)
  applyMigrations(dbh)
  initAddressSchema(dbh)
  initEstimateApprovalSchema(dbh)
  initBillingRecipientSchema(dbh)
  initPaymentCollectionSchema(dbh)
  initProductionSchema(dbh)
  initShippingSchema(dbh)
  initSlackOperator(dbh)
  initCostingSchema(dbh)
  initShopCrmSchema(dbh)
  return dbh
}

/*
 * A zero-byte file is not a database, and neither is a truncated one.
 *
 * `new DatabaseSync(path)` CREATES on open, and initDb then writes the whole current schema into
 * whatever it opened. So a printshop.db left empty or half-written — a full disk mid-write, an
 * interrupted cp, a truncated restore, a volume that came back blank — was adopted silently as a
 * brand-new shop. Driven on a single-shop install carrying a customer, EST-1001, INV-1001 and a
 * $1,200 payment, truncated to 0 bytes: the ordinary banner printed, /api/dashboard answered 200
 * with every KPI at zero, /api/contacts returned [], and BOTH /health and /health?strict=1 said
 * ok — which is exactly the signal deploy/ship.sh polls to decide whether to roll a release back,
 * so the deploy that lost the books reports itself green and the shop starts re-minting EST-1001
 * over numbers it has already issued.
 *
 * control.db and every per-shop database have refused this since lib/tenants.mjs was written
 * (`refuseControl`, and `tenant_db_missing`). The default handle — the whole shop on a
 * single-shop install, which is what INSTALL.md's own quick start produces — never got it.
 *
 * Not a throw: this is module scope, before server.mjs installs its startup handlers, so a throw
 * here is a raw stack trace ending in `PRAGMA journal_mode` under a Restart=always loop. Say what
 * is wrong, say nothing was overwritten, and name the tool that fixes it.
 */
if (existsSync(DB_PATH) && !isSqliteFile(DB_PATH)) {
  let size = '?'
  try { size = String(statSync(DB_PATH).size) } catch { /* report what we can */ }
  console.error(`\n  PrintShopCRM will not start — the database at ${DB_PATH} is not a database (${size} bytes).`)
  console.error(`\n  Starting anyway would write a fresh, empty schema over it and report itself healthy:`)
  console.error(`  every screen would load, every total would read zero, and /health would answer ok.`)
  console.error(`  Nothing has been overwritten. Restore it from a snapshot:\n`)
  console.error(`    npm run restore -- /path/to/backup\n`)
  process.exit(1)
}
...
export function canWrite(dbh = getDb()) {
  ...
  try {
    ...
    if (process.env.PSC_IMPORT_DIAGNOSTICS === '1' && performance.now() - probeAt > 500) console.log('[import-diagnostics]', JSON.stringify({ health_write_ms: Math.round(performance.now() - probeAt) }))
    return { ok: true }
  } catch (e) {
...
const SETTING_DEFAULTS = {
  ...
  shop_address: '',
  tax_rate: '0',
  // How the shop writes money and dates. Every amount the app prints — screens, PDFs, emails, the
  // assistant, the public estimate and pay pages, the gang-sheet embed — goes through
  // public/js/shared/format.js with these two. ISO 4217 code and a BCP 47 locale; anything the
  // runtime does not recognise falls back to these defaults rather than failing on a customer
  // document. The defaults are what the app always did, so nothing changes for an existing shop.
  currency: 'USD',
  locale: 'en-US',
  ...
  // The last QuickBooks company this shop was connected to, remembered ACROSS a disconnect. It is
  // deliberately not in DISCONNECT_GROUPS.quickbooks: surviving the disconnect is the whole point.
  // See the realm-change guard in /api/qbo/callback.
  qbo_last_realm_id: '',
  ...
  anet_login_id: '', anet_transaction_key: '', anet_signature_key: '', anet_environment: 'sandbox', anet_currency: 'USD',
  api_access: 'full', // preserve existing integrations; setup creates new agent keys read-only
  ...
  estimate_terms: DEFAULT_ESTIMATE_TERMS,
...
export const SECRET_KEYS = [
  'payment_callback_key', 'stripe_webhook_secret', 'anet_transaction_key', 'anet_signature_key',
...
/**
 * Settings that are the SERVER's own proof of something, and are never sent to a client.
 *
 * Distinct from SECRET_KEYS: those are the shop's credentials, and the form needs to know whether
 * each one is set (hence the `_set` flags), so they are blanked rather than removed. These are not
 * the shop's at all — they are single-use nonces the server generates and then compares against.
 *
 * qbo_oauth_state / gdrive_oauth_state were on neither list, so GET /api/settings — which has no
 * requireRole; only its `members` array is gated — handed the live OAuth state to every signed-in
 * account of any role. That state is the ONLY thing standing between a caller and
 * /api/qbo/callback, so a staff member could read it, consent to the shop's own Intuit app against
 * their OWN QuickBooks company, and repoint the shop's books. The Drive half is worse: with
 * gdrive_refresh_token theirs, every subsequent customer proof uploads into their account and the
 * local copy is deleted.
 */
export const INTERNAL_KEYS = ['qbo_oauth_state', 'gdrive_oauth_state']
 ...
 */
/*
 * The same rule, for the values a ROUTE establishes rather than a person types.
 *
 * stripe_account_id and stripe_charges_enabled are exactly the pair connectReady() tests, and
 * together they are where the money goes: createConnectedCheckout puts stripe_account_id in
 * payment_intent_data.transfer_data.destination and in on_behalf_of, so it is both the payout
 * destination and the merchant of record on the customer's card statement. They were in
 * SETTING_DEFAULTS, therefore in SETTINGS_WRITABLE, and in neither SECRET_KEYS nor this set — so
 * one `PUT /api/settings {"stripe_account_id":"acct_…","stripe_charges_enabled":"1"}` repointed
 * every card payment the shop takes. POST /api/stripe/connect and its status route are
 * deliberately requireRole('owner'); this was the same capability at manager, and there was no
 * screen that undid it either.
 *
 * The Google and QuickBooks token expiries and gdrive_connected are the same shape: written only
 * by the OAuth callbacks and the refresh path, read as truth about a live connection. Nothing in
 * public/ has ever posted any of these — the Settings page renders them as badges, never as
 * fields — so refusing them costs the product nothing.
 */
const SETTINGS_NOT_PATCHABLE = new Set([
  'payment_callback_key',
  'shop_logo',
  'stripe_account_id', 'stripe_charges_enabled',
  'qbo_token_expires', 'gdrive_token_expires', 'gdrive_connected',
  'qbo_last_realm_id',
])
// Settings whose stored value is a JSON document rather than a scalar. Kept beside the patcher
// because the only thing that reads this list is the object-shape branch below.
const JSON_SETTING_KEYS = new Set(['size_upcharges', 'service_pricing', 'price_book', 'reorder_snooze', 'onboarding_progress'])
...
export function applySettingsPatch(patch) {
  for (const [k, v] of Object.entries(patch || {})) {
    ...
    // A JSON-shaped setting can legitimately arrive as an object — it is the natural shape, it is
    // what docs/API.md's own settings examples imply, and it is what a shop's integration sends.
    // setSetting does String(value), and String({}) is "[object Object]". Every reader then throws
    // on JSON.parse and silently substitutes the shipped default, so PUT /api/settings answered
    // 200 for a change that did not happen — and for size_upcharges that means the shop goes on
    // billing +$2.00 on a 2XL it charges +$3.00 for, on every quote, invoice, PDF and export,
    // while holding a success response. Serialise it instead of stringifying it.
    if (JSON_SETTING_KEYS.has(k) && v !== null && typeof v === 'object') { setSetting(k, JSON.stringify(v)); continue }
    setSetting(k, v)
  }
}
...
import { moneyFormatter } from '../public/js/shared/format.js'

/**
 * The formatter for the CURRENT shop — its currency and locale, read from settings on each call so
 * a change in Settings is live on the next request. This is the only way server code should turn
 * a number into money: server.mjs, the assistant, the receptionist, Slack, automations and the
 * quick-quote engine all had their own `'$' + toLocaleString('en-US')`, which is why a UK shop was
 * invoicing in dollars.
 */
export const shopFormat = () => {
  const s = getSettings()
  return moneyFormatter({ currency: s.currency, locale: s.locale })
}
 ...
 */
// `??` only falls back on undefined/null, and `cp .env.example .env` — the documented first step
// of every install path — set this to the EMPTY STRING. Number('') is 0, and 0 means "keep
// everything", so every install that followed the docs silently switched webhook retention from
// 30 days to forever, on a table that grows with every outbound delivery. Blank means unset; an
// explicit "0" still means keep everything.
export function pruneWebhookDeliveries(days = Number(String(process.env.PSC_WEBHOOK_RETENTION_DAYS ?? '').trim() || 30)) {
 ...
 */
/**
 * The seven columns of the Job Board, and the ONLY values `jobs.stage` may hold.
 *
 * It lived in server.mjs, which lib/automations.mjs cannot import (cycle) — so the automation
 * action "Move the job to a stage" wrote whatever string was typed into a free-text box, and a
 * stage no column matches is a job that vanishes from the board while every count still includes
 * it. Here, so the engine, the routes and the builder all measure against one list.
 */
...
/** A job is 'complete' exactly when it is in the Complete column. Four writers already agree. */
export const statusForStage = (stage) => (stage === 'complete' ? 'complete' : 'active')

/**
 * Stamp the day a job actually shipped, on the first crossing into shipping or complete.
 *
 ...
 * ordinary order, printed on the document the receiving clerk signs. The board drag was taught to
 * stamp it, then POST /api/scan and POST /api/v1/jobs/:id/stage, and the helper's own docstring
 * then claimed it was "called by all three, so the fourth writer cannot forget".
 *
 * There were five. lib/automations.mjs's `job.move` action and lib/assistant.mjs's doMoveJob both
 * write `stage` directly and never stamped it: an automation move printed a packing slip 72 days
 * stale, and "move JOB-1002 to shipping" in the assistant printed one 122 days stale. No route or
 * field anywhere in the product writes ship_date, so neither is fixable from any screen.
 *
 * It lives here, next to statusForStage, because that is the other rule every stage-writer has to
 * obey and it is here for exactly the same reason — the automations action was once the one
 * stage-writer of five that forgot THAT one. A helper that lives in server.mjs cannot be obeyed by
 * the two writers that live in lib/.
 *
 * Stamped only on the first crossing: a card dragged back and forth, or a ticket re-scanned at the
 * pack bench, must not keep rewriting the day the box actually went out.
 */
export function stampShipDate(job, stage) {
  if (stage !== 'shipping' && stage !== 'complete') return
  if (!job || job.ship_date) return
