...
export async function deliverWebhook(rawUrl, payload, { timeoutMs = 10000, resolver = dns, secret = '', event = '' } = {}) {
  ...
  // The lookup a rebind can't move: whatever hostname the stack tries to resolve, it gets the
  // address we already checked, and that address was re-validated against the block list.
  const pinnedLookup = (_hostname, _opts, cb) => {
    if (isBlockedIp(pinnedIp)) { cb(new Error('blocked address')); return }
