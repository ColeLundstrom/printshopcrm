...
/**
 * Did the payload actually reach an endpoint? Only a 2xx says so.
 *
 * This read `< 400`, so a 3xx was recorded as 'delivered' — and since redirects are deliberately
 * NOT followed (following one would re-open the SSRF via a Location the guard never vetted), the
 * body reached nothing at all. An endpoint that moves is the ordinary case: a re-pointed Zapier
 * hook, an SSO login redirect in front of the handler, a plain http URL the host now 301s to
 * https. Every event for that subscription was dropped, never retried, and the shop's Developers
 * log said "delivered" the whole time. docs/API.md tells integrators only a 2xx counts.
 *
 * Exported so the rule can be unit-tested without a socket.
 */
export function deliveryVerdict(statusCode) {
  const code = Number(statusCode)
  if (code >= 200 && code < 300) return { ok: true, error: null }
  if (code >= 300 && code < 400) {
    return { ok: false, error: `HTTP ${code} — redirects are not followed; point the subscription at the final URL` }
  }
    ...
    return await new Promise((resolve) => {
      const req = lib.request(
        ...
        (res) => {
          ...
          resolve({ ...deliveryVerdict(res.statusCode), status: res.statusCode, url: safe })
        },
      )
