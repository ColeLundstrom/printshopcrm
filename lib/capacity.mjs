...
export function promise(jobs, settings, { pieces = 0, colors = 1, press, dueDate } = {}) {
  const capInfo = dailyCapacity(settings)
  const cap = capInfo.minutes
  // The same bound schedule() carries, for the same reason — a job runs on ONE press, so it cannot
  // finish faster than one press can image it however many the shop owns. schedule() was taught
  // this and promise() was not, and promise() is the only function in the product that answers the
  // CUSTOMER-facing question: /api/capacity/promise renders its verdict verbatim on the front desk
  // as "Yes — ships by Aug 31". Once that same job was actually booked, the Capacity table's
  // "Projected finish" column — driven by schedule(), in this file, from the same inputs — said
  // Sep 2. Two screens, two functions, one job, two answers, and the over-promise scaled with the
  // station count: 1/2/4/8 presses answered Sep 2 / Sep 1 / Aug 31 / Aug 31 against schedule()'s
  // Sep 2 every time. So the more presses a shop bought, the more it over-committed.
  const perJobPerDay = Math.max(1, Math.round(cap / Math.max(1, capInfo.stations)))
