...
async function stripeRead(settings,path,send=fetch) {
  ...
  const r=await send('https://api.stripe.com/v1/'+path,{headers:{Authorization:`Bearer ${settings.stripe_secret}`},signal:AbortSignal.timeout(15000),redirect:'error'})
  const data=await r.json().catch(()=>null)
  if(!r.ok || !data) throw new Error(data?.error?.message || 'Stripe could not verify the refund')
  return data
}
