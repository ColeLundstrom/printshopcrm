...
export const AGENT_SCOPES = {
  ...
  'production:read': 'Read tasks, department queues and team names',
  'production:write': 'Complete, assign and schedule production tasks'
}
const fail = (message,status=400) => { throw Object.assign(new Error(message),{status}) }
const date = () => new Date().toISOString()
const hash = token => crypto.createHash('sha256').update(token).digest('hex')
const publicKey = row => row && ({id:row.id,name:row.name,prefix:row.prefix,member_id:row.member_id,scopes:JSON.parse(row.scopes),created_at:row.created_at,expires_at:row.expires_at,revoked_at:row.revoked_at,last_used_at:row.last_used_at,...(row.member_status!==undefined?{member_name:row.member_name,member_active:row.member_status==='active' && ['owner','manager'].includes(row.member_role)}:{})})
...
export function requiredAgentScope(method,path) {
  ...
  if(read && (['/api/v1/production/queue','/api/v1/production/team'].includes(p) || /^\/api\/v1\/jobs\/\d+\/workflow$/.test(p)))return 'production:read'
...
export function createAgentAccess(control) {
  ...
  return {
    list(tenantId) { return control.prepare('SELECT k.*,m.name member_name,m.status member_status,m.role member_role FROM agent_keys k JOIN members m ON m.id=k.member_id AND m.tenant_id=k.tenant_id WHERE k.tenant_id=? ORDER BY k.id DESC').all(tenantId).map(publicKey) },
